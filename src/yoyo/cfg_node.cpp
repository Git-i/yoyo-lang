...
struct CFGPreparator {
    ...
    void operator()(IfExpression* expr) {
        ...
        auto else_node = expr->else_expr
                             ? node->manager->newNode(depth, "if_else")
                             : nullptr;
        auto cont = node->manager->newNode(depth, "if_cont");
        if (!else_node) node->addChild(cont);
        node->addChild(then);
        auto then_prep =
            CFGPreparator{then, exit, depth, break_to, continue_to};
        std::visit(then_prep, expr->then_expr->toVariant());

        CFGNode* goto_if_not_cont = nullptr;
        if (then_prep.node != exit && then_prep.node != break_to &&
            then_prep.node != continue_to) {
        ...
        if (else_node) {
            ...
            auto else_prep =
                CFGPreparator{else_node, exit, depth, break_to, continue_to};
            std::visit(else_prep, expr->else_expr->toVariant());
            if (else_prep.node != exit && else_prep.node != break_to &&
                else_prep.node != continue_to) {
                else_prep.node->addChild(cont);
                expr->else_transfers_control = false;
            } else {
                else_node->debug_name += "(surrenders control)";
                if (goto_if_not_cont == continue_to)
                    goto_if_not_cont = else_prep.node;
        ...
        node = cont->parents.empty()
                   ?
                   // if both branches transfer control we have a few
                   // possibilites
                   // - one branch returns (we continue generating from the
                   // other branch)
                   // - one branch breaks and the other continues (we
                   // continuw from the breaking branch
                   goto_if_not_cont
                   : cont;
    }
    ...
    void operator()(WhileStatement* stat) {
        ...
        if (while_prep.node != exit && while_prep.node != cont &&
            while_prep.node != cond)
            while_prep.node->addChild(cond);
        if (while_prep.node == exit)
            node = exit;
        else
            node = cont;
    }
    void operator()(BlockExpression* stat) {
        ...
        node->addChild(body);
        auto visistor =
            CFGPreparator{body, exit, depth + 1, break_to, continue_to};
        for (auto& sub : stat->statements) {
            std::visit(visistor, sub->toVariant());
            if (visistor.node == exit || visistor.node == break_to ||
                visistor.node == continue_to) {
                node = visistor.node;
                return;
            }
        }
    ...
    void operator()(ForStatement* stat) {
        ...
        if (then_prep.node != exit && then_prep.node != cont &&
            then_prep.node != check)
            then_prep.node->addChild(check);
        if (then_prep.node == exit)
            node = exit;
        else
            node = cont;
    }
    ...
    void operator()(ConditionalExtraction* stat) {
        ...
        auto else_node = stat->else_body ? node->manager->newNode(
                                               depth + 1, "cond_extract_else")
                                         : nullptr;
        auto cont = node->manager->newNode(depth, "cond_extract_cont");
        if (!else_node) node->addChild(cont);
        node->addChild(then);
        auto then_prep =
            CFGPreparator{then, exit, depth + 1, break_to, continue_to};
        std::visit(then_prep, stat->body->toVariant());
        CFGNode* goto_if_not_cont = nullptr;
        if (then_prep.node != exit && then_prep.node != break_to &&
            then_prep.node != continue_to) {
        ...
        if (else_node) {
            node->addChild(else_node);
            auto else_prep = CFGPreparator{else_node, exit, depth + 1, break_to,
                                           continue_to};
    ...
    void operator()(WithStatement* stat) {
        ...
        node->addChild(with);
        auto with_prep =
            CFGPreparator{with, exit, depth + 1, break_to, continue_to};
...
struct UsedVariablesExpression {
    bool is_first;
    std::unordered_map<std::string, Expression*> operator()(Expression*) {
        return {};
    }
    ...
    std::unordered_map<std::string, Expression*> operator()(ArrayLiteral* lit) {
        ...
        if (std::holds_alternative<std::vector<std::unique_ptr<Expression>>>(
                lit->elements)) {
            for (auto& expr :
                 std::get<std::vector<std::unique_ptr<Expression>>>(
                     lit->elements)) {
                auto st = std::visit(*this, expr->toVariant());
                for (auto& [name, use] : st) {
                    if (is_first && vars.contains(name)) continue;
    ...
    std::unordered_map<std::string, Expression*> operator()(
        StringLiteral* lit) {
        std::unordered_map<std::string, Expression*> vars;
    ...
    std::unordered_map<std::string, Expression*> operator()(ScopeOperation*) {
        return {};
    }  //???
    std::unordered_map<std::string, Expression*> operator()(
        ObjectLiteral* lit) {
...
struct UsedVariables {
    bool is_first;
    std::unordered_map<std::string, Expression*> operator()(Statement*) {
        return {};
    }
    std::unordered_map<std::string, Expression*> operator()(
        ExpressionStatement* stat) {
        return std::visit(UsedVariablesExpression{is_first},
                          stat->expression->toVariant());
    }
    std::unordered_map<std::string, Expression*> operator()(
        VariableDeclaration* decl) {
        if (decl->initializer) {
            auto uses = std::visit(UsedVariablesExpression{is_first},
...
CFGNode* CFGNode::prepareFromFunction(CFGNodeManager& mgr,
...
std::unordered_map<std::string, std::set<Expression*>> findFirstUsesInternal(
    CFGNode* node, std::vector<std::set<std::string>> vars) {
    ...
    if (node->depth == vars.size())
        found_here = &vars.emplace_back();
    else if (vars.size() >= node->depth + 1)
        found_here = &vars[node->depth];
    else
        debugbreak();
    ...
    auto exists_before = [&vars](const std::string& name, uint32_t scope) {
        return std::ranges::any_of(
            std::ranges::subrange(vars.begin(), vars.begin() + scope),
            [&name](auto& set) { return set.contains(name); });
    };
    if (node->visited == true) return out;
    node->visited = true;
    for (auto child : node->children) {
        // if the child used it, but we used it too:
        //   We probably used our own local copy (child is less deep than us
        //   and the variable is defined before)
        ...
        for (auto& [var, use] : uses)
            // child is deeper, or it's shallow but the var is defined
            // before so its only first use if we don't use it
            if (child->depth >= node->depth ||
                (child->depth < node->depth &&
                 exists_before(var, node->depth))) {
                if (!init_here.contains(var))
                    out[var].insert(std::make_move_iterator(use.begin()),
...
std::unordered_map<std::string, std::set<Expression*>> findLastUsesInternal(
    CFGNode* node, std::vector<std::set<std::string>> vars) {
    std::unordered_map<std::string, std::set<Expression*>> out;
    std::set<std::string>* found_here = nullptr;
    // size is either depth + 1, more or one less(same as depth)
    ...
    std::unordered_map<std::string,
                       std::vector<std::pair<CFGNode*, std::set<Expression*>>>>
        child_uses;
    for (auto child : node->children) {
        node->looped = false;
        auto uses = findLastUsesInternal(child, vars);
        // if we looped then last uses of variables on this depth and lower
        // are not guaranteed
        if (node->looped) {
            for (auto& var_set : std::ranges::subrange(
                     vars.begin(), vars.begin() + node->depth + 1)) {
                for (auto& var : var_set) out[var].clear();
            }
        }
        for (auto& [var, use] : uses)
            // child is deeper, or it's shallow but the var is defined
            ...
            if (child->depth >= node->depth ||
                ...
                out[var].clear();
                if (!node->looped ||
                    (node->looped && !exists_before(var, node->depth + 1)))
    ...
    // if we can reach any of the uses with no branching from one use, that
    // use is not a last use
    ...
    for (auto& [var, uses] : child_uses) {
        for (auto& use : uses)
            out[var].insert(std::make_move_iterator(use.second.begin()),
                            std::make_move_iterator(use.second.end()));
    }
...
std::unordered_map<std::string, std::set<Expression*>>
    CFGNodeManager::findFirstUses() {
    return findFirstUsesInternal(root_node, {});
}

std::unordered_map<std::string, std::set<Expression*>>
    CFGNodeManager::findLastUses() {
