...
namespace Yoyo {
static bool is_type_variable(const Type& tp) { return tp.name[0] == '?'; }
static Statement* get_type_stat(const Type& type, const std::string& hash,
                                ModuleBase* const md,
                                bool add_non_generics = false) {
    if (auto [hsh, decl] = md->findGenericClass(hash, type.name); decl) {
        return decl;
    } else if (auto [this_hash, fn] = md->findGenericFn(hash, type.name); fn) {
        return fn;
    } else if (auto [hsh, itf] = md->findGenericInterface(hash, type.name);
               itf) {
        return itf;
    }
    if (add_non_generics) {
        if (auto [hsh, decl] = md->findClass(hash, type.name); decl) {
            ...
            return decl;
        } else if (auto [hsh, decl] = md->findInterface(hash, type.name);
                   decl) {
            return decl;
        }
    }
...
Statement* get_next_stat(Statement* stat, const std::string_view name) {
    if (auto cls = dynamic_cast<ClassDeclaration*>(
            stat)) {  // Class and generic class declarations
        auto it = std::ranges::find_if(cls->stats, [name](auto& elem) {
            return std::visit([](auto* arg) { return get_decl_name(arg); },
                              elem->toVariant()) == name;
        });
        if (it != cls->stats.end()) {
            return it->get();
        ...
        return get_next_stat(fn->body.get(),
                             name);  // maybe its defined in the function body
    ...
    // interfaces cannot have sub-definitions (i.e cant define struct within
    // interface)
    ...
    //        auto stat_name = std::visit([](auto* arg) { return
    //        get_decl_name(arg); }, stat->toVariant()); if (stat_name ==
    //        name) {
    //            return stat.get();
    //        }
    ...
    //     if (if_stat->else_stat) return
    //     get_next_stat(if_stat->else_stat.get(), name);
    // }
    //  else if (auto cond = dynamic_cast<ConditionalExtraction*>(stat)) {
    //      auto exists = get_next_stat(cond->body.get(), name);
    //      if (exists) return exists;
    //      if (cond->else_body) return get_next_stat(cond->else_body.get(),
    //      name);
    //  }
...
std::pair<Statement*, std::unordered_map<std::string, Type>> normalize_type(
    Type& tp, TypeCheckerState* stt, IRGenerator* irgen,
...
GenericClause* get_generic_clause(T* arg) {
    if constexpr (has_clause<T>) {
        ...
        return static_cast<GenericClause*>(nullptr);
};
template <class T>
...
bool advanceScope(Type& type, ModuleBase*& md, std::string& hash,
                  IRGenerator* irgen, bool first);
std::optional<Error> advanceScopeNormalize(
    Statement*& stat, ModuleBase*& md, std::string& hash, Type& type,
    TypeCheckerState* stt, IRGenerator* irgen,
    std::unordered_map<std::string, Type>& generic_instantiations) {
    if (stat) {
        // in generic mode

        // instead of searching the module tree, we search the AST directly
        // because the parent has not been monomorphized
        stat = get_next_stat(stat, type.name);
        if (!stat) {
            return Error(SourceSpan{}, "No entity named " + type.name);
        }
        auto clause =
            std::visit([](auto* arg) { return get_generic_clause(arg); },
                       stat->toVariant());
        ...
        for (auto i :
             std::views::iota(type.subtypes.size(), num_types_in_clause)) {
            generic_instantiations[clause->types[i]] =
                subtypes.emplace_back(stt->new_type_var());
        }
        ...
        if (auto dets = md->findClass(hash, type.name); dets.second) {
            if (std::get<0>(*dets.second) != hash + type.name + "::")
                return Error(SourceSpan{}, "Internal error");
        ...
        if (auto [hsh, interface] = md->findInterface(hash, type.name);
            interface) {
            hash = hsh + "%%" + type.name +
                   "%%interface";  // interfaces are terminal and cannot
                                   // have subtyes
        }
        if (auto alias = md->findAlias(hash, type.name); alias) {
            advanceScope(*alias, md, hash, irgen, false);
        }
        if (auto [hsh, interface] = md->findGenericInterface(hash, type.name);
            interface) {
            stat = interface;
            hash = hsh;
            auto clause =
            ...
            hash = hsh + type.name + IRGenerator::mangleGenericArgs(subtypes) +
                   "::";
            return std::nullopt;
        } else if (auto [this_hash, fn] = md->findGenericFn(hash, type.name);
                   fn) {
            stat = fn;
            hash = hsh;
            auto clause =
...
/// Similar to saturate but can switch to the AST for incomplete generic
/// types also substitutes generics given an already existing context also
/// returns the statement that the type belongs to if we followed that path
auto normalize_type(
    Type& tp, TypeCheckerState* stt, IRGenerator* irgen,
    std::unordered_map<std::string, Type> generic_instantiations)
    ...
    if (iterator.is_end()) {
        ...
        if (!from_builtins(tp)) {
            ...
            if (current_stat) {
                auto clause =
                    std::visit([](auto* a) { return get_generic_clause(a); },
                               current_stat->toVariant());
                if (clause) {
                    for (auto i : std::views::iota(0u, tp.subtypes.size())) {
                        normalize_type(tp.subtypes[i], stt, irgen,
                                       generic_instantiations);
                        generic_instantiations[clause->types[i]] =
                            tp.subtypes[i];
                    }
                    for (auto i : std::views::iota(tp.subtypes.size(),
                                                   clause->types.size())) {
                        auto var = stt->new_type_var();
                        generic_instantiations[clause->types[i]] =
                            tp.subtypes.emplace_back(var);
                    }
                }
            }
        } else
        ...
        if (type.subtypes.size() > num_subtypes) {
            debugbreak();
            irgen->error(
                Error(nullptr, "Provided more types than schema allows"));
        }
        for (auto i : std::views::iota(0u, type.subtypes.size())) {
            normalize_type(type.subtypes[i], stt, irgen,
                           generic_instantiations);
        ...
        for (auto i : std::views::iota(type.subtypes.size(), num_subtypes)) {
            ...
            //     state->add_constraint(ImplInterfaceConstraint{ var, tp,
            //     scp });
            // }
        }
        ...
        while (!iterator.is_end()) {
            type = iterator.next();
            if (auto err =
                    advanceScopeNormalize(current_stat, md, hash, type, stt,
                                          irgen, generic_instantiations)) {
                debugbreak();
            }
        }
        ...
        current_stat = current_stat
                           ? current_stat
                           : get_type_stat(tp, tp.block_hash, tp.module, true);
        if (!current_stat) {
            debugbreak();
        }
        // update instantiations for last type
        clause = std::visit([](auto* arg) { return get_generic_clause(arg); },
                            current_stat->toVariant());
    ...
    if (tp.name == "__arr_s_uneval") {
        ...
        auto size =
            std::visit(ConstantEvaluator{irgen}, size_expr->toVariant());
        if (!std::holds_alternative<uint64_t>(size.internal_repr) &&
            !std::holds_alternative<int64_t>(size.internal_repr))
            debugbreak();
        size_t val = std::holds_alternative<uint64_t>(size.internal_repr)
                         ? std::get<uint64_t>(size.internal_repr)
...
enum class NoOperatorReason { MulipleMatches, NoMatches };
std::optional<Error> generic_match(Type tp1, Type tp2, ASTNode* loc,
                                   TypeCheckerState* solver);
...
std::variant<std::pair<Type, std::vector<TypeCheckerConstraint>>,
    ...
    Type result_type;
    // we can safely deref here because references don't have any unary
    // operators defined on them by default and some operators allow &T
    // while being defined in T's module
    for (auto& [blk, ovl] :
         subject.deref().module->overloads.unary_datails_for(op)) {
        ...
        if (ovl.statement && !ovl.statement->clause.types.empty()) {
            if (!substitution_cache.contains(ovl.statement))
                for (auto& tp : ovl.statement->clause.types)
                    substitution_cache[ovl.statement][tp] =
                        state->new_type_var();
        ...
        if (generic_match(subject, ovl_subject, nullptr, state) ==
            std::nullopt) {
...
FunctionType TypeChecker::operator()(IfExpression* stat) const {
    ...
    if (stat->else_expr) {
        ...
        state->add_constraint(IfStatementConstraint{
            if_type, else_type, stat->then_transfers_control,
            stat->else_transfers_control, stat->evaluated_type, stat});
    } else
        stat->evaluated_type = Type{.name = "void", .module = core_module};
...
FunctionType TypeChecker::operator()(BlockExpression* stat) const {
    ...
    if (stat->expr)
        stat->evaluated_type =
            std::visit(targetless(), stat->expr->toVariant());
    else
        stat->evaluated_type = Type{.name = "void", .module = core_module};
    state->pop_variable_block();
    return stat->evaluated_type;
...
FunctionType TypeChecker::operator()(TryExpression* exp) const {
    ...
    state->add_constraint(EqualConstraint{
        Type{.name = "__res",
             .subtypes = {exp->evaluated_type, ret_fail},
             .module = core_module},
        std::visit(targetless(), exp->expression->toVariant()), exp
    });
    state->add_constraint(EqualConstraint{
        Type{.name = "__res",
             .subtypes = {ret_success, ret_fail},
             .module = core_module},
        state->return_type, exp
    });
...
void TypeChecker::operator()(ReturnStatement* stat) {
    ...
    if (!state->return_type.is_void() && !stat->expression) {
        irgen->error(Error(stat,
                           "Expected expression in return statement of "
                           "non-void function"));
        return;
    }
    if (stat->expression) {
        ...
        state->add_constraint(
            EqualConstraint{std::visit(new_target(state->return_type),
                                       stat->expression->toVariant()),
                            state->return_type, stat});
    }
}
...
FunctionType TypeChecker::operator()(ConditionalExtraction* stat) const {
    ...
    if (stat->then_capture_tp != ConditionalExtraction::Own) {
        ...
        // ref_conversion_result::<T> extracts to T, ref extracts to &T or
        // &mut T
        state->add_constraint(RefExtractsToConstraint{
            tp, obj_ty, stat->then_capture_tp == ConditionalExtraction::RefMut,
            stat});
    } else {
    ...
    if (!stat->else_capture.empty()) {
        ...
        if (stat->else_capture_tp != ConditionalExtraction::Own)
            state->add_constraint(ElseRefExtractsToConstraint{
                tp, else_ty,
                stat->else_capture_tp == ConditionalExtraction::RefMut, stat});
        else
    ...
    if (stat->else_body) {
        ...
        stat->evaluated_type = state->new_type_var();
        state->add_constraint(IfStatementConstraint{
            then_ty, else_ty, stat->then_transfers_control,
            stat->else_transfers_control, stat->evaluated_type, stat});
    } else
        stat->evaluated_type = Type{.name = "void", .module = core_module};
    return stat->evaluated_type;
...
FunctionType TypeChecker::operator()(IntegerLiteral* lit) const {
    ...
    state->add_constraint(CanStoreIntegerConstraint{
        lit->evaluated_type, std::stoull(lit->text), lit});
...
FunctionType TypeChecker::operator()(TupleLiteral* lit) const {
    if (target) {
        if (!target->is_tuple()) {
            irgen->error(Error(
                lit, "Cannot convert tuple literal to " + target->full_name()));
    ...
    for (auto i : std::views::iota(0u, lit->elements.size())) {
        auto elem_ty =
            std::visit(target ? new_target(target->subtypes[i]) : *this,
                       lit->elements[i]->toVariant());
...
FunctionType TypeChecker::operator()(ArrayLiteral* lit) const {
    ...
    // [<expr>, <expr> ...]
    if (auto exprs = std::get_if<std::vector<std::unique_ptr<Expression>>>(
            &lit->elements)) {
        array_size = exprs->size();
        for (auto& expr : *exprs) {
            expr->evaluated_type = sub_type;
            state->add_constraint(EqualConstraint{
                sub_type, std::visit(targetless(), expr->toVariant()), lit});
        }
    }
    // [<expr>; <size>]
    else {
        ...
        auto size_as_const =
            std::visit(ConstantEvaluator{irgen}, size->toVariant());
        if (auto as_uint =
                std::get_if<uint64_t>(&size_as_const.internal_repr)) {
            if (*as_uint > std::numeric_limits<uint32_t>::max())
                irgen->error(
                    Error(lit, "Array size too large",
                          std::format("Evaluated size is {}", *as_uint)));
            else if (*as_uint == 0)
                irgen->error(
                    Error(lit, "Array size must be a positive integer",
                          std::format("Evaluated size is {}", *as_uint)));
            array_size = static_cast<uint32_t>(*as_uint);
        } else if (auto as_int =
                       std::get_if<int64_t>(&size_as_const.internal_repr)) {
            if (*as_int <= 0)
                irgen->error(
                    Error(lit, "Array size must be a positive integer",
                          std::format("Evaluated size is {}", *as_int)));
        ...
        expr->evaluated_type = sub_type;
        state->add_constraint(EqualConstraint{
            sub_type, std::visit(targetless(), expr->toVariant()), expr.get()});
        // TODO expr type must implement clone too
    }
    lit->evaluated_type = Type{.name = "__arr_s" + std::to_string(array_size),
                               .subtypes = {sub_type},
                               .module = core_module};
    return lit->evaluated_type;
}
FunctionType TypeChecker::operator()(RealLiteral* lit) const {
    ...
    state->add_constraint(CanStoreRealConstraint{
        lit->evaluated_type, std::stod(std::string(lit->token.text)), lit});
    return lit->evaluated_type;
}
FunctionType TypeChecker::operator()(StringLiteral* str) const {
    if (target && !target->is_str()) {
        irgen->error(Error(
            str, "Cannot convert string literal to " + target->full_name()));
    }
    str->evaluated_type =
        Type{.name = "str",
             .module = irgen->module->engine->modules.at("core").get()};
    ...
    for (auto& entry : str->literal) {
        if (std::holds_alternative<std::unique_ptr<Expression>>(entry)) {
            ...
            state->add_constraint(ToStringConstraint{
                std::visit(targetless(), interp->toVariant()), interp.get()});
        }
    }
...
FunctionType TypeChecker::operator()(NameExpression* ex) const {
    ...
    if (auto [name_prefix, fn] = module->findFunction(hash, ex->text); fn) {
        ...
        std::ranges::copy(
            fn->sig.parameters |
                std::views::transform([](auto& e) { return e.type; }),
    ...
    if (auto [name_prefix, fn] = module->findGenericFn(hash, ex->text); fn) {
        ...
        for (auto i : std::views::iota(0u, num_subtypes)) {
            generic_instantiations[clause->types[i]] =
                subtypes.emplace_back(state->new_type_var());
        }
        // separate loop to apply generic constraints, because the
        // interface might reference other type parameters e.g ::<O, T:
        // Iterator::<O>>
        for (auto i : std::views::iota(0u, num_subtypes)) {  // for each type
            for (auto& con :
                 clause->constraints[clause->types[i]]) {  // for each
                                                           // constraint on type
                auto& as_impl = std::get<ImplConstraint>(con);
                auto tp = as_impl.other;
                normalize_type(tp, state, irgen, generic_instantiations);
                state->add_constraint(ImplInterfaceConstraint{
                    generic_instantiations[clause->types[i]], tp, ex});
            }
        }
        tp.block_hash +=
            fn->name + IRGenerator::mangleGenericArgs(subtypes) + "::";
        std::ranges::move(
            fn->signature.parameters |
                std::views::transform([&generic_instantiations, this](auto& e) {
                    Type tp = e.type;
                    normalize_type(tp, state, irgen, generic_instantiations);
                    return tp;
                }),
        ...
        normalize_type(tp.subtypes.back(), state, irgen,
                       generic_instantiations);
        ex->evaluated_type = tp;
        return tp;
    }
...
FunctionType TypeChecker::operator()(PrefixOperation* op) const {
    ...
    case Minus: {
        ...
        state->add_constraint(HasUnaryMinusConstraint{
            std::visit(targetless(), op->operand->toVariant()),
            op->evaluated_type, op});
        break;
    }
    case Bang: {
        // !<expr>
        op->evaluated_type = state->new_type_var();
        state->add_constraint(HasUnaryNotConstraint{
            std::visit(targetless(), op->operand->toVariant()),
            op->evaluated_type, op});
        break;
    }
    case Star: {
        op->evaluated_type = state->new_type_var();
        state->add_constraint(IsReferenceToConstraint{
            std::visit(targetless(), op->operand->toVariant()),
            op->evaluated_type, op});
        break;
    }
    case Ampersand: {
        ...
        op->evaluated_type = state->new_type_var();
        state->add_constraint(
            BorrowResultConstraint{std::move(inner), op->evaluated_type, op});
        break;
    }
    case RefMut: {
        auto inner = std::visit(targetless(), op->operand->toVariant());
        op->evaluated_type = state->new_type_var();
        state->add_constraint(BorrowResultMutConstraint{
            std::move(inner), op->evaluated_type, op});
        break;
    }
    default:
        break;  // unreachable
...
FunctionType TypeChecker::operator()(BinaryOperation* expr) const {
    ...
    auto do_overloadable_explicit_token = [expr, this](TokenType tk) {
        ...
        state->add_constraint(BinaryOperableConstraint{
            left, right, expr->evaluated_type, tk, {}, expr});
        ...
        if (target) {
            state->add_constraint(
                EqualConstraint{expr->evaluated_type, *target});
            // op->type = *target; // the node is going to be converted
        }
    ...
    case Spaceship:
        [[fallthrough]];
    case Greater:
        return do_overloadable_explicit_token(
            TokenType::Spaceship);  // comparisons are all overloaded with
                                    // <=>
    case Dot: {
        expr->evaluated_type = state->new_type_var();
        state->add_constraint(BinaryDotCompatibleConstraint{
            std::visit(targetless(), expr->lhs->toVariant()), expr->rhs.get(),
            expr->evaluated_type, expr});
        return expr->evaluated_type;
    }
    case Equal: {
        state->add_constraint(EqualConstraint{
            std::visit(targetless(true), expr->lhs->toVariant()),
            std::visit(targetless(), expr->rhs->toVariant()), expr});
...
FunctionType TypeChecker::operator()(LogicalOperation* expr) const {
    if (target && !target->is_boolean()) {
        irgen->error(
            Error(expr, "Cannot convert bool to " + target->full_name()));
        return Type{"__error_type"};
    }
    auto left = std::visit(targetless(), expr->lhs->toVariant());
    auto right = std::visit(targetless(), expr->rhs->toVariant());
    // nothing can implicitly convert to bool and logical operations only
    // work for and return bools
    state->add_constraint(EqualConstraint{
        left, Type{.name = "bool", .module = core_module},
         expr->lhs.get()
    });
    state->add_constraint(EqualConstraint{
        right, Type{.name = "bool", .module = core_module},
         expr->rhs.get()
    });
    expr->evaluated_type = Type{.name = "bool", .module = core_module};
    return expr->evaluated_type;
...
FunctionType TypeChecker::operator()(CallOperation* op) const {
    ...
    if (callee_ty.name.starts_with("__union_var")) {
        ...
        std::string variant = std::string(
            callee_ty.name.begin() + dollar_off + 1, callee_ty.name.end());
        std::string type_name =
            std::string(callee_ty.name.begin() + "__union_var"sv.size(),
                        callee_ty.name.begin() + dollar_off);
        ...
        state->add_constraint(
            EqualConstraint{decl->fields.at(variant), arg_ty, op});
    ...
    for (auto i : std::views::iota(0u, op->arguments.size())) {
        state->add_constraint(ValidAsFunctionArgConstraint{
            std::visit(targetless(), op->arguments[i]->toVariant()), callee_ty,
            i + is_bound, op->arguments[i].get()});
    }
    op->evaluated_type = state->new_type_var();
    state->add_constraint(
        IsReturnOfConstraint{op->evaluated_type, callee_ty, op});
    if (target) {
...
FunctionType TypeChecker::operator()(SubscriptOperation* subs) const {
    ...
    state->add_constraint(IndexOperableConstraint{
        obj, idx, subs->evaluated_type, in_mutable_ctx, subs});
    return subs->evaluated_type;
}
FunctionType TypeChecker::operator()(LambdaExpression*) const { return Type{}; }

bool advanceScopeModified(Type& type, ModuleBase*& md, std::string& hash,
                          IRGenerator* irgen) {
    ...
    if (auto [hsh, decl] = md->findGenericClass(hash, type.name); decl) {
        ...
        auto mangled_name =
            decl->name + IRGenerator::mangleGenericArgs(type.subtypes);
    ...
    if (auto [hsh, interface] = md->findInterface(hash, type.name); interface) {
        hash =
            hsh + "%%" + type.name + "%%interface";  // interfaces are terminal
                                                     // and cannot have subtyes
        return true;
    }
    if (auto alias = md->findAlias(hash, type.name); alias) {
...
FunctionType TypeChecker::operator()(ScopeOperation* scp) const {
    ...
    // till we occur an incomplete generic (subtypes not monomorphized into
    // module table) we switch to looking directly at the generic's syntax
    // tree
    ModuleBase* md = irgen->module;
    std::string hash = irgen->block_hash;
    std::string second_to_last = "";
    ...
    while (!iterator.is_end()) {
        type = iterator.next();
        if (auto err =
                advanceScopeNormalize(current_stat, md, hash, type, state,
                                      irgen, generic_instantiations)) {
            debugbreak();
        }
        second_to_last.swap(type.name);
    ...
    if (current_stat) {
        // at this point we are at the last element of the iter (
        // foo::bar::...::[end] )
        auto last = iterator.last();
        // scope expressions must refer to values not types
        // so the last element must be a function, union variant, constant,
        // enum variant and the function can be from an interface too so
        // thats a slightly special case
        Statement* next_stat = get_next_stat(current_stat, last.name);
        // there is no next valid statement so its either an enum variant,
        // interface function or union variant
        if (!next_stat) {
            if (auto intf = dynamic_cast<InterfaceDeclaration*>(current_stat)) {
                auto it = std::ranges::find_if(
                    intf->methods,
                    [&last](auto& mth) { return mth->name == last.name; });
                if (it == intf->methods.end()) {
                    irgen->error(
                        Error(scp, "No method name '" + last.name +
                                       "' in the specified interface"));
                    return Type{"__error_type"};
                }
                // return { Type{"__interface_fn" + actual_hash +
                // interface->name + "$" + last.name } };
            }
            // enums
            if (auto enm = dynamic_cast<EnumDeclaration*>(current_stat)) {
                ...
                if (!enm->values.contains(last.name)) {
                    irgen->error(
                        Error(scp, "Enum doesn't contain specified value"));
                }
                ...
                scp->evaluated_type = Type{
                    .name = second_to_last, .module = md, .block_hash = hash};
            }
            if (auto unn = dynamic_cast<UnionDeclaration*>(current_stat)) {
                if (!unn->fields.contains(last.name)) {
                    irgen->error(
                        Error(scp, "Union doesn't contain specified value"));
                }
                if (!last.subtypes.empty()) {
                    irgen->error(
                        Error(scp, "Union child cannot have subtypes"));
                    return Type{"__error_type"};
                }
        ...
        if (auto fn = dynamic_cast<FunctionDeclaration*>(next_stat)) {
            ...
            // function may be generic so we add its types to our generic
            // instantiations
            if (dynamic_cast<GenericFunctionDeclaration*>(fn)) {
                ...
                for (auto i : std::views::iota(last.subtypes.size(),
                                               clause->types.size())) {
                    generic_instantiations[clause->types[i]] =
                        state->new_type_var();
                }
            ...
            for (auto& param : fn->signature.parameters) {
                ...
                normalize_type(scp->evaluated_type.subtypes.back(), state,
                               irgen, generic_instantiations);
            }
            scp->evaluated_type.subtypes.push_back(fn->signature.returnType);
            normalize_type(scp->evaluated_type.subtypes.back(), state, irgen,
                           generic_instantiations);
        }
        if (auto constant = dynamic_cast<ConstantDeclaration*>(next_stat)) {
            scp->evaluated_type = constant->type;
            normalize_type(scp->evaluated_type, state, irgen,
                           generic_instantiations);
        }
    }
    // no generics were encountered at all and we've reached the last entry
        ...
        if (auto [actual_hash, enm] = md->findEnum(hash, second_to_last); enm) {
            if (md != irgen->module && enm->is_private()) {
                irgen->error(Error(scp, "The enum type " + actual_hash +
                                            enm->identifier + " is private"));
                return Type{"__error_type"};
            }
            if (!last.subtypes.empty()) {
                irgen->error(Error(scp, "Enum child cannot have subtypes"));
            ...
            if (enm->values.contains(last.name)) {
                scp->evaluated_type = Type{.name = second_to_last,
                                           .module = md,
                                           .block_hash = actual_hash};
            } else {
                irgen->error(
                    Error(scp, "Enum does not contain specified value"));
                return Type{"__error_type"};
            }
        } else if (auto [actual_hash, unn] =
                       md->findUnion(hash, second_to_last);
                   unn) {
            ...
            if (unn->fields.contains(last.name)) {
                scp->evaluated_type =
                    Type{.name = "__union_var" + unn->name + "$" + last.name,
                         .module = md,
                         .block_hash = actual_hash};
            } else {
                irgen->error(
                    Error(scp, "Union does not contain specified variant"));
                return Type{"__error_type"};
            }
        } else if (auto [name, fn] = md->findFunction(hash, last.name); fn) {
            if (md != irgen->module && fn->is_private()) {
                irgen->error(Error(scp, "The function type " + name + fn->name +
                                            " is private"));
                return Type{"__error_type"};
            }
            ...
            std::ranges::copy(
                fn->sig.parameters |
                    std::views::transform([](auto& elem) { return elem.type; }),
...
FunctionType TypeChecker::operator()(ObjectLiteral* lit) const {
    ...
    for (auto& [field, elem] : lit->values) {
        ...
        state->add_constraint(
            HasFieldConstraint{lit->evaluated_type, field, result, lit});
        Type other;
        if (elem) {
...
FunctionType TypeChecker::operator()(NullLiteral* lit) const {
    if (target) {
        if (!target->is_optional()) {
            irgen->error(
                Error(lit, "Cannot convert from 'null' to non-optional type"));
            return Type{"__error_type"};
        }
    ...
    lit->evaluated_type =
        Type{.name = "__opt", .subtypes = {inner}, .module = core_module};
    return lit->evaluated_type;
}
FunctionType TypeChecker::operator()(AsExpression* ex) const {
    // AsExpressions are a little bit more complicated than a cast, because
    // of unions: if the source is a union and the destination is on of the
    // variants, we return the __conv_result_{val, ref} types else the final
    // type is always the same as the destination
    auto src = std::visit(targetless(), ex->expr->toVariant());
    ex->evaluated_type = state->new_type_var();
    state->add_constraint(AsConstraint{src, ex->dest, ex->evaluated_type, ex});
    return ex->evaluated_type;
}
FunctionType TypeChecker::operator()(CharLiteral* ch) const {
    if (target) {
        if (!target->is_char()) {
            irgen->error(
                Error(ch, "Cannot convert char to " + target->full_name()));
        }
    }
...
FunctionType TypeChecker::operator()(GCNewExpression* gcn) const {
    auto sub_type = std::visit(target ? new_target(target->subtypes[0]) : *this,
                               gcn->target_expression->toVariant());
    state->add_constraint(
        OwningConstraint{sub_type});  // cannot store non owning value in gc
    gcn->evaluated_type =
        Type{.name = "__gcref", .subtypes = {sub_type}, .module = core_module};
...
TypeChecker TypeChecker::new_target(std::optional<Type> tgt,
                                    bool in_mutable_ctx) const {
...
bool has_type_variable(const Type& tp) {
    ...
    if (auto pos = tp.block_hash.find('?'); pos != std::string::npos)
        ...
        return tp.block_hash.size() > pos + 1 &&
               std::isdigit(static_cast<unsigned char>(tp.block_hash[pos + 1]));
    for (auto& sub : tp.subtypes) {
...
bool ConstraintSolver::operator()(IsIntegerConstraint& con) {
    ...
    if (is_type_variable(type)) {
        ...
        state->push_step(Info::TypeCheckerStateDiff{
            ...
            .arg = Info::SubstitutionInformation{type.name,
                                                 domain->get_type_list(),
                                                 state->current_constraint}
        });
    } else {
        if (!type.is_integral())
            irgen->error(Error(con.expr, "This type must be an integer"));
...
bool ConstraintSolver::operator()(CanStoreIntegerConstraint& con) {
    ...
    if (is_type_variable(type)) {
        if (auto as_u64 = std::get_if<uint64_t>(&con.value)) {
            ...
            if (auto err = state->get_type_domain(type)->constrain_to_store(
                    std::get<int64_t>(con.value))) {
                irgen->error(err.value());
            }
        }
        return true;
...
bool ConstraintSolver::operator()(IsFloatConstraint& con) {
    ...
    if (is_type_variable(type)) {
        ...
        if (!type.is_floating_point())
            irgen->error(
                Error(con.expr, "This type must be an floating point"));
        // this constraint is done
    }
    return true;
}
...
/// to see if they're compatible and generates equality constraints for
/// nested type variables the equality constraint is always ordered as
/// "type1 elem = type2 elem"
std::optional<Error> generic_match(Type tp1, Type tp2, ASTNode* loc,
                                   TypeCheckerState* solver) {
    // include the block in the name
    // because type.subtypes only occurs generics at the end
    // ─────────────────╮ so we can check cases where the generic occurs
    // earlier ──╮             │
    //                        ╭─────────────────────────────────╯ like here
    //                        │
    //                       vvv │
    ...
    if (tp1_iter.num_iters() != tp2_iter.num_iters()) {
        return Error(loc, std::format("The types {} and {} are incompatible",
                                      tp1.name, tp2.name));
    }
    while (!tp1_iter.is_end()) {
        auto this1 = tp1_iter.next();
        auto this2 = tp2_iter.next();
        // we now have an atomic framgent of the type path that isn't the
        // end using the the earlier example we have    ╭ one of these
        // individually with correct subtype representation
        //   ╭────────────┬─────────────┬───────────╯
        // vvvvvv  vvvvvvvvvvvvvv  vvvvvvvvvv
        // Module::Something::<T>::OtherStuff::Type::<U>
        if (this1.name != this2.name) {
            return Error(loc,
                         std::format("The types {} and {} are incompatible",
                                     tp1.name, tp2.name));
        }
        ...
        for (auto i : std::views::iota(0u, this1.subtypes.size())) {
            ...
            if (is_type_variable(this1.subtypes[i]) ||
                is_type_variable(this2.subtypes[i])) {
                solver->add_constraint(
                    EqualConstraint{this1.subtypes[i], this2.subtypes[i], loc});
            } else {
                if (auto failed = generic_match(this1.subtypes[i],
                                                this2.subtypes[i], loc, solver))
                    return failed;
            }
        }
    }
    auto end1 = tp1_iter.last();  // using the example this is `Type` with
                                  // proper subtypes
    ...
    for (auto i : std::views::iota(0u, end1.subtypes.size())) {
        if (is_type_variable(end1.subtypes[i]) ||
            is_type_variable(end2.subtypes[i])) {
            solver->add_constraint(
                EqualConstraint{end1.subtypes[i], end2.subtypes[i], loc});
        } else {
            if (auto failed = generic_match(end1.subtypes[i], end2.subtypes[i],
                                            loc, solver))
                return failed;
        }
    }
    ...
    if (is_type_variable(type1)) {
        ...
        // ?1 and Something::<?2>::Type <case 2>
            ...
            state->push_step(Info::TypeCheckerStateDiff{
                ...
                .apply_to = Info::TypeCheckerStateDiff::Substitutions,
                .arg = Info::SubstitutionInformation{
                                                     type1.name, state->get_type_domain(type1)->get_type_list(),
                                                     state->current_constraint}
            });
        }
        // ?1 and concrete <case 3>
            ...
            if (auto err = state->get_type_domain(type1)->equal_constrain(
                    std::move(type2))) {
                err->span =
                    SourceSpan{.begin = con.expr->beg, .end = con.expr->end};
                irgen->error(*err);
            }
            state->push_step(Info::TypeCheckerStateDiff{
                .op = Info::TypeCheckerStateDiff::Replace,
        ...
        if (is_type_variable(type2)) {
            ...
            state->push_step(Info::TypeCheckerStateDiff{
                ...
                .apply_to = Info::TypeCheckerStateDiff::Substitutions,
                .arg = Info::SubstitutionInformation{
                                                     type2.name, state->get_type_domain(type2)->get_type_list(),
                                                     state->current_constraint}
            });
        }
        // Something::<?1>::Type and Something::<?1>::Type <case 4>
            ...
            // the block string can have the variables
            // ...::Something::<?1>::... the actual subtypes can have the
            // variables (much easier) the actual subtypes match either form
            // 1 or 2 (recursive) (actual hell) or both luckily we have a
            // neat routine for that
            // begin checking subtypes
            if (auto err = generic_match(std::move(type1), std::move(type2),
                                         con.expr, state))
                irgen->error(*err);
        }
        ...
        // concrete and concrete
            ...
            if (!type1.is_equal(type2)) {
                irgen->error(
                    Error(con.expr,
                          std::format("The types {} and {} have to be equal",
...
bool ConstraintSolver::operator()(OwningConstraint& con) {
    ...
    if (type.is_non_owning(irgen))
        irgen->error(
            Error(con.expr, std::format("They type {} must be owning",
                                        type.pretty_name(irgen->block_hash))));
    return true;
}
...
bool ConstraintSolver::operator()(ValidAsFunctionArgConstraint& con) {
    ...
    if (is_bound && con.arg_no == uint32_t(-1)) {
        ...
        if (callee.subtypes[0].is_gc_reference()) {
            add_new_constraint(
                EqualConstraint{callee.subtypes[0], arg, con.expr});
        }
        // &mut this works with mutable references and mutable arguments
            ...
            if (is_type_variable(arg)) {
                ...
                if (auto err =
                        domain->add_and_intersect(std::move(gp), state)) {
                    irgen->error(err.value());
                }
                state->push_step(Info::TypeCheckerStateDiff{
                    .op = Info::TypeCheckerStateDiff::Replace,
                    .apply_to = Info::TypeCheckerStateDiff::Substitutions,
                    .arg = Info::SubstitutionInformation{
                                                         arg.name, domain->get_type_list(),
                                                         state->current_constraint}
                });
            } else {
                if (arg.is_mutable_reference() || arg.is_mutable) {
                    add_new_constraint(EqualConstraint{
                        callee.subtypes[0].deref(), arg.deref(), con.expr});
                } else {
                    irgen->error(Error(con.expr,
                                       "Tried to bind to mutable reference "
                                       "with immutable parameter"));
                }
            }
        }
        // &this works for arguments of type Type, &Type, &mut Type and
        // ^Type
            ...
            if (is_type_variable(arg)) {
                ...
                gp.add_type(
                    Type(callee.subtypes[0].deref().mutable_reference_to()));
                ...
                add_new_constraint(EqualConstraint{callee.subtypes[0].deref(),
                                                   arg.deref(), con.expr});
            }
        }
        return true;
    ...
    add_new_constraint(
        EqualConstraint{callee.subtypes[con.arg_no + is_bound], arg, con.expr});
    return true;
}
...
bool ConstraintSolver::operator()(ImplInterfaceConstraint& con) {
    ...
    // An interface can be implemented in 2 places(and what it means for
    // generics):
    ...
    //     }
    // }
    // all of Generic::<T, E> implement Interface even with enable_if and
    // generic constraints (the easy one)
    //
    ...
    // - Interface: interface::<T> defines an interface with type parameters
    // T (i.e a type cannot implement Interface::<i32> and Interface::<f32>
    // because they are the same you can be generic with a bit of hackery:
    // ```
    // GenericInterface: struct::<T> = {
    //     Interface: interface = {}
    // }
    // a type can implement
    // GenericInterface::<i32>::Interface and
    // GenericInterface::<f32>::Interface
    // ```
    ...
    //      return true (we can return true because the domain can never
    //      grow)
    ...
    auto check_type_against_interface =
        // take_action means whether to go ahead and constrain the type(s)
        // or to just check if there's a possibility
        [this, &con, &ret_val](Type& intf, Type& type,
                               InterfaceDeclaration* decl, bool take_action) {
            auto [type_decl, generics] = normalize_type(type, state, irgen, {});
            auto impls = std::visit([](auto* arg) { return get_impls(arg); },
                                    type_decl->toVariant());
            std::vector<TypeCheckerConstraint> collector;
            auto collector_ptr = &collector;
            std::swap(state->write_new_constraints_to, collector_ptr);
            // this is to see if we can constrain the subject type
            // we can only constrain it if we encounter 1 valid impl (if not
            // we may introduce false constraints)
            std::unordered_map<uint32_t, Domain> subject_constraints;
            uint32_t num_matches = 0;
            // we don't intersect domains instantly, we collect all possible
            // types into a group then intersect the domains at once
            ...
            if (impls) {
                for (auto& impl : *impls) {
                    ...
                    if (auto err =
                            generic_match(impl_for, intf, con.expr, state);
                        !err) {
                        if (!take_action) {
                            std::swap(state->write_new_constraints_to,
                                      collector_ptr);
                            return true;
                        }
                        num_matches++;
                        ...
                        for (auto& con :
                             collector | transform([](auto& con) -> auto& {
                                 return std::get<EqualConstraint>(con);
                             })) {
                            // type1 is always from type and type2 is from
                            // interface and one of either must be a type
                            // variable if the interface is a type variable
                            // we constrain its domain like ?T2 in the above
                            // example can be {i32, f32} if the type is a
                            // type variable we constrain its domain to be
                            // exactly from the interface but we can only
                            // constrain this if this is the right
                            // implementation so we have to return false for
                            // further info
                            if (is_type_variable(con.type2) &&
                                !is_type_variable(con.type1)) {
                                itf_generic_groups[state->tbl.type_to_id(
                                                       con.type2)]
                                    .add_type(std::move(con.type1));
                            } else if (is_type_variable(con.type1) &&
                                       !is_type_variable(con.type2)) {
                                type_generic_groups[state->tbl.type_to_id(
                                                        con.type1)]
                                    .add_type(std::move(con.type2));
                            }
                            // I don't actually know this is the right thing
                            // to do here
                            else {
                                var_to_var_constraints.push_back(
                                    std::move(con));
                            }
                        }
            ...
            // here we check all type the interface is implemented for (2nd
            // syntax) we normalize each of the types and check if they can
            // match our input type if so
            for (const auto& elem :
                 decl->impl_fors | transform([this](const ImplBlock& blk) {
                     std::unordered_map<std::string, Type> substitutions = {};
                     auto impl_for = blk.impl.impl_for;
                     for (auto& type : blk.clause.types) {
                         // need a way to make type vars without requiring
                         // them to be solved
                         substitutions[type] = state->new_type_var();
                     }
                     normalize_type(impl_for, state, irgen,
                                    std::move(substitutions));
                     return impl_for;
                 }) | filter([&type](const Type& tp) {
                ...
                for (auto& con : collector | transform([](auto& con) -> auto& {
                    ...
                    if (is_type_variable(con.type2) &&
                        !is_type_variable(con.type1)) {
                        itf_generic_groups[state->tbl.type_to_id(con.type2)]
                            .add_type(std::move(con.type1));
                    } else if (is_type_variable(con.type1) &&
                               !is_type_variable(con.type2)) {
                        type_generic_groups[state->tbl.type_to_id(con.type1)]
                            .add_type(std::move(con.type2));
                    } else {
            ...
            for (auto& [id, group] : itf_generic_groups) {
                if (auto err =
                        state->get_type_domain(state->tbl.id_to_type(id))
                            ->add_and_intersect(std::move(group), state)) {
                    irgen->error(err.value());
                }
                ...
                state->push_step(Info::TypeCheckerStateDiff{
                    ...
                    .apply_to = Info::TypeCheckerStateDiff::Substitutions,
                    .arg = Info::SubstitutionInformation{
                                                         type.name, domain->get_type_list(),
                                                         state->current_constraint}
                });
            }
            ...
            if (num_matches == 1) {
                ...
                for (auto& [id, domain] : subject_constraints) {
                    ...
                    state->push_step(Info::TypeCheckerStateDiff{
                        ...
                        .arg = Info::SubstitutionInformation{
                                                             type.name,
                                                             state->get_type_domain(type)->get_type_list(),
                                                             state->current_constraint}
                    });
                }
                ...
                irgen->error(
                    Error(con.expr, "Type doesn not implement interface"));
            }
            // if we have more than one match we probably need more info (or
            // are wrong idk)
            else {
                ret_val = false;
            }
    ...
    if (intf_decl == nullptr) {
        return true;  // there was an error, but it was reported by
                      // normalize type (hopefully)
    }
    auto as_intf = dynamic_cast<InterfaceDeclaration*>(intf_decl);
    if (!as_intf) {
        irgen->error(Error(con.expr, "The type " + con.interface.full_name() +
                                         " is not an interface"));
        return true;
    }
    ...
    if (is_type_variable(type)) {
        ...
        std::erase_if(
            domain->concrete_types.types,
            [&intf, as_intf, &check_type_against_interface](Type& tp) {
...
bool ConstraintSolver::operator()(IsReferenceToConstraint& con) {
    ...
    if (!type.is_reference())
        irgen->error(Error(con.expr, "Cannot derefernce non-reference"));
    else
        add_new_constraint(EqualConstraint{type.subtypes[0], other, con.expr});
    return true;
}
bool ConstraintSolver::operator()(IsNotReferenceConstraint& con) {
    ...
    if (type.is_reference()) {
        irgen->error(
            Error(con.expr, std::format("The type {} must not a reference",
                                        type.pretty_name(irgen->block_hash))));
    }
    return true;
}
std::pair<std::string, OverloadDetailsBinary*> resolveBin(const Type& lhs,
                                                          const Type& rhs,
                                                          TokenType t,
                                                          IRGenerator* irgen);
bool ConstraintSolver::operator()(BinaryOperableConstraint& con) {
    ...
    auto is_compatible = [&result, this, &con](const Type& left,
                                               const Type& right, TokenType tt,
                                               bool take_action) {
        using namespace std::views;
        std::vector<ModuleBase*> valid_modules;
        if (left.module && right.module) {
            valid_modules.reserve(4);
            std::array<ModuleBase*, 4> modules{left.module, left.deref().module,
                                               right.module,
                                               right.deref().module};
            ...
            auto all_modules =
                irgen->module->engine->modules |
                transform([](auto& in) { return in.second.get(); });
            valid_modules.insert(valid_modules.end(), all_modules.begin(),
                                 all_modules.end());
        }
        ...
        for (auto module : valid_modules) {
            for (auto& [block, ovl] :
                 module->overloads.binary_details_for(tt)) {
                std::unordered_map<std::string, Type> generic_subs;
                if (ovl.statement && !ovl.statement->clause.types.empty()) {
                    if (!con.substitution_cache.contains(ovl.statement))
                        for (auto& tp : ovl.statement->clause.types)
                            con.substitution_cache[ovl.statement][tp] =
                                state->new_type_var();
                ...
                normalize_type(expected_right, state, irgen,
                               std::move(generic_subs));
                std::swap(irgen->module, module);
                irgen->block_hash.swap(block);
                // much like for interfaces, if both sides can match we take
                // it into consideration and if there's only one in
                // consideration thats our answer, if there's zero its an
                // error and if its more than one we have to wait a bit
                auto left_old_size = l_collector.size();
                intermediate.clear();
                if (generic_match(left, expected_left, nullptr, state) ==
                    std::nullopt) {
                    std::ranges::move(std::move(intermediate),
                                      std::back_inserter(l_collector));
                } else
                    continue;
                intermediate.clear();
                if (generic_match(right, expected_right, nullptr, state) ==
                    std::nullopt) {
                    // they both successfuly match
                    std::ranges::move(std::move(intermediate),
                                      std::back_inserter(r_collector));
                    matches.emplace_back(module, &ovl);
                    // we found a match if we're not taking action we can
                    // abort we can return here
                    if (!take_action) {
                        std::swap(state->write_new_constraints_to,
                                  intermediate_ptr);
                        return true;
                ...
                // on failure revert left to its original state
                    ...
                    l_collector.erase(l_collector.begin() + left_old_size,
                                      l_collector.end());
                }
            };
        }
        ...
        if (matches.size() == 1) {
            if (take_action) {
                for (auto& elem : l_collector)
                    add_new_constraint(std::move(elem));
                for (auto& elem : r_collector)
                    add_new_constraint(std::move(elem));
                auto this_result = matches[0].second->result;
                normalize_type(
                    this_result, state, irgen,
                    con.substitution_cache[matches[0].second->statement]);
                std::visit(
                    [&matches, &con]<typename T>(T* expr) {
                        if constexpr (std::same_as<T, BinaryOperation> ||
                                      std::same_as<T, SubscriptOperation>) {
                            expr->selected = matches[0].second;
                            expr->module = matches[0].first;
                            if (matches[0].second->statement) {
                                for (auto& generic :
                                     matches[0]
                                         .second->statement->clause.types) {
                                    auto& subs =
                                        con.substitution_cache
                                            [matches[0].second->statement];
                ...
                add_new_constraint(
                    EqualConstraint{result, this_result, con.expr});
            }
            return true;
        } else if (matches.size() == 0 && take_action) {
            irgen->error(
                Error(con.expr, "Overload does not exist between types"));
        }
        return false;
    };
    ...
    if (is_type_variable(left)) {
        ...
        // we should probably generate all possible types to populate the
        // domain
        if (domain->concrete_types.types.empty()) return false;
        std::erase_if(domain->concrete_types.types,
                      [&con, &is_compatible, &right](const Type& tp) {
        ...
        if (domain->concrete_types.types.empty()) return false;
        std::erase_if(domain->concrete_types.types,
                      [&con, &is_compatible, &left](const Type& tp) {
...
bool ConstraintSolver::operator()(ComparableConstraint& con) {
    ...
    if (!((type.name == "CmpOrd" || type.name == "CmpPartOrd") &&
          type.module == core_module)) {
        irgen->error(
            Error(con.expr, std::format("{} does not support <, >, >= or <=",
                                        type.pretty_name(irgen->block_hash))));
    }
    return true;
}
uint32_t UnificationTable::unite(uint32_t id, uint32_t id2, IRGenerator* irgen,
    ...
    auto finda = find(id);
    auto findb = find(id2);
    if (finda == findb) return finda;
    if (rank[finda] < rank[findb]) std::swap(finda, findb);
    parents[findb] = finda;
    if (rank[finda] == rank[findb]) rank[finda]++;
...
void TypeCheckerState::resolve_function(FunctionDeclaration* decl,
                                        IRGenerator* irgen,
    ...
    info.initial_state =
        Info::RecordedTypeCheckerState{.active_constraints = {},
                                       .generated_constraints = {},
                                       .subsitutions = {}};
    std::ranges::transform(
        constraints, std::back_inserter(info.initial_state.active_constraints),
        [](const TypeCheckerConstraint& con) {
    ...
    while (has_change) {
        ...
        if (!sv.temp_constraints.empty())
            push_step(Info::TypeCheckerStateDiff{
                .op = Info::TypeCheckerStateDiff::Flush});
...
bool TypeCheckerState::is_non_owning(const Type& tp, IRGenerator* irgen) {
    if ((tp.is_reference() && !tp.is_gc_reference()) || tp.is_slice())
        return true;
    ...
    if (auto decl = get_type_stat(tp, tp.block_hash, tp.module))
        ...
        if (auto as_cls = dynamic_cast<ClassDeclaration*>(decl))
            return as_cls->ownership == Ownership::NonOwning ||
                   as_cls->ownership == Ownership::NonOwningMut;
    return false;
}
bool ConstraintSolver::operator()(ExtractsToConstraint& con) {
    // only 3 kinds of types satisfy this
    // T? (__opt::<T>) extracts to T
    // __conv_result_ref::<T> extracts to T (special internal type returned
    // by variant conversion)
    // __conv_result_val::<T> extracts to T              ^^^^^^----same case
    // as above
    ...
    if (type.name != "__opt" && type.name != "__conv_result_ref" &&
        type.name != "__conv_result_val" && type.name != "__res") {
        irgen->error(
            Error(con.expr, "Invalid type for conditional extraction"));
        return true;
    }
    add_new_constraint(EqualConstraint{type.subtypes[0], target, con.expr});
    return true;
}
...
bool ConstraintSolver::operator()(NonOwningConstraint& con) {
    ...
    if (!type.is_non_owning(irgen))
        irgen->error(
            Error(con.expr, std::format("They type {} must be non owning",
                                        type.pretty_name(irgen->block_hash))));
    return true;
}
bool ConstraintSolver::operator()(AsConstraint& con) {
    auto src = state->best_repr(con.input_type);
    // TODO: we can probably partially initialized unions here
    // (`has_type_variable` && !`is_type_variable`)
    if (has_type_variable(src)) return false;

    if (auto decl = src.get_decl_if_union()) {
        // TODO: consider user defined conversions
        if (!con.dest.subtypes.empty())
            irgen->error(Error(con.expr,
                               "'as' expressions for unions must use only "
                               "the variant name"));
        if (!con.dest.block_hash.empty())
            irgen->error(Error(con.expr,
                               "'as' expressions for unions must use only "
                               "the variant name"));
        if (!decl->fields.contains(con.dest.name))
            irgen->error(
                Error(con.expr, "Specified field does not exist in the union"));
        auto dest_tye = Type{"__conv_result_ref",
                             {decl->fields.at(con.dest.name)},
                             nullptr,
                             irgen->module->engine->modules.at("core").get(),
                             src.is_mutable,
                             src.is_lvalue};
...
bool ConstraintSolver::operator()(ConvertibleToConstraint& con) {
    ...
    if (is_type_variable(dst)) {
        if (!has_type_variable(src)) {
            // src can convert to src? and depending on the type other
            // things
            Domain::Group gp;
            gp.add_type(
...
/// 1 - if <expr1> is (functionally equivalent to) a tuple and <expr2> is an
/// integer constant expression then the expression is a tuple access
///
/// 2 - if <expr1> is a struct type and <expr2> is one of the field names of
/// that class, then the expression is a struct field access
///
/// 3 - if <expr2> is one of the inherent methods of <expr1> the expression
/// is binding <expr1> to the first arg of the function call
///
/// 4 - if <expr2> is one of the inherent methods of operator&(<expr>) the
/// expression is binding operator&(<expr1>) to the first arg of the
/// function call
///
/// 5 - if <expr2> is a method of an interface that is implemented by
/// <expr1>, the expression is binding <expr1> to the interface function
/// call
///
/// 6 - if <expr2> evaluates to a function, the expression binds <expr1> as
/// the first arg of that function call
///
...
bool ConstraintSolver::operator()(BinaryDotCompatibleConstraint& con) {
    ...
    if (left.deref().is_tuple()) {
        ...
        if (auto as_int = dynamic_cast<IntegerLiteral*>(con.right)) {
            auto return_type =
                left.deref()  // remove reference to get the underlying
                              // tuple
                    .subtypes[std::stol(
                        std::string{as_int->text})]  // get the right subtype
    ...
    // TODO probably remove this functionality from normalize_type and
    ...
    if (as_name) {
        ...
        if (auto cls = dynamic_cast<ClassDeclaration*>(stat)) {
            auto it =
                std::ranges::find_if(cls->vars, [as_name](ClassVariable& var) {
                    return var.name == as_name->text;
                });
            if (it != cls->vars.end()) {
                // we copy out because we may be in a generic declaration
                // and we don't modify the AST to not affect future
                // instantiations
                auto type = it->type;
        ...
        auto check_inherent_submethod =
            [this, &result](
                ...
                // in checking subfunctions, if the type is not generic we can
                // search the module directly in the block of the type
                if (gctx.empty()) {
                    std::string function_block =
                        no_reference.full_name() + "::";
                    // we could use "findFunction" but we know the exact block
                    if (no_reference.module->functions.contains(
                            function_block)) {
                        auto& funs =
                            no_reference.module->functions.at(function_block);
                        if (auto it = std::ranges::find_if(
                            ...
                            it != funs.end()) {
                            if (!it->sig.parameters.empty() &&
                                it->sig.parameters[0].name == "this") {
                                right.name = "__fn";
                                right.block_hash =
                                    function_block + std::string(name) + "::";
                                ...
                                std::ranges::copy(
                                    it->sig.parameters |
                                        ...
                                        std::views::transform(
                                            [this, gctx](Type param) {
                                                normalize_type(param, state,
                                                               irgen, {});
                                                return param;
                                            }),
                                    std::back_inserter(right.subtypes));
                                right.subtypes.push_back(it->sig.returnType);
                                normalize_type(right.subtypes.back(), state,
                                               irgen, gctx);
                                irgen->block_hash.swap(function_block);

                                auto& first_type = right.subtypes[0];
                                // first_type name is "This" we edit it to
                                // left.deref()
                                if (!first_type.is_reference())
                                    first_type = no_reference;
                                else
                                    first_type.subtypes[0] = no_reference;

                                auto result_type = Type{
                                    .name =
                                        "__bound_fn",  // the first parameter
                                    .subtypes = right.subtypes,
                                    .module = type.module,
                                    .block_hash = right.block_hash};
                                add_new_constraint(
                                    EqualConstraint{result, result_type, expr});
                                add_new_constraint(ValidAsFunctionArgConstraint{
                                    type, std::move(result_type), uint32_t(-1),
                                    expr});
                                return true;
                            }
                        }
                ...
                // For generic types we search the AST directly because they
                // don't exist yet
                else if (auto fn = dynamic_cast<FunctionDeclaration*>(
                             get_next_stat(stat, name))) {
                    if (!fn->signature.parameters.empty() &&
                        ...
                        std::ranges::copy(
                            fn->signature.parameters |
                                ...
                                std::views::transform([this, gctx](Type param) {
                                    normalize_type(param, state, irgen, gctx);
                                    return param;
                                }),
                            std::back_inserter(right.subtypes));
                        auto& first_type = right.subtypes[0];
                        ...
                        normalize_type(right.subtypes.back(), state, irgen,
                                       gctx);
                        auto result_type =
                        ...
                        add_new_constraint(
                            EqualConstraint{result, result_type, expr});
                        add_new_constraint(ValidAsFunctionArgConstraint{
                            type, std::move(result_type), uint32_t(-1), expr});
                        return true;
                    }
                }
                return false;
        ...
        // [case 3]
        // if the right is a name it could also be a function binding i.e
        // `vec.push` we check the type for subfunctions whose first
        // parameter is `this`, `&this`, or `&mut this`

        // the type itslef has a inherent method named "<expr2>"
        if (check_inherent_submethod(gctx, stat, left, as_name->text,
                                     con.right->evaluated_type, con.expr))
            return true;
        // else we check the types "deref" [case 4]
        // worth noting that they type may not be fully resolved so the
        // deref may have multiple results in that case we exit and also
        // operator &mut and operator & must return the same thing

        // if weve already checked before and had a result don't check again
        if (!con.deref_result_opt) {
            auto op_result = unary_operator_result(
                TokenType::RefMut, mutable_reference_to(no_reference, irgen),
                con.substitution_cache, state, irgen);
            if (auto invalid = std::get_if<NoOperatorReason>(&op_result)) {
                ...
                // if there's no &mut we check for &
                    ...
                    auto op_result_immutable = unary_operator_result(
                        TokenType::Ampersand, reference_to(no_reference, irgen),
                        con.substitution_cache, state, irgen);
                    if (auto invalid_immutable = std::get_if<NoOperatorReason>(
                            &op_result_immutable)) {
                        if (*invalid == NoOperatorReason::MulipleMatches)
                            return false;
                        else if (*invalid_immutable ==
            ...
            con.deref_result_opt =
                state->best_repr(con.deref_result_opt.value());
        }
        ...
        if (con.deref_result_opt) {
            ...
            // come back later this should be relatively cheap because the
            // result is cached
    ...
    // [case 6]
    auto right_t = std::visit(TypeChecker{std::nullopt, irgen, state},
                              con.right->toVariant());
    auto right = state->best_repr(right_t);
    if (right.name != "__fn") {
        irgen->error(Error(con.expr, "Cannot bind to non function"));
    }
    auto result_type = Type{.name = "__bound_fn",  // the first parameter
                            .subtypes = right.subtypes,
                            .module = left.module,
                            .block_hash = right.block_hash};
    add_new_constraint(EqualConstraint{result, result_type, con.expr});
    add_new_constraint(ValidAsFunctionArgConstraint{
        left, std::move(result_type), uint32_t(-1), con.expr});
    return true;
}
bool ConstraintSolver::operator()(ElseRefExtractsToConstraint& con) {
    ...
    if (is_type_variable(type)) {
        ...
        grp.add_type(Type{
            .name = "__res", .subtypes = {state->new_type_var(), error_type}
        });
        auto err = state->get_type_domain(type)->add_and_intersect(
            std::move(grp), state);
        state->push_step(Info::TypeCheckerStateDiff{
            .op = Info::TypeCheckerStateDiff::Replace,
            .apply_to = Info::TypeCheckerStateDiff::Substitutions,
        ...
        add_new_constraint(
            EqualConstraint{type.is_mutable ? mutable_reference_to(error_type)
                                            : reference_to(error_type),
                            target, con.expr});
        return true;
    }
    ...
    if (type.name != "__res") {
        irgen->error(Error(con.expr,
                           "Conditional extraction can also have else "
                           "branch capture with results"));
        return true;
    }
    add_new_constraint(EqualConstraint{
        type.is_mutable ? mutable_reference_to(type.subtypes[1])
                        : mutable_reference_to(type.subtypes[1]),
        target, con.expr});
    return true;
}
bool ConstraintSolver::operator()(ElseExtractsToConstraint& con) {
    ...
    if (is_type_variable(type)) {
        ...
        if (err) irgen->error(err.value());
        add_new_constraint(EqualConstraint{error_type, target, con.expr});
        return true;
    }
    ...
    add_new_constraint(EqualConstraint{type.subtypes[1], target, con.expr});
    return true;
}
bool ConstraintSolver::operator()(IfStatementConstraint& con) {
    ...
    if (con.then_transfers_control && con.else_transfers_control) {
        ...
        add_new_constraint(EqualConstraint{
            result, Type{.name = "void", .module = core_module},
             con.expr
        });
...
bool ConstraintSolver::operator()(EqualOrIsVoidConstraint& con) {
    ...
    if (is_type_variable(tp1)) {
        ...
        // if tp1 cannot be void we can go ahead and add the equal
        // constraint
...
bool ConstraintSolver::operator()(IndexOperableConstraint& con) {
    ...
    auto right = state->best_repr(con.right);
    auto token_type =
        con.is_mutable ? TokenType::SquarePairMut : TokenType::SquarePair;
    ...
    auto can_be = [this, &can_be_is_concrete](const Type& type, auto pred) {
        if (is_type_variable(type)) {
            ...
            return domain->concrete_types.types.empty() ||
                   std::ranges::any_of(domain->concrete_types.types, pred);
        }
        return pred(type);
    };
    if (can_be(left,
               [&can_be](const Type& tp) {
                   return tp.is_static_array() ||
                          (tp.is_reference() &&
                           can_be(tp.subtypes[0], [](const Type& tp2) {
                               return tp2.is_static_array();
                           }));
               }) &&
        can_be(right, [](const Type& tp) {
            return tp.is_unsigned_integral() && tp.integer_width() == 64;
        })) {
        if (can_be_is_concrete) {
            // the expression evaluator should already have a special case
            // for this
            add_new_constraint(EqualConstraint{left.deref().subtypes[0],
                                               con.result, con.expr});
            return true;
        } else
            return false;
    } else {
        // for index operations left is valid as T or &T so we need to stall
        // until we can tell its not a reference
        if (is_type_variable(left)) return false;
        Type new_left = Type{.subtypes = {left.deref()}, .module = core_module};
        if (token_type == TokenType::SquarePair)
            new_left.name = "__ref";
        else {
            if (!(left.is_mutable_reference() || left.is_mutable)) {
                irgen->error(Error(
                    con.expr,
                    std::format("{} is immutable but used in mutable context",
                                left.pretty_name(irgen->block_hash))));
            }
        ...
        add_new_constraint(BinaryOperableConstraint{
            new_left, right, new_result, token_type, {}, con.expr});
        return true;
    }
}
...
bool ConstraintSolver::operator()(AllFieldsConstraint& con) {
    ...
    if (auto cls = dynamic_cast<ClassDeclaration*>(stat)) {
        ...
        auto second_range =
            cls->vars | std::views::transform(
                            [](const ClassVariable& var) { return var.name; });
        auto first_set =
            std::set<std::string>{first_range.begin(), first_range.end()};
        auto second_set =
            std::set<std::string>{second_range.begin(), second_range.end()};
        if (first_set.size() != first_range.size())
            irgen->error(Error(con.expr, "Duplicate entries"));
        if (first_set != second_set)
            irgen->error(Error(con.expr, "All fields must be initialized"));
        return true;
    }
...
bool ConstraintSolver::operator()(BorrowResultConstraint& con) {
    ...
    // [T; N] -> &[T; N], &[T]
    // [T; *] -> &[T; *], &[T]
    // T -> &T, &view (Interfaces implemented by T), & (Borrow operator
    // result(if defined))
    auto subject = state->best_repr(con.subject);
    auto result = state->best_repr(con.result);
    ...
    if (is_type_variable(subject) && is_type_variable(result)) {
        ...
        possible_types.add_type(Type{
            .name = "__ref", .subtypes = {subject}, .module = core_module});
        //--------------operator&-------------------
        if (subject.is_str())
            possible_types.add_type(
                Type{.name = "__ref",
                     .subtypes = {Type{.name = "__string_view",
                                       .module = core_module}},
                     .module = core_module});
        else if (subject.is_static_array() || subject.is_dynamic_array())
            possible_types.add_type(
                Type{.name = "__ref",
                     .subtypes = {Type{.name = "__slice",
                                       .subtypes = {subject.subtypes[0]},
                                       .module = core_module}},
                     .module = core_module});
        else {
            auto op_result = unary_operator_result(
                TokenType::Ampersand, reference_to(subject, irgen),
                con.substitution_cache, state, irgen);
            if (auto invalid = std::get_if<NoOperatorReason>(&op_result)) {
                if (*invalid == NoOperatorReason::MulipleMatches)
                    return false;
        ...
        if (result.name != "__ref") {
            irgen->error(
                Error(con.expr, "Operator & always yields a reference"));
        }
        // filter the domain of "subject"
        if (auto domain = state->get_type_domain(subject);
            !domain->concrete_types.types.empty()) {
            std::erase_if(
                domain->concrete_types.types, [&result](const Type& type) {
                    if (can_match(type, result.deref())) return false;
                    if (result.deref().name == "__string_view" && type.is_str())
                        return false;
                    if (result.deref().name == "__slice" && type.is_array())
                        return false;
                    return true;
            ...
            state->push_step(Info::TypeCheckerStateDiff{
                ...
                .arg = Info::SubstitutionInformation{
                                                     ...
                                                     .result_type =
                        state->get_type_domain(subject)->get_type_list(),
                                                     .generated_by = state->current_constraint}
            });
    ...
    // TODO:  handle `has_type_variable`
        ...
        if (result.name != "__ref") {
            irgen->error(
                Error(con.expr, "Operator & always returns a reference"));
        }
...
bool ConstraintSolver::operator()(BorrowResultMutConstraint& con) {
    ...
    if (is_type_variable(subject) && is_type_variable(result)) {
        ...
        //------------The base T -> &T mapping-------------
        possible_types.add_type(Type{
            .name = "__ref_mut", .subtypes = {subject}, .module = core_module});
        //--------------operator&-------------------
        if (subject.is_str())
            possible_types.add_type(
                Type{.name = "__ref_mut",
            ...
            auto op_result = unary_operator_result(
                TokenType::RefMut, mutable_reference_to(subject, irgen),
                con.substitution_cache, state, irgen);
            if (auto invalid = std::get_if<NoOperatorReason>(&op_result)) {
                if (*invalid == NoOperatorReason::MulipleMatches)
                    return false;
                ...
                if (!extra_constraints.empty()) {
                    add_new_constraint(IfEqualThenConstrain{
                        std::move(type), result, std::move(extra_constraints)});
                }
            }
        }
        if (auto error = state->get_type_domain(result)->add_and_intersect(
        ...
        if (result.name != "__ref_mut") {
            irgen->error(Error(
                con.expr, "Operator &mut always yields a mutable reference"));
        }
        // filter the domain of "subject"
        if (auto domain = state->get_type_domain(subject);
            !domain->concrete_types.types.empty()) {
    ...
    // TODO:  handle `has_type_variable`
        ...
        if (result.name != "__ref_mut") {
            irgen->error(Error(
                con.expr, "Operator &mut always returns a mutable reference"));
        }
        if (result.subtypes[0].is_equal(subject)) {
            return true;
        }
...
bool ConstraintSolver::operator()(IfEqualThenConstrain& con) {
    ...
    if (type1.full_name() == type2.full_name()) {
        for (auto& elem : con.apply_if_true)
            add_new_constraint(std::move(elem));
    }
    return true;
}
...
void TypeCheckerState::unify_types(const Type& t1, const Type& t2,
                                   IRGenerator* irgen) {
...
void TypeCheckerState::push_variable_block() { variables.emplace_back(); }
void TypeCheckerState::pop_variable_block() { variables.pop_back(); }
    ...
    if (is_type_variable(tp)) {
        ...
        if (domain->is_solved()) {
            ...
            if (has_type_variable(domain->concrete_types.types[0])) {
                ...
                // should probably prevent recusrion by checking if the
                // solution contains the variable
                domain->concrete_types.types[0] =
                    best_repr(domain->concrete_types.types[0]);
            }
        ...
        // decompose tp and rebuild but substitute all type vars with best
        // repr
        ...
        while (!it.is_end()) {
            ...
            actual_block +=
                elem.name +
                IRGenerator::mangleGenericArgs(
                    elem.subtypes | std::views::transform([this](auto& elem) {
                        return best_repr(elem);
                    })) +
                "::";
        }
        Type final_t = it.last();
        for (auto& elem : final_t.subtypes) {
            elem = best_repr(elem);
        }
...
void Domain::Group::add_type(Type&& tp) { types.push_back(tp); }
std::optional<Error> Domain::add_and_intersect(Group&& grp,
    ...
    // We could have non concrete types so the intersection process is a bit
    // more delicate
    std::vector<Type> out;
    ...
    for (auto& type : grp.types) {
        ...
        for (auto& current_type : concrete_types.types) {
            // we redirect generated constraints from generic match to a
            // different vector so that if they don't match we don't have
            // wrong constraints in our solver
...
std::optional<Error> Domain::constrain_to_store(uint64_t val) {
    ...
    std::erase_if(concrete_types.types, [val](Type& elem) {
        ...
        return !((elem.is_signed_integral() ||
                  static_cast<uint64_t>(getIntMinOf(elem)) <= val) &&
                 getIntMaxOf(elem) >= val);
    });
    if (concrete_types.types.empty()) {
...
std::optional<Error> Domain::equal_constrain(Type other) {
    ...
    // so instead of replacing the entire domain we remove elements that
    // can't match if we empty the domain then there's an error
    std::erase_if(concrete_types.types,
                  [&other](auto& elem) { return !can_match(other, elem); });
    if (!concrete_types.types.empty()) {
        return std::nullopt;
    } else {
...
bool Domain::is_solved() { return concrete_types.types.size() == 1; }
std::vector<Type> Domain::get_type_list() { return concrete_types.types; }
Type Domain::get_solution() { return concrete_types.types[0]; }
}  // namespace Yoyo
