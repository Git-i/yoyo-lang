...
std::string Type::full_name_no_block() const {
    ...
    if (!subtypes.empty()) {
        ...
        for (auto& sub :
             std::ranges::subrange(subtypes.begin() + 1, subtypes.end()))
            final += "," + sub.full_name();
        final += ">";
    }
    return final;
}
std::string Type::full_name_no_generics() const { return block_hash + name; }
...
std::vector<std::string_view> split(std::string_view str,
    ...
    for (size_t it = 0; it < str.size() - delim.size(); ++it) {
        std::string_view curr(str.begin() + it,
                              str.begin() + it + delim.size());
        if (curr == delim) {
    ...
    result.emplace_back(str.begin() + left, str.ends_with(delim)
                                                ? str.end() - delim.size()
                                                : str.end());
    return result;
}
...
bool Type::is_assignable_from(const Type& other, IRGenerator* irgen) const {
    ...
    if (is_signed_integral()) {
        if (other.name == "ilit") return true;
        if (other.is_unsigned_integral())
            return *integer_width() > *other.integer_width();
    ...
    if (is_floating_point()) {
        ...
        if (other.is_floating_point())
            return *float_width() >= *other.float_width();
    ...
    if (name == "__called_fn" && (other.is_function() || other.is_lambda())) {
        ...
        if (other.is_function())
            as_fn = &reinterpret_cast<const FunctionType&>(other).sig;
        else
            debugbreak();
        // if (other.is_lambda()) as_fn =
        // &other.module->lambdas[other.name].second->sig;
        if (signature->parameters.size() != as_fn->parameters.size())
            return false;
        for (size_t i = 0; i < signature->parameters.size(); ++i) {
            if (signature->parameters[i].type != as_fn->parameters[i].type)
                return false;
        }
    ...
    if (is_optional()) {
        return other.name == "__null" ||
               subtypes[0].is_assignable_from(other, irgen);
    }
    if (is_gc_reference()) return false;
    if (is_mutable_reference())
        return other.is_gc_reference() && other.is_mutable &&
               deref().is_equal(other.deref());
    if (is_reference())
        return other.is_reference() && deref().is_equal(other.deref());
    // for variants, the rhs must be assignable to only one of the subtypes
    if (is_variant()) {
        bool is_valid = false;
        for (auto& subtype : subtypes) {
    ...
    if (is_mut_slice()) {
        if (other.is_mutable_reference())
            if (other.deref().is_array())
                return other.deref().subtypes[0].is_equal(subtypes[0]);
        return false;
    }
    if (is_slice()) {
    ...
    if (is_view()) {
        if (is_mut_view()) {
            if (other.is_gc_view() && subtypes[0].is_equal(other.subtypes[0]))
                return true;
        } else if (is_gc_view()) {
        } else {
            if (other.is_view() && subtypes[0].is_equal(other.subtypes[0]))
                return true;
        }
        ...
        if (auto cls = other.deref().get_decl_if_class(irgen)) {
            ...
            if (it != cls->impls.end()) {
                if (is_mut_view())
                    if (other.is_mutable_reference() || other.is_gc_reference())
                        return true;
                if (is_gc_view())
...
bool Type::is_error_ty() const { return name == "__error_type"; }
...
bool Type::is_view() const { return name.starts_with("__view"); }
bool Type::is_mut_view() const { return name == "__view_mut"; }
bool Type::is_gc_view() const { return name == "__view_gc"; }
bool Type::is_equal(const Type& other) const {
    if (is_variant() && other.is_variant()) {
        // variant ordering doesn't matter TODO consider sorting variant
        // subtypes in `saturate`
        return block_hash == other.block_hash &&
               std::set(subtypes.begin(), subtypes.end()) ==
                   std::set(other.subtypes.begin(), other.subtypes.end());
    }
    return name == other.name && subtypes == other.subtypes &&
           module == other.module && block_hash == other.block_hash;
}
...
bool Type::is_non_owning(IRGenerator* irgen) const {
    ...
    if (auto decl = get_decl_if_class(irgen))
        return decl->ownership == Ownership::NonOwning ||
               decl->ownership == Ownership::NonOwningMut;
    return false;
}
...
bool Type::is_trivially_destructible(IRGenerator* irgen,
                                     bool consider_no) const {
...
bool Type::can_be_stored() const {
    return name != "void" && !name.starts_with("__conv_result");
}
...
bool from_builtins(const Type& tp) {
    return (tp.is_conversion_result() || tp.is_array() || tp.is_char() ||
            tp.is_void() || tp.is_builtin() || tp.is_tuple() || tp.is_str() ||
            tp.name == "__called_fn" || tp.name == "impl" ||
            tp.name == "__fn" || tp.is_opaque_pointer() || tp.is_optional() ||
            tp.is_variant() || tp.is_reference() || tp.is_slice() ||
            tp.is_view() || tp.is_result() ||
            (tp.block_hash == "core::" && tp.name == "Fiber"));
}
bool advanceScope(Type& type, ModuleBase*& md, std::string& hash,
                  IRGenerator* irgen, bool);
void Type::saturate(ModuleBase* src, IRGenerator* irgen, bool do_verify) {
    ...
    if (is_mutable_reference()) subtypes[0].is_mutable = true;
    if (is_reference()) subtypes[0].is_lvalue = true;
    if (!it.is_end()) {
        ModuleBase* md = src;
        ...
        while (!it.is_end()) {
            type = it.next();
            if (!advanceScope(type, md, hash, irgen, false)) debugbreak();
        }
        name = it.last().name;
        module = md;
    ...
    } else if (auto [blk, galias] = module->findGenericAlias(block_hash, name);
               galias) {
        irgen->generateGenericAlias(module, blk, galias, subtypes);
        auto alias = module->findAlias(
            blk, name + IRGenerator::mangleGenericArgs(subtypes));
        auto old_is_lvalue = is_lvalue;
        auto old_is_mutable = is_mutable;
        *this = *alias;
        this->is_lvalue = old_is_lvalue;
    ...
    if (signature) {
        ...
        for (auto& tp : signature->parameters)
            tp.type.saturate(src, irgen, do_verify);
    }
}
...
bool Type::verify() const {
    ...
    if (!blk) return false;
    if (block_hash != blk) return false;
    return true;
}
bool Type::is_static_array() const { return name.starts_with("__arr_s"); }
...
bool Type::is_dynamic_array() const { return name == "__arr_d"; }

bool Type::is_array() const { return is_dynamic_array() || is_static_array(); }
...
Type Type::take_mutability_characteristics(const Type& other) const {
    Type t = *this;
    t.is_lvalue = other.is_lvalue;
    t.is_mutable = other.is_mutable;
    return t;
}
...
Type Type::variant_merge(Type a, Type b) {
    ...
    if (a.name == "__var") {
        if (b.name == "__var") {
            a.subtypes.insert(a.subtypes.end(), b.subtypes.begin(),
                              b.subtypes.end());
            std::vector<Type> unduplicated;
            for (auto& subtype : a.subtypes) {
                if (std::ranges::find_if(unduplicated,
                                         [&subtype](const Type& elem) {
                                             return elem.is_equal(subtype);
                                         }) == unduplicated.end())
        ...
        if (std::ranges::find_if(a.subtypes, [&b](const Type& elem) {
                return elem.is_equal(b);
            }) == a.subtypes.end())
    ...
    return {
        .name = "__var", .subtypes = {std::move(a), std::move(b)}
    };
}

ClassDeclaration* Type::get_decl_if_class(IRGenerator* irgen) const {
    if (module) {
        if (auto decl = module->findClass(block_hash, full_name_no_block());
            decl.second)
            return decl.second->second.get();
        if (auto [hsh, decl] = module->findGenericClass(block_hash, name);
            decl) {
            if (subtypes.size() != decl->clause.types.size()) {
                debugbreak();
                return nullptr;
            }
...
bool Type::is_builtin() const {
    return name == "i8" || name == "i16" || name == "i32" || name == "i64" ||
           ...
           name == "f32" || name == "f64" || name == "bool" || name == "ilit" ||
           name == "flit";
}
...
bool Type::is_boolean() const { return name == "bool"; }
...
bool Type::should_sret() const {
    return !is_primitive() && (get_decl_if_enum() == nullptr) &&
           !is_reference() && !is_char() && !is_opaque_pointer();
}
...
bool Type::is_floating_point() const {
    return name == "f32" || name == "f64" || name == "flit";
}
...
UnsaturatedTypeIterator::UnsaturatedTypeIterator(const Type& type)
    : type(type) {
...
struct MangleScanner {
    enum Type { SubOpen, Name, SubClose, AtAt, Scope };
    struct Token {
        Type type;
    ...
    std::optional<Token> next(bool update_pos = true) {
        ...
        if (next_at_at == pos) {
            if (from_pos.starts_with("::<")) {
                if (update_pos) pos += 3;
                return Token{
                    SubOpen, {from_pos.begin(), from_pos.begin() + 2}
                };
            }
            if (from_pos.starts_with("::")) {
                if (update_pos) pos += 2;
                return Token{
                    Scope, {from_pos.begin(), from_pos.begin() + 2}
                };
            }
            if (from_pos.starts_with(">")) {
                if (update_pos) pos += 1;
                return Token{
                    SubClose, {from_pos.begin(), from_pos.begin() + 1}
                };
            }
            if (update_pos) pos += 1;
            return Token{
                AtAt, {from_pos.begin(), from_pos.begin() + 2}
            };
        }
        auto ret_val = std::string_view{from_pos.begin(),
                                        from_pos.begin() + (next_at_at - pos)};
...
std::string pretty_name_suffix(const Type& tp, const std::string& blk) {
    ...
    if (tp.is_gc_reference()) return "^" + tp.subtypes[0].pretty_name(blk);
    if (tp.is_reference()) return "&" + tp.subtypes[0].pretty_name(blk);
    if (tp.is_static_array())
        return "[" + tp.subtypes[0].pretty_name(blk) + "; " +
               std::to_string(tp.static_array_size()) + "]";
    if (tp.is_mut_slice())
        return "[" + tp.subtypes[0].pretty_name(blk) + ":&mut]";
    if (tp.is_slice()) return "[" + tp.subtypes[0].pretty_name(blk) + ":&]";
    if (tp.name == "__called_fn")
        return "called " + tp.signature->pretty_name("");
    if (tp.name.starts_with("?")) return "{unknown:" + tp.name + "}";
    if (tp.is_tuple()) {
        std::string res = "(";
    ...
    if (!tp.subtypes.empty()) {
        res += "::<";
        for (auto& i : std::ranges::subrange(tp.subtypes.begin(),
                                             tp.subtypes.end() - 1)) {
...
std::string FunctionSignature::pretty_name(
    const std::string& block_hash) const {
    std::string final_str = "fn(";
    if (parameters.size() >= 1) {
        final_str.append(parameters[0].type.pretty_name(block_hash));
        for (auto& param :
             std::ranges::subrange(parameters.begin() + 1, parameters.end())) {
...
Type parseType(MangleScanner& scanner) {
    ...
    if (next->type == MangleScanner::SubOpen) {
        ...
        scanner.buffer.push_back(next.value());
