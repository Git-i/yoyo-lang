    ...
    while (!waiting.empty() || !routs.empty()) {
        ...
        for (size_t i = 0; i < routs.size(); i++) {
            ...
            if (mco_status(rou) == MCO_SUSPENDED) {
                ...
                if (auto find_it = std::ranges::find(key_range, rou);
                    find_it == key_range.end()) {
            ...
            if (mco_status(rou) == MCO_DEAD) {
                auto find_it =
                    std::ranges::find_if(waiting, [rou](const auto& elem) {
...
void Runtime::release_job(mco_coro* co) { is_externally_owned[co] = false; }
}  // namespace Yoyo
