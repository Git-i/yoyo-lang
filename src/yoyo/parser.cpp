...
Parser::Parser(std::string& src) : source(src), scn(source) {
    ...
    prefixParselets[TokenType::IntegerLiteral] =
        std::make_shared<IntLiteralParselet>();
    prefixParselets[TokenType::RealLiteral] =
        std::make_shared<RealLiteralParselet>();
    prefixParselets[TokenType::StringLiteral] =
        std::make_shared<StringLiteralParselet>();
    prefixParselets[TokenType::CharLiteral] =
        std::make_shared<CharLiteralParselet>();
    ...
    prefixParselets[TokenType::LSquare] =
        std::make_shared<ArrayLiteralParselet>();
    ...
    auto sum_parselet =
        std::make_shared<BinaryOperationParselet>(Precedences::Sum);
    auto product_parselet =
        std::make_shared<BinaryOperationParselet>(Precedences::Product);
    auto equality_parselet =
        std::make_shared<BinaryOperationParselet>(Precedences::Equality);
    auto relational_parselet =
        std::make_shared<BinaryOperationParselet>(Precedences::Relational);
    auto access_parselet =
        std::make_shared<BinaryOperationParselet>(Precedences::MemberAccess);
    auto bsh_parselet =
        std::make_shared<BinaryOperationParselet>(Precedences::BitShift);
    auto range_parselet =
        std::make_shared<BinaryOperationParselet>(Precedences::RangeOp);
    ...
    infixParselets[TokenType::Pipe] =
        std::make_shared<BinaryOperationParselet>(Precedences::BitOr);
    infixParselets[TokenType::Ampersand] =
        std::make_shared<BinaryOperationParselet>(Precedences::BitAnd);
    infixParselets[TokenType::Caret] =
        std::make_shared<BinaryOperationParselet>(Precedences::BitXor);

    auto assign_parselet = std::make_shared<BinaryOperationParselet>(
        Precedences::Assignment, true);
    ...
    infixParselets[TokenType::As] =
        std::make_shared<AsExpressionParselet>(Precedences::As);

    infixParselets[TokenType::DoubleAmpersand] =
        std::make_shared<LogicalOperationParselet>(Precedences::LogicAnd);
    infixParselets[TokenType::DoublePipe] =
        std::make_shared<LogicalOperationParselet>(Precedences::LogicOr);

    infixParselets[TokenType::Bang] =
        std::make_shared<PostfixOperationParselet>(
            Precedences::InvalidPropagate);
    infixParselets[TokenType::LParen] =
        std::make_shared<CallOperationParselet>(Precedences::Call);

    infixParselets[TokenType::DoubleColon] =
        std::make_shared<ScopeOperationParselet>(Precedences::ScopeResolve);

    infixParselets[TokenType::LSquare] =
        std::make_shared<SubscriptParselet>(Precedences::Call);
}
std::unique_ptr<ASTNode> Parser::parseExpressionOrDeclaration() {
    ...
    case TokenType::Continue:
        return parseBreakContinueStatement(*Get());
    default: {
...
std::unique_ptr<Statement> Parser::parseModuleImport(Token identifier) {
    ...
    return std::make_unique<ModuleImport>(std::move(name),
...
std::unique_ptr<Statement> Parser::parseTopLevelDeclaration() {
    ...
    auto iden = Get();
    if (!iden) return nullptr;
    if (iden->type == TokenType::Operator)
        return parseOperatorOverload(iden.value());
    if (iden->type == TokenType::Using)
        return parseUsingDeclaration(iden.value());
    ...
    case TokenType::Fn:
        decl = parseFunctionDeclaration(iden.value());
        break;
    case TokenType::Class:
        decl = parseClassDeclaration(iden.value(), false);
        break;
    case TokenType::Struct:
        decl = parseClassDeclaration(iden.value(), true);
        break;
    case TokenType::Enum:
        decl = parseEnumDeclaration(iden.value());
        break;
    case TokenType::Interface:
        decl = parseInterfaceDeclaration(iden.value());
        break;
    case TokenType::Const:
        decl = parseConstDeclaration(iden.value());
        break;
    case TokenType::EnumFlag:
        break;  // TODO
    case TokenType::Union:
        decl = parseUnionDeclaration(iden.value());
        break;
    case TokenType::Alias:
        decl = parseAliasDeclaration(iden.value());
        break;
    case TokenType::Module:
        decl = parseModuleImport(iden.value());
        break;
    case TokenType::Macro:
        decl = parseMacroDeclaration(iden.value());
        break;
    default:
        error(
            "Expected 'fn', 'enum', 'const', 'alias', 'module', "
            "'interface' or 'union'",
            iden);
    }
...
std::unique_ptr<Statement> Parser::parseOperatorOverload(const Token& tok) {
    ...
    if (op->type == TokenType::LSquare) {
        ...
        auto lbrace = Get();
        auto rbrace = Get();
        if (!lbrace || lbrace->type != TokenType::LSquare)
            error("Operator cannot be overloaded", op);
        if (!rbrace || rbrace->type != TokenType::RSquare)
            error("Operator cannot be overloaded", op);
    ...
    } else if (!op->can_be_overloaded())
        error("Operator cannot be overloaded", Peek());
    GenericClause clause;
    if (auto generic_open = Peek();
        generic_open->type == TokenType::TemplateOpen)
    ...
    auto return_val = std::make_unique<OperatorOverload>(
...
void Parser::error(std::string message, std::optional<Token> tk) {
    ...
    std::cerr << "at token: " << (tk ? tk->text : "Invalid")
...
std::unique_ptr<Statement> Parser::parseFunctionDeclaration(Token identifier) {
    ...
    std::optional<GenericClause> gclause =
        next_tok->type == TokenType::TemplateOpen ? parseGenericClause()
                                                  : std::nullopt;
    auto sig = parseFunctionSignature();
    ...
    if (tk && tk->type == TokenType::Directive && tk->text == "c_import") {
        ...
        auto fn = std::make_unique<FunctionDeclaration>(
            std::string{identifier.text}, *std::move(sig), nullptr);
        auto stat = parseCimport(*this);
        stat->parent = fn.get();
        auto end = stat->end;
        fn->body = std::move(stat);
        return Statement::attachSLAndParent(std::move(fn), identifier.loc, end,
                                            parent);
    }
    ...
    if (gclause)
        fn_decl = std::make_unique<GenericFunctionDeclaration>(
            *std::move(gclause),
            FunctionDeclaration(std::string{identifier.text}, *std::move(sig),
                                std::move(stat)));
    else
        fn_decl = std::make_unique<FunctionDeclaration>(
            std::string{identifier.text}, *std::move(sig), std::move(stat));
    stat_ptr->parent = fn_decl.get();
    return Statement::attachSLAndParent(std::move(fn_decl), identifier.loc,
                                        stat_ptr->end, parent);
}
std::unique_ptr<Statement> Parser::parseInterfaceDeclaration(Token identifier) {
    ...
    if (Peek() && Peek()->type == TokenType::TemplateOpen) {
        ...
        intf = std::make_unique<InterfaceDeclaration>();
    if (!discard(TokenType::Equal)) error("Expected '='", Peek());
    ...
    while (!discard(TokenType::RCurly)) {
        ...
        if (tk->type == TokenType::Impl) {
            ...
            while (!discard(TokenType::RCurly)) {
                ...
                if (dynamic_cast<GenericFunctionDeclaration*>(stat.get()))
                    error("Generic not allowed here", iden);
                impl.impl.methods.emplace_back(
                    reinterpret_cast<FunctionDeclaration*>(stat.release()));
            }
        ...
        auto decl = Statement::attachSLAndParent(
            std::make_unique<FunctionDeclaration>(std::move(name),
                                                  std::move(sig), nullptr),
            tk->loc, discardLocation, intf.get());
        intf->methods.emplace_back(
            reinterpret_cast<FunctionDeclaration*>(decl.release()));
    }
    return Statement::attachSLAndParent(std::move(intf), identifier.loc,
                                        discardLocation, parent);
}
std::vector<char> parseDomainList(Parser& p) {
...
std::unique_ptr<Statement> Parser::parseUnionDeclaration(Token iden) {
    ...
    while (!discard(TokenType::RCurly)) {
        ...
        if (isTopLevelDeclaration()) {
            ...
            std::ignore =
                discard(TokenType::Comma);  // optional ',' after declaration
        } else {
            auto iden = Get();
            if (!iden) return nullptr;
            if (iden->type != TokenType::Identifier)
            ...
            if (fields.contains(as_str))
                error("Name with field already exists", Peek());
            fields[as_str] = parseType(0, true).value_or(Type{});
            if (!discard(TokenType::Comma)) {
                if (!discard(TokenType::RCurly))
...
std::unique_ptr<Statement> Parser::parseMacroDeclaration(Token identifier) {
    ...
    return Statement::attachSLAndParent(std::move(decl), identifier.loc, end,
                                        parent);
}
std::unique_ptr<Statement> Parser::parseUsingDeclaration(Token using_tok) {
    ...
    return Statement::attachSLAndParent(
        std::make_unique<UsingStatement>(std::move(tp)), using_tok.loc,
        discardLocation);
}
Attribute parseAttribute(Parser& p) {
    ...
    if (p.discard(TokenType::LParen)) {
        ...
        if (next_tk->type != TokenType::Identifier &&
            next_tk->type != TokenType::StringLiteral)
...
std::optional<FunctionSignature> Parser::parseFunctionSignature() {
    if (discard(TokenType::Arrow))
        return FunctionSignature{.returnType = *parseType(0),
                                 .return_is_ref = false,
                                 .parameters = {}};
    // empty signature
    if (!discard(TokenType::LParen)) {
        return FunctionSignature{.returnType = {"__inferred"},
                                 .return_is_ref = false,
                                 .parameters = {}};
    }
    ...
    while (discard(TokenType::Comma)) {
        ...
        if (tk->type == TokenType::Underscore) {
            ...
            sig.parameters.push_back(
                std::move(p).value_or(FunctionParameter{}));
        } else if (tk->type == TokenType::Colon) {
            Get();
            auto p = parseParam("");
            if (!p)
...
std::optional<GenericClause> Parser::parseGenericClause() {
    ...
    if (iden->type != TokenType::Identifier)
        ...
        if (discard(TokenType::Colon))
            constraints[names.back()] = parseGenericConstraints(*this).value_or(
        ...
        if (iden->type != TokenType::Identifier)
            ...
            if (discard(TokenType::Colon))
                constraints[names.back()] =
                    parseGenericConstraints(*this).value_or(
                        std::vector<Constraint>{});
        }
    }
...
std::unique_ptr<Statement> Parser::parseVariableDeclaration(Token identifier) {
    ...
    if (tk->type == TokenType::Equal) {
        ...
        if (!discard(TokenType::SemiColon) &&
            !canOmitSemiColon(initializer.get()))
            error("Expected ';'", Peek());
        return Statement::attachSLAndParent(
            std::make_unique<VariableDeclaration>(
                identifier, std::nullopt, std::move(initializer), is_mut),
    ...
    if (!discard(TokenType::SemiColon)) error("Expected ';'", Peek());
    return Statement::attachSLAndParent(
        std::make_unique<VariableDeclaration>(identifier, type, std::move(init),
                                              is_mut),
        identifier.loc, discardLocation, parent);
}
std::unique_ptr<Statement> Parser::parseEnumDeclaration(Token identifier) {
    ...
    while (!discard(TokenType::RCurly)) {
        ...
        auto name_tk = Get();
        if (name_tk->type != TokenType::Identifier) {
            error("Expected identifier", Peek());
            continue;
        }
        ...
        if (discard(TokenType::Equal)) {
            ...
            if (value_tk->type != TokenType::IntegerLiteral) {
                ...
                if (std::ranges::find(usedIntegers, value) !=
                    usedIntegers.end())
                    error("Duplicate enum values", Peek());
            }
        }
    ...
    return Statement::attachSLAndParent(
        std::make_unique<EnumDeclaration>(std::string{identifier.text},
                                          std::move(values), std::move(stats)),
        identifier.loc, discardLocation, parent);
}
// struct and classes are virtually the same, except structs don't have
// access specs
std::unique_ptr<Statement> Parser::parseClassDeclaration(Token identifier,
    ...
    std::vector<char> domains = parseDomainList(*this);
    if (!discard(TokenType::Equal)) error("Expected '='", Peek());
    if (!discard(TokenType::LCurly)) error("Expected '{'", Peek());
    while (!discard(TokenType::RCurly)) {
        auto attr_list = parseAttributeList();
        ...
        AccessSpecifier spec =
            isStruct ? AccessSpecifier::Public : AccessSpecifier::Private;
        auto peek_tk = Peek();
        ...
        while (true) {
            ...
            if (tk->type == TokenType::Static) {
                ...
                if (spec != AccessSpecifier::Private)
                    error("Multiple access specifiers on one object", tk);
                spec = AccessSpecifier::Public;
                Get();
            } else if (tk->type == TokenType::Mod) {
                if (isStruct)
                    error("Cannot specify module-level access for a struct",
                          tk);
                if (spec != AccessSpecifier::Private)
                    error("Multiple access specifiers on one object", tk);
                ...
                error(
                    "Unexpected token, expected '_', 'pub', 'mod' or "
                    "identifier",
                    Peek());
        }
        auto iden = *Get();
        ...
        auto type = parseType(0, true);
        vars.push_back(ClassVariable{.name = std::string{iden.text},
    ...
    Ownership own_method;
    if (!clause)
        cs_stat = std::make_unique<ClassDeclaration>(
            identifier, std::move(vars), std::move(stat), own_method,
            std::move(impls), std::move(domains));
    else
        cs_stat = std::make_unique<GenericClassDeclaration>(
            identifier, std::move(vars), std::move(stat), own_method,
            std::move(impls), std::move(clause).value(), std::move(domains));
...
std::unique_ptr<Statement> Parser::parseAliasDeclaration(Token identifier) {
    ...
    std::optional<GenericClause> clause;
    if (!next_tok) {
        error("Unexpected EOF", next_tok);
        return nullptr;
    }
    if (next_tok->type == TokenType::TemplateOpen)
        clause = parseGenericClause();
    if (!discard(TokenType::Equal)) error("Expected '='", Peek());
    Type tp = parseType(0).value_or(Type{});
    if (!discard(TokenType::SemiColon)) error("Expected ';'", Peek());
    return Statement::attachSLAndParent(
        clause ? std::make_unique<GenericAliasDeclaration>(
                     std::string(identifier.text), std::move(tp),
                     *std::move(clause))
               : std::make_unique<AliasDeclaration>(
                     std::string(identifier.text), std::move(tp)),
        identifier.loc, discardLocation, parent);
}
std::unique_ptr<Statement> Parser::parseConstDeclaration(Token identifier) {
    ...
    auto decl = std::make_unique<ConstantDeclaration>(
        std::string(identifier.text), std::move(type), nullptr);
...
std::unordered_map<std::string, std::unique_ptr<Expression>>
    ...
    Get();  // discard '{'
    if (discard(TokenType::RCurly))
        return {};  // empty initialization, should this be allowed!??
    ...
    while (!discard(TokenType::RCurly)) {
        ...
        if (result.contains(name))
            error("Duplicate initialization of " + name, iden);
        Get();  // get the identifier
        // if not equal default to a name expression i.e Object{.y = 10, .x,
        // .z};
        if (!discard(TokenType::Equal)) {
            auto nexpr = std::make_unique<NameExpression>(name);
            nexpr->beg = iden->loc;
            nexpr->parent = parent;
            nexpr->end = SourceLocation{iden->loc.line,
                                        iden->loc.column + iden->text.size()};
...
std::unique_ptr<Statement> Parser::parseReturnStatement(Token return_tok) {
    ...
    if (discard(TokenType::SemiColon)) {
        return Statement::attachSLAndParent(
            std::make_unique<ReturnStatement>(nullptr), return_tok.loc,
            discardLocation, parent);
    }
    auto expr = parseExpression(0);
    if (!discard(TokenType::SemiColon)) error("Expected ';'", Peek());
    return Statement::attachSLAndParent(
        std::make_unique<ReturnStatement>(std::move(expr)), return_tok.loc,
        discardLocation, parent);
}
...
std::unique_ptr<Expression> Parser::parseConditionalExtraction(Token tk) {
    ...
    if (else_tk && else_tk->type == TokenType::Else) {
        ...
        if (discard(TokenType::Pipe)) {
            if (discard(TokenType::Ampersand)) {
                ...
                if (discard(TokenType::Mut))
                    else_cap = ConditionalExtraction::RefMut;
            }
            auto else_iden = Get();
            if (!else_iden) return nullptr;
            if (else_iden->type != TokenType::Identifier)
                error("Expected identifier", else_iden);
            if (!discard(TokenType::Pipe)) error("Expected '|'", Peek());
    ...
    return Expression::attachSLAndParent(
        std::make_unique<ConditionalExtraction>(
            name, then_cap, std::move(condition), std::move(then),
            std::move(else_stat), else_name, else_cap),
        tk.loc, end, parent);
}
std::unique_ptr<Statement> Parser::parseForStatement(Token tk) {
    ...
    if (!iden) return nullptr;
    if (iden->type != TokenType::Identifier &&
        iden->type != TokenType::Underscore)
        error("Expected identifier or '_'", Peek());
    Get();
...
std::unique_ptr<Statement> Parser::parseBreakContinueStatement(Token tk) {
    ...
    if (tk.type == TokenType::Break)
        stat = std::make_unique<BreakStatement>();
    else if (tk.type == TokenType::Continue)
        stat = std::make_unique<ContinueStatement>();
    else
        return nullptr;
    return Statement::attachSLAndParent(std::move(stat), tk.loc,
                                        discardLocation, parent);
}
...
std::unique_ptr<Statement> Parser::parseWithStatement(Token tk) {
    ...
    auto expr = parseExpression(0);
    if (!discard(TokenType::RParen)) error("Expected ')'", Peek());
    auto stat = std::make_unique<WithStatement>(
        std::string(name->text), std::move(expr), parseStatement());
...
// [T; <expr>] -> static array
// [T; *] -> dynammic array
std::optional<Type> parseArrayType(Token t, Parser& parser, bool full) {
    auto next = parser.parseType(0, full);
    ...
    if (parser.discard(TokenType::SemiColon)) {
        if (parser.discard(TokenType::Star) && parser.Peek() &&
            parser.Peek()->type == TokenType::RSquare) {
            name = "__arr_d";
        } else {
            auto size = parser.parseExpression(0);
            // this is really dirty and bad code don't learn from this
            struct Deleter {
                void operator()(FunctionSignature* ptr) {
                    delete (Expression*)ptr;
                }
            };
    ...
    return Type{.name = std::move(name),
                .subtypes = {std::move(next).value_or(Type{})},
                .signature = sig};
}
...
std::optional<Type> parseTemplateTypeExpr(Parser& p, Type left, bool full) {
    // Types come first then annotations
    auto do_annotations = [&]() {
        ...
        while (p.discard(TokenType::Comma)) {
            ...
            left.domains.push_back(tk->text[0]);
        }
        return true;
    };
...
std::optional<Type> parseViewType(Token t, Parser& p) {
    if (auto pk = p.Peek();
        pk && pk->type == TokenType::Identifier && pk->text == "str") {
        return Type{.name = "__str_view"};
    }
    // similar to dyn Trait in rust
    return Type{.name = "__view", .subtypes = {p.parseType(0).value()}};
}
std::optional<Type> parsePostfixTypeExpr(Parser& p, Type left, Token t) {
    if (t.type == TokenType::Question)
        return Type{.name = "__opt", .subtypes = {std::move(left)}};
    else if (t.type == TokenType::BackSlash) {
        auto err_ty = p.parseType(
            0);  // TODO result type precedence(type precedence in general)
        return Type{
            .name = "__res",
            .subtypes = {std::move(left), std::move(err_ty).value()}
        };
    }
    return std::nullopt;
}
...
std::optional<Type> parseGroupType(Parser& p, bool full) {
    if (p.discard(TokenType::RParen)) return Type{"void"};
    std::optional<TokenType> seperator;
    std::vector<Type> subtypes;
    while (!p.discard(TokenType::RParen)) {
        subtypes.push_back(p.parseType(0, full).value_or(Type{}));
        ...
        if (!p.discard(*seperator)) p.error("Expected ','", p.Peek());
    }
    Type t{.module = nullptr};
    if (subtypes.size() == 1)
        t = std::move(subtypes[0]);
    else
        t.subtypes = std::move(subtypes);

    if (seperator && *seperator == TokenType::Pipe)
        t.name = "__var";
    else if (seperator && *seperator == TokenType::Comma)
        t.name = "__tup";
...
UsingStatement::ContentTy Parser::parseUsingContent(uint32_t generic_num) {
    ...
    if (auto peek_tk = Peek();
        !peek_tk || peek_tk->type != TokenType::DoubleColon) {
        return UsingSingle{.block = "", .entity = std::string(tk->text)};
    }
        ...
        if (tk && tk->type == TokenType::DoubleColon) {
            ...
            if (discard(TokenType::LCurly)) {
                ...
                while (!discard(TokenType::RCurly)) {
                    entities.emplace_back(
                        parseUsingContent(generic_num + new_generic_nums));
                    if (!discard(TokenType::Comma)) {
                        if (!discard(TokenType::RCurly)) {
                            error("Expected '}'", Peek());
                            return {};
                ...
                return UsingMultiple{.block = std::move(block),
            ...
            if (discard(TokenType::Star)) {
                ...
                if (generic_num + new_generic_nums > 0)
                    error(
                        "...::* is not supported in generic using "
                        "declaration",
            ...
            if (!tk || tk->type != TokenType::Identifier) {
                error("Expected identifier", tk);
                return {};
            }
            if (discard(TokenType::TemplateOpen)) {
                block += "::<";
                while (true) {
                    // I don't think its possible to encounter the ">>" case
                    // here
                    if (discard(TokenType::Greater)) {
                        // remove trailing comma
                        if (block.back() == ',') block.pop_back();
                        // there were no generics so we error (we could just
                        // proceed but idk
                        if (block.ends_with("::<"))
                            error("Empty generics not allowed",
                    ...
                    if (discard(TokenType::Question)) {
                        ...
                        new_generic_nums++;
                        auto expected_num =
                            std::to_string(new_generic_nums + generic_num);
                        if (num_tk->type == TokenType::IntegerLiteral &&
                            num_tk->text == expected_num) {
                            // valid
                        } else
                            error("Expected " + expected_num, num_tk);
                        block += "?" + expected_num;
                    }
                    ...
                    if (!discard(TokenType::Comma)) {
                        ...
                        if (Peek() && Peek()->type == TokenType::Greater)
                            continue;
                        error("Expected ',' or '>'", Peek());
                    }
                }
                auto t = parseTemplateTypeExpr(
                    *this, Type{.name = std::string(tk->text)}, false);
                block += t->full_name() + "::";
                if (!Peek() || Peek()->type != TokenType::DoubleColon) {
                    error("Expected '::'", tk);
                    return {};
                }
            }
            // check if it ends here
                ...
                if (!peek_tk || peek_tk->type != TokenType::DoubleColon) {
                    ...
                    return UsingSingle{.block = std::move(block),
...
std::optional<Type> Parser::parseType(uint32_t precedence, bool full) {
    auto tk = Peek();
    if (!tk) return std::nullopt;
    std::optional<Type> t;
    switch (tk->type) {
    case TokenType::Identifier:
        Get();
        t = Type(std::string(tk->text), {});
        break;
    case TokenType::LSquare:
        Get();
        t = parseArrayType(*tk, *this, full);
        break;
    case TokenType::Ampersand:
        Get();
        t = parseRefType(*tk, *this, full);
        break;
    case TokenType::LParen:
        Get();
        t = parseGroupType(*this, full);
        break;
    case TokenType::Caret:
        Get();
        t = parseGCRefType(*tk, *this);
        break;
    case TokenType::Called:
        Get();
        t = parseCalledFnType(*tk, *this);
        break;
    ...
    while (precedence < GetNextTypePrecedence()) {
        ...
        switch (tk->type) {
        case TokenType::TemplateOpen:
            t = parseTemplateTypeExpr(*this, std::move(t).value(), full);
            break;
        case TokenType::Question:
            [[fallthrough]];
        case TokenType::BackSlash:
            t = parsePostfixTypeExpr(*this, std::move(t).value(), *tk);
            break;
        case TokenType::DoubleColon: {
            std::optional<Type> tp = parseType(precedence, full);
