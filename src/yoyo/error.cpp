...
#include <span>
namespace Yoyo {
SourceView::SourceView(std::string& source, std::string filename)
...
std::string markers_for(
    std::string prefix, bool color, size_t line,
    std::span<const std::pair<SourceSpan, std::string>> markers,
    ...
    auto begins_but_not_ends =
        [line](const std::pair<SourceSpan, std::string>& sp) {
    ...
    for (auto& marker : markers | filter(ends_here) | views::reverse) {
        ...
        size_t col_begin =
            span.begin.line == line
                ? span.begin.column
                : 1;  // view.lines[line - 1].find_first_not_of(' ') + 1;
        ...
        rows.front().replace(col_begin - 1, col_end - col_begin,
                             std::string(col_end - col_begin, '$'));
        size_t bar_position = col_end - col_begin == 1
                                  ? col_begin - 1
                                  : col_begin + (col_end - col_begin - 2) / 2;
        ...
        if (color)
            rows.back().insert(
                bar_position + 3,
                std::format("\033[1;34m{}\033[0m", marker.second));
        else
    ...
    for (auto& span : markers | filter(begins_but_not_ends) | keys) {
        ...
        rows.front().replace(col_begin - 1, line_size - col_begin,
                             std::string(line_size - col_begin, '$'));
    }
...
std::string Error::to_string(const SourceView& view, bool enable_color) const {
    ...
    auto result = std::format("{}{}:{}:{} error: {}{}\n",
                              enable_color ? "\033[1;31m" : "", view.filename,
                              span.begin.line, span.begin.column, summary,
                              enable_color ? "\033[0m" : "");
    auto render_line = [this, &view, enable_color, &lines](size_t line) {
        ...
        if (enable_color && span.end.line >= line && span.begin.line <= line) {
            size_t col_begin =
                span.begin.line == line ? span.begin.column - 1 : 0;
            size_t col_end =
                span.end.line == line ? span.end.column - 1 : line_body.size();
        ...
        lines.emplace_back(
            markers_for("       ", enable_color, line, markers, view));
    };
    ...
    for (auto& marker : markers) {
        if (auto it =
                std::ranges::find(rendered_lines, marker.first.begin.line);
            it != rendered_lines.end())
            continue;
        ...
        lines.emplace_back(
            "\n" + std::string(red) +
            std::string(
                result.size() - (enable_color ? red.size() + reset.size() : 0),
                '=') +
            reset);

        for (auto line : std::views::iota(marker.first.begin.line,
                                          marker.first.end.line + 1)) {
            rendered_lines.push_back(line);
            render_line(line);
        }
    }

    for (const auto& line : lines) result.append(line);
