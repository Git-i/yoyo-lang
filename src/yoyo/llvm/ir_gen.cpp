...
llvm::Type* LLVMIRGenerator::ToLLVMType(const Type& type, bool is_ref) {
    ...
    auto t = type.module ? reinterpret_cast<LLModule*>(type.module)
                               ->ToLLVMType(type, block_hash, this, {})
                         : reinterpret_cast<LLModule*>(module)->ToLLVMType(
                               type, block_hash, this, {});
    if (t) return t;
    if (type.is_tuple()) {
    ...
    if (type.is_variant()) {
        ...
        for (auto& subtype : type.subtypes) {
            ...
            size_t sz = 0;
            if (!as_struct)
                sz = sub_t->getPrimitiveSizeInBits() / 8;
            else
                sz = layout.getStructLayout(as_struct)->getSizeInBytes();
            if (sz > size) size = sz;
        }
        args[0] = llvm::ArrayType::get(llvm::Type::getInt8Ty(context), size);
        args[1] = llvm::Type::getInt32Ty(
            context);  // 2^32 is a reasonable amount of variant subtypes
        return llvm::StructType::get(context, args);
    }
    // this may be a generic instantiation that doesn't exist yet
    if (type.module) {
        if (auto [_, exists] =
                type.module->findGenericClass(type.block_hash, type.name);
            exists) {
            generateGenericClass(
                type.module, type.block_hash, exists,
                type.subtypes);  // this should generally be safe(hopefully)
            std::string name = type.name + mangleGenericArgs(type.subtypes);
            if (auto det =
                    std::get<1>(reinterpret_cast<LLModule*>(type.module)
                                    ->findClassWithType(type.block_hash, name)
                                    .second))
                return det;
        }
    }
...
void* Yoyo_malloc_wrapper_dont_use_name(size_t size) { return GC_malloc(size); }
}
...
llvm::Function* getMallocFunction(LLVMIRGenerator* irgen) {
    ...
    fn = llvm::Function::Create(type, llvm::GlobalValue::ExternalLinkage,
                                "Yoyo_malloc_wrapper_dont_use_name",
                                irgen->code);
    return fn;
}
...
llvm::FunctionType* LLVMIRGenerator::ToLLVMSignature(
    ...
    // Non primitives are passed via pointers and returned via
    // pointers(sret)
    ...
    std::ranges::transform(sig.parameters, args.begin() + use_sret,
                           [this](const FunctionParameter& p) {
                               auto t = ToLLVMType(p.type, false);
                               if (p.type.should_sret()) t = t->getPointerTo();
                               return t;
                           });
...
llvm::AllocaInst* LLVMIRGenerator::Alloca(std::string_view name,
                                          llvm::Type* type) {
    auto entry_block = &builder->GetInsertBlock()->getParent()->getEntryBlock();
    llvm::IRBuilder<> temp(entry_block, entry_block->begin());
...
llvm::Value* LLVMIRGenerator::Malloc(std::string_view name, llvm::Value* size) {
    ...
    if (!malloc_fn) {
        ...
        malloc_fn = llvm::Function::Create(
            malloc_sig, llvm::GlobalValue::ExternalLinkage, "malloc", code);
    }
...
void LLVMIRGenerator::Free(llvm::Value* value) {
    ...
    if (!free_fn) {
        auto free_sig = llvm::FunctionType::get(
            llvm::Type::getVoidTy(context),
            {llvm::PointerType::get(context, 0)}, false);
        free_fn = llvm::Function::Create(
            free_sig, llvm::GlobalValue::ExternalLinkage, "free", code);
    }
    builder->CreateCall(free_fn, {value});
}

bool IRGenerator::isShadowing(const std::string& name) const { return false; }
...
llvm::Value* prepareValidDropFlagFor(LLVMIRGenerator* irgen, const Type& tp) {
    ...
    auto flag =
        irgen->Alloca("drop_flag", llvm::Type::getInt1Ty(irgen->context));
    irgen->builder->CreateStore(llvm::ConstantInt::getTrue(irgen->context),
                                flag);
    return flag;
}
bool should_sret_C(llvm::Type* tp, ModuleBase* md) {
    ...
    if (target.getArch() == llvm::Triple::x86_64 &&
        target.getEnvironment() == llvm::Triple::MSVC) {
...
llvm::FunctionType* toCSignature(LLVMIRGenerator* irgen,
                                 const FunctionSignature& sig) {
    ...
    if (!return_as_llvm->isVoidTy()) {
        ...
        // use an integer register win64 only
            ...
            return_t = llvm::IntegerType::get(
                irgen->context,
                irgen->module->engine->jit->getDataLayout().getTypeSizeInBits(
                    return_as_llvm));
    } else
        return_t = return_as_llvm;
            ...
            arg_tys.push_back(
                as_llvm->isPointerTy()
                    ...
                    : llvm::IntegerType::get(
                          irgen->context,
                          irgen->module->engine->jit->getDataLayout()
                              .getTypeSizeInBits(as_llvm)));
        }
    }
    return llvm::FunctionType::get(return_t, arg_tys, false);
}

void handleCImport(LLVMIRGenerator* irgen, FunctionDeclaration* decl,
                   llvm::Function* func) {
    ...
    auto c_fn = llvm::Function::Create(sig, llvm::GlobalValue::ExternalLinkage,
                                       c_import->function_name, irgen->code);
    size_t param_idx = 0;
    for (auto arg : std::ranges::views::iota(offset, func->arg_size())) {
        ...
        if (decl->signature.parameters[param_idx].type.should_sret()) {
            auto as_llvm = irgen->ToLLVMType(
                decl->signature.parameters[param_idx].type, false);
            if (should_sptr_C(as_llvm, irgen->module)) {
                args.push_back(arg_val);
            } else {
                auto ty = llvm::IntegerType::get(
                    irgen->context, irgen->module->engine->jit->getDataLayout()
                                        .getTypeSizeInBits(as_llvm));
    ...
    if (decl->signature.returnType.is_void())
...
void LLVMIRGenerator::operator()(FunctionDeclaration* decl) {
    ...
    if (builder->GetInsertBlock())
        oldBuilder = std::make_unique<llvm::IRBuilder<>>(
    ...
    if ((func = code->getFunction(name))) {
        if (!func->empty()) {
            error(Error(decl, "Function already exists"));
            return;
        }
        ...
        func = llvm::Function::Create(ToLLVMSignature(this_entry->sig),
                                      llvm::GlobalValue::ExternalLinkage, name,
                                      code);
        uses_sret = decl->signature.returnType.should_sret();
        if (uses_sret)
            func->addAttributeAtIndex(
                1, llvm::Attribute::get(context, llvm::Attribute::StructRet,
                                        return_as_llvm_type));
    }
    ...
    if (!return_t.is_void())
        currentReturnAddress =
            uses_sret ? static_cast<llvm::Value*>(func->getArg(0))
                      : static_cast<llvm::Value*>(
                            Alloca("return_address", return_as_llvm_type));
    auto old_hash = block_hash;
    block_hash = name + "::";
    pushScope();
    CFGNode::prepareFromFunction(function_cfgs.emplace_back(CFGNodeManager{}),
                                 decl);
    ...
    for (auto& param : this_entry->sig.parameters) {
        if (!param.name.empty()) {
            auto param_type =
                func->getFunctionType()->getFunctionParamType(idx + uses_sret);
            ...
            new_fn_vars.back()[param.name] = {var, std::move(type), flag};
        }
        if (param.type.is_lambda()) {
            auto& entry = reinterpret_cast<LLModule*>(param.type.module)
                              ->lambdas[param.type.name];
            ...
            for (auto& capture : caps) {
                auto var =
                    builder->CreateStructGEP(llvm_type, arg, capture_idx);
                NameExpression nexpr(capture.name);
                auto type = ExpressionTypeChecker{this}(&nexpr);
                Type final_type;
                if (capture.cp_type == Ownership::NonOwning)
                    final_type = type->reference_to();
                else if (capture.cp_type == Ownership::NonOwningMut)
                    final_type = type->mutable_reference_to();
                else
                    final_type = std::move(type).value();
                new_fn_vars.back()[capture.name] = {var, std::move(final_type),
                                                    nullptr};
                capture_idx++;
            }
        }
    ...
    // evaluate body we give a new scope whatever so that the function can't
    // access outside vars
    ...
    if (uses_sret || return_t.is_void())
        builder->CreateRetVoid();
    else
        builder->CreateRet(builder->CreateLoad(
            reinterpret_cast<llvm::AllocaInst*>(currentReturnAddress)
                ->getAllocatedType(),
            currentReturnAddress));
    block_hash = old_hash;
    if (oldBuilder) builder.swap(oldBuilder);
...
void LLVMIRGenerator::operator()(ExpressionStatement* stat) {
    ...
    if (!ty.is_lvalue) eval.destroy(val, ty);
}
void LLVMIRGenerator::operator()(ConstantDeclaration* decl) {
    ...
    if (std::holds_alternative<void*>(val.internal_repr)) {
        auto gv = reinterpret_cast<llvm::GlobalVariable*>(
            std::get<void*>(val.internal_repr));
        code->insertGlobalVariable(gv);
    }
    std::get<2>(*constant) = val;
}
void LLVMIRGenerator::operator()(InterfaceDeclaration* decl) { debugbreak(); }
bool implementsInterfaceMethod(const FunctionSignature& cls,
                               const FunctionSignature& interface) {
    ...
    for (size_t i = 0; i < cls.parameters.size(); i++) {
        ...
        if (!cls_param.type.is_equal(intf_param.type))
            if (cls_param.name != "this" || intf_param.name != "this")
                return false;
    }
    return true;
}
...
void LLVMIRGenerator::operator()(UnionDeclaration* decl) {
    ...
    auto llvm_t = llvm::StructType::get(
        context,
        {llvm::ArrayType::get(llvm::Type::getInt8Ty(context), biggest_size),
         llvm::Type::getInt32Ty(context)});
    module->unions[curr_hash].emplace_back(
        std::unique_ptr<UnionDeclaration>{decl}, llvm_t);
    auto old_in_class = in_class;
    in_class = true;
    auto old_this = std::move(this_t);
...
std::optional<Type> LLVMIRGenerator::getVariableType(const std::string& name,
                                                     Expression* expr) {
    for (size_t i = variables.size(); i > 0; --i) {
        ...
        if (auto var = variables[idx].find(name); var != variables[idx].end()) {
            ...
            bool is_last_use =
                function_cfgs.back().last_uses.at(name).contains(expr);
...
void LLVMIRGenerator::operator()(ClassDeclaration* decl) {
    ...
    reinterpret_cast<LLModule*>(module)
        ->classes_types[cls.second->second.get()] =
        hanldeClassDeclaration(decl->vars, decl->ownership, "");
    auto old_in_class = in_class;
    in_class = true;
    auto old_this = std::move(this_t);
    this_t = Type{.name = name, .subtypes = {}};
    ...
    for (auto& impl : decl->impls) {
        ...
        if (!impl.impl_for.subtypes.empty())
            pair = impl.impl_for.module->findInterface(
                impl.impl_for.block_hash,
                impl.impl_for.name +
                    IRGenerator::mangleGenericArgs(impl.impl_for.subtypes));
        else
        ...
        for (auto& mth : impl.methods) {
            auto it = std::ranges::find_if(
                interface->methods,
                [&mth](auto& method) { return method->name == mth->name; });
            if (it == interface->methods.end())
                error(
                    Error(mth.get(),
                          "Function does not exist as part of the interface"));
            else {
                ...
                in_class = true;
                if (!implementsInterfaceMethod(mth->signature,
                                               (*it)->signature))
                    error(Error(mth.get(),
                                "Provided function is not a valid "
...
void LLVMIRGenerator::operator()(VariableDeclaration* decl) {
    ...
    if (isShadowing(name)) {
        error(Error(decl, "The name '" + name + "' already exists"));
        return;
    }
    if (decl->type) decl->type->saturate(module, this);
    FunctionType type = decl->type ? decl->type.value()
                                   : std::visit(ExpressionTypeChecker{this},
                                                decl->initializer->toVariant())
                                         .value_or_error();
    if (!type.can_be_stored()) {
        error(Error(decl, "The type tp cannot be stored"));
        return;
    }
    if (type.is_non_owning(this)) {
        error(Error(decl, "Variable types must not be non-owning"));
        return;
    }
    ...
    if (!type.is_trivially_destructible(this)) {
        std::string name =
            "drop_flag_for_" + std::string{decl->identifier.text};
        drop_flag = Alloca(name, llvm::Type::getInt1Ty(context));
    }
    if (decl->initializer) {
        auto expr_type = std::visit(ExpressionTypeChecker{this, type},
                                    decl->initializer->toVariant())
                             .value_or_error();
        if (!expr_type.is_error_ty())
        ...
        if (!type.should_sret()) {
            ...
            LLVMExpressionEvaluator{this}.implicitConvert(
                decl->initializer.get(), init, expr_type, type, alloc);
        } else
            alloc = LLVMExpressionEvaluator{this}.implicitConvert(
                decl->initializer.get(), init, expr_type, type, nullptr);
        alloc->setName(decl->identifier.text);
        if (drop_flag)
            builder->CreateStore(llvm::ConstantInt::getTrue(context),
                                 drop_flag);
    } else if (drop_flag)
        builder->CreateStore(llvm::ConstantInt::getFalse(context), drop_flag);
    type.saturate(module, this);
...
llvm::Function* declareFunction(const std::string& mangled_name,
                                IRGenerator* irgen, FunctionSignature& fn_sig);
void LLVMIRGenerator::operator()(ForStatement* stat) {
    auto fn = builder->GetInsertBlock()->getParent();
    auto ty =
        std::visit(ExpressionTypeChecker{this}, stat->iterable->toVariant())
            .value_or_error();
    if (!ty.is_mutable && ty.is_lvalue)
        error(Error(stat->iterable.get(),
                    "Iterator object must be mutable or a temporary"));
    // check if it implements iterator interface
    auto value =
        std::visit(LLVMExpressionEvaluator{this}, stat->iterable->toVariant());
    if (auto cls = ty.get_decl_if_class(this)) {
        ...
        if (!next_fn)
            next_fn =
                declareFunction(fn_name, this, impl->methods[0]->signature);
        auto memory_ty =
            ToLLVMType(impl->methods[0]->signature.returnType, false);
        ...
        auto then_bb =
            llvm::BasicBlock::Create(context, "forthen", fn, returnBlock);
        auto cont_bb =
            llvm::BasicBlock::Create(context, "forcont", fn, returnBlock);
        ...
        has_next =
            builder->CreateLoad(llvm::Type::getInt1Ty(context), has_next);
        ...
        variables.back()[std::string{stat->names[0].text}] = {
            val, impl->impl_for.subtypes[0], flg};
        ...
        popScope();
        if (builder->GetInsertBlock()->back().getOpcode() !=
            llvm::Instruction::Br)
            builder->CreateBr(for_bb);
...
void LLVMIRGenerator::operator()(BreakStatement* s) {
    if (!break_to)
        error(Error(s, "'break' must be used within a loop"));
    else {
        callDestructors();
...
void LLVMIRGenerator::operator()(ContinueStatement* s) {
    if (!continue_to)
        error(Error(s, "'continue' statement must be used within a loop"));
    else {
        callDestructors();
...
void LLVMIRGenerator::operator()(WhileStatement* expr) {
    auto fn = builder->GetInsertBlock()->getParent();
    auto cond_ty =
        std::visit(ExpressionTypeChecker{this}, expr->condition->toVariant());
    if (!cond_ty) error(cond_ty.error());
    if (cond_ty && !cond_ty->is_boolean() && !cond_ty->is_error_ty()) {
        error(Error(expr->condition.get(),
                    "Condition in 'while' must evaluate to a boolean"));
        return;
    }
    validate_expression_borrows(expr->condition.get(), this);
    auto while_bb = llvm::BasicBlock::Create(context, "while", fn, returnBlock);
    auto then_bb =
        llvm::BasicBlock::Create(context, "loopthen", fn, returnBlock);
    auto cont_bb =
        llvm::BasicBlock::Create(context, "loopcont", fn, returnBlock);
    builder->CreateBr(while_bb);
    builder->SetInsertPoint(while_bb);
    auto value =
        std::visit(LLVMExpressionEvaluator{this}, expr->condition->toVariant());
...
void LLVMIRGenerator::operator()(ConditionalExtraction* stat) {
    auto tp_e =
        std::visit(ExpressionTypeChecker{this}, stat->condition->toVariant());
    auto expression =
        std::visit(LLVMExpressionEvaluator{this}, stat->condition->toVariant());
    auto tp = tp_e.value_or_error();
    if (!tp.is_optional() && !tp.is_conversion_result()) {
        error(Error(stat->condition.get(), "Expression cannot be extracted"));
        return;
    }
    ...
    if (stat->is_ref && tp.is_value_conversion_result()) {
        error(Error({}, {}, "Expanded expression cannot be borrowed", ""));
        return;
    }
    std::array<std::pair<Expression*, BorrowResult::borrow_result_t>, 1>
        borrow_res;
    borrow_res[0].first = stat->condition.get();
    borrow_res[0].second =
        tp.is_mutable
            ? std::visit(BorrowResult::LValueBorrowResult{this},
                         stat->condition->toVariant())
            : std::visit(BorrowResult{this}, stat->condition->toVariant());
    validate_borrows(borrow_res, this);

    if (isShadowing(stat->captured_name)) {
        error(Error({}, {}, "Name is already in use"));
        return;
    }
    ...
    auto is_valid =
        builder->CreateLoad(llvm::Type::getInt1Ty(context),
                            builder->CreateStructGEP(llvm_t, expression, 1));

    auto fn = builder->GetInsertBlock()->getParent();
    auto then_bb = llvm::BasicBlock::Create(context, "then", fn, returnBlock);
    llvm::BasicBlock* else_bb = nullptr;
    if (stat->else_body)
        else_bb = llvm::BasicBlock::Create(context, "else", fn, returnBlock);
    auto merge_bb =
        llvm::BasicBlock::Create(context, "ifcont", fn, returnBlock);
    builder->CreateCondBr(is_valid, then_bb, else_bb ? else_bb : merge_bb);
    builder->SetInsertPoint(then_bb);

    auto names = borrow_res[0].second | std::views::keys;
    if (stat->is_ref) {
        pushScopeWithConstLock(names.begin(), names.end());
        lifetimeExtensions[stat->captured_name] =
            std::move(borrow_res[0].second);
    } else
        pushScope();

    auto ptr =
        builder->CreateStructGEP(llvm_t, expression, 0, stat->captured_name);
    if (!tp.is_value_conversion_result()) {
        // if it's not a `ref` we clone the value
        if (!stat->is_ref) {
            if (tp.is_ref_conversion_result())
                ptr = builder->CreateLoad(llvm::PointerType::get(context, 0),
                                          ptr);
            ...
            ptr = LLVMExpressionEvaluator{this}.clone(
                stat->condition.get(), ptr, tp.subtypes[0], into);
            if (into) ptr = into;
        }
    }
    Type variable_type =
    ...
    variables.back()[stat->captured_name] = {ptr, std::move(variable_type),
                                             flg};
    current_Statement = &stat->body;
    std::visit(*this, stat->body->toVariant());
...
void IRGenerator::pushScopeWithConstLock(It begin, It end) {
    ...
    for (const std::string& str : std::ranges::subrange(begin, end)) {
        for (auto& i : variables | std::views::reverse) {
            ...
            variables.back()[str] = {std::get<0>(i.at(str)), std::move(new_tp),
                                     nullptr};
            break;
        }
    }
...
void LLVMIRGenerator::operator()(WithStatement* stat) {
    auto ty =
        std::visit(ExpressionTypeChecker{this}, stat->expression->toVariant())
            .value_or_error();
    if (!ty.is_non_owning(this)) {
        error(Error(stat->expression.get(),
                    "'with' statement can only capture non-owning types"));
        return;
    }
    if (isShadowing(stat->name)) {
        error(Error({}, {}, "Name is already in use"));
        return;
    }
    ...
    auto expr = std::visit(LLVMExpressionEvaluator{this},
                           stat->expression->toVariant());
    llvm::Value* val;
    auto names = borrow_res[0].second | std::views::keys;
    pushScopeWithConstLock(names.begin(), names.end());
    ...
    if (!ty.should_sret()) {
        ...
        LLVMExpressionEvaluator{this}.clone(stat->expression.get(), expr, ty,
                                            val);
    } else
        val = LLVMExpressionEvaluator{this}.clone(stat->expression.get(), expr,
                                                  ty);
    ...
    ty.is_lvalue = true;
    variables.back()[stat->name] = {
        val, std::move(ty),
        prepareValidDropFlagFor(this, ty)};  // TODO: capture parents
    current_Statement = &stat->body;
    std::visit(*this, stat->body->toVariant());
...
void LLVMIRGenerator::operator()(OperatorOverload*) { debugbreak(); }
...
void LLVMIRGenerator::operator()(ReturnStatement* stat) {
    for (auto i : std::views::iota(size_t{0}, variables.size()))
        callDestructors(i);
    if (stat->expression) {
        auto t = std::visit(ExpressionTypeChecker{this, return_t},
                            stat->expression->toVariant())
                     .value_or_error();
        if (!return_t.is_assignable_from(t, this)) {
            error(Error(stat, "Type is not convertible to return type"));
            return;
        }
        // `doAssign` for reference types works like c++ (dereference and
        // assign) rather than rebind the ref, because references cannot be
        // rebound, except in return statements, which is why we handle them
        // specially
        if (return_t.is_non_owning(this))
            if (!std::visit(LifetimeExceedsFunctionChecker{this},
                            stat->expression->toVariant()))
                debugbreak();
        auto value = std::visit(LLVMExpressionEvaluator{this, return_t},
                                stat->expression->toVariant());
        LLVMExpressionEvaluator{this}.implicitConvert(
            stat->expression.get(), value, t, return_t, currentReturnAddress);
        builder->CreateBr(returnBlock);
    } else
        builder->CreateBr(returnBlock);
}
...
void LLVMIRGenerator::operator()(IfStatement* stat) {
    ...
    if (!expr_type.is_boolean() && !expr_type.is_error_ty()) {
        error(Error(stat->condition.get(),
    ...
    builder->CreateCondBr(
        std::visit(LLVMExpressionEvaluator{this}, stat->condition->toVariant()),
        then_bb, else_bb ? else_bb : merge_bb);
    builder->SetInsertPoint(then_bb);
...
bool canReturn(Statement* stat) {
    if (dynamic_cast<ReturnStatement*>(stat)) return true;
    if (auto res = dynamic_cast<IfStatement*>(stat))
        return canReturn(res->then_stat.get()) ||
               (res->else_stat && canReturn(res->else_stat.get()));
...
void LLVMIRGenerator::callDestructors(size_t depth) {
    // if there's a `br` dont steal it and assume destructors have already
    // been called
    ...
    for (auto& [name, var] : *(variables.end() - depth - 1)) {
        ...
        auto drop = llvm::BasicBlock::Create(context, "drop_var" + name, fn,
                                             returnBlock);
        auto drop_cont = llvm::BasicBlock::Create(context, "drop_cont" + name,
                                                  fn, returnBlock);
        builder->CreateCondBr(
        ...
        if (!type.should_sret()) {
            ...
            if (to_drop->getName().starts_with("__del_parents"))
                memcpy(name.data(), to_drop->getName().data(),
                       16 + sizeof(void*));
            to_drop =
                builder->CreateLoad(ToLLVMType(type, false), to_drop, name);
        }
...
llvm::StructType* LLVMIRGenerator::hanldeClassDeclaration(
    std::span<const ClassVariable> vars, Ownership own, std::string_view name) {
    std::vector<std::string> var_names(vars.size());

    std::vector<llvm::Type*> args(vars.size());
    std::ranges::transform(
        vars, args.begin(), [this, own](const ClassVariable& p) {
            if (own == Ownership::Owning && p.type.is_non_owning(this))
                error(Error({}, {}, "Non owning type in owning class"));
            if (own == Ownership::NonOwning && p.type.is_non_owning_mut(this))
                error(Error({}, {},
                            "Mutably non owning type in non owning class"));
...
bool LLVMIRGenerator::GenerateIR(
    std::string_view name, std::vector<std::unique_ptr<Statement>> statements,
    LLModule* md, Engine* eng) {
    block_hash = md->module_hash;
    md->code = llvm::orc::ThreadSafeModule(
        std::make_unique<llvm::Module>(name, context), eng->llvm_context);
    module = md;
