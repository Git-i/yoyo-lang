...
namespace Yoyo {
llvm::Type* LLModule::ToLLVMType(const Type& type, const std::string& hash,
                                 IRGenerator* irgen,
                                 const std::vector<Type>& disallowed_types) {
    auto& context =
        *reinterpret_cast<LLVMEngine*>(engine)->llvm_context.getContext();
    if (type.is_integral())
        return llvm::Type::getIntNTy(context, *type.integer_width());
    if (type.is_floating_point())
        return *type.float_width() == 32 ? llvm::Type::getFloatTy(context)
                                         : llvm::Type::getDoubleTy(context);
    if (type.is_boolean()) return llvm::Type::getInt1Ty(context);
    if (type.name == "void") return llvm::Type::getVoidTy(context);
    if (type.is_opaque_pointer() || type.is_reference())
        return llvm::PointerType::get(context, 0);
    if (type.is_char()) return llvm::Type::getInt32Ty(context);
    ...
    if (type.is_tuple()) {
        ...
        for (auto& subtype : type.subtypes) {
            auto ty = reinterpret_cast<LLModule*>(subtype.module)
                          ->ToLLVMType(subtype, hash, irgen, disallowed_types);
    ...
    if (type.is_lambda()) {
        return reinterpret_cast<LLModule*>(type.module)
            ->lambdas.at(type.name)
            .first;
    }
    if (type.is_optional()) {
        std::array<llvm::Type*, 2> args{};
        args[0] =
            reinterpret_cast<LLModule*>(type.subtypes[0].module)
                ->ToLLVMType(type.subtypes[0], hash, irgen, disallowed_types);
    ...
    if (type.is_str()) {
        ...
        for (auto& sub_t : std::ranges::subrange(args.begin() + 1, args.end()))
            sub_t = llvm::Type::getInt64Ty(context);
        return llvm::StructType::get(context, args);
    }
    if (type.is_variant()) {
        std::array<llvm::Type*, 2> args{};
        ...
        for (auto& subtype : type.subtypes) {
            auto sub_t =
                reinterpret_cast<LLModule*>(subtype.module)
                    ->ToLLVMType(subtype, hash, irgen, disallowed_types);
    ...
    if (auto [blk, unn] = findUnionWithType(type.block_hash, type.name);
        unn.second) {
        return unn.second;
    }
    ...
    if (type.is_view()) {
        ...
        if (intf) {
            ...
            std::vector<llvm::Type*> subtypes(
                intf->methods.size() + 1, llvm::PointerType::get(context, 0));
            return llvm::StructType::get(context, subtypes);
        }
        return nullptr;
    }
    if (type.is_lambda()) return nullptr;
    if (auto t = findClassWithType(type.block_hash, type.name);
        std::get<2>(t.second)) {
        auto ptr = std::get<1>(t.second);
        if (ptr) return ptr;
        // class is not yet defined but is recursive
        if (auto find_it = std::ranges::find(disallowed_types, type);
            find_it != disallowed_types.end()) {
        ...
        for (auto& subvar : decl->vars) {
            ...
            auto ty = reinterpret_cast<LLModule*>(subtype.module)
                          ->ToLLVMType(subtype, hash, irgen, not_allowed);
            if (!ty) return nullptr;
            args.push_back(ty);
        }
        return llvm::StructType::get(context, args);
...
void addTrigFunctions(Module* md, std::span<Type> types,
                      llvm::IRBuilder<>& bld);
void addFloatConsts(Module* md, std::span<Type> types);
void addRangeIteratorMethods(Module* md, std::span<Type> types,
                             llvm::IRBuilder<>& bld);
void LLModule::makeBuiltinModule(Engine* engi) {
    ...
    auto cmp_eq =
        std::make_unique<EnumDeclaration>("CmpEq",
                                          decltype(EnumDeclaration::values){
                                              {"Eq", eq},
                                              {"Ne", ne}
    },
                                          decltype(EnumDeclaration::stats){});
    auto cmp_ord = std::make_unique<EnumDeclaration>(
        "CmpOrd",
        decltype(EnumDeclaration::values){
            {     "Eq",      eq},
            {   "Less",    less},
        ...
        decltype(EnumDeclaration::stats){});
    auto cmp_pord = std::make_unique<EnumDeclaration>(
        "CmpPartOrd",
        decltype(EnumDeclaration::values){
            {     "Eq",      eq},
            {   "Less",    less},
    ...
    module->code = llvm::orc::ThreadSafeModule(
        std::make_unique<llvm::Module>("__builtin", ctx), eng->llvm_context);
    llvm::IRBuilder<> builder(ctx);
    for (auto& t : types) {
        auto as_llvm = module->ToLLVMType(t, "", nullptr, {});
        auto fn_ty =
            llvm::FunctionType::get(as_llvm, {as_llvm, as_llvm}, false);
        ...
        auto plus_fn = llvm::Function::Create(
            fn_ty, llvm::GlobalValue::ExternalLinkage, mangled_name_for("plus"),
            module->code.getModuleUnlocked());
        auto minus_fn = llvm::Function::Create(
            fn_ty, llvm::GlobalValue::ExternalLinkage,
            mangled_name_for("minus"), module->code.getModuleUnlocked());
        auto mul_fn = llvm::Function::Create(
            fn_ty, llvm::GlobalValue::ExternalLinkage, mangled_name_for("mul"),
            module->code.getModuleUnlocked());
        auto div_fn = llvm::Function::Create(
            fn_ty, llvm::GlobalValue::ExternalLinkage, mangled_name_for("div"),
            module->code.getModuleUnlocked());
        ...
        if (t.is_integral())
            builder.CreateRet(
                builder.CreateAdd(plus_fn->getArg(0), plus_fn->getArg(1)));
        else
            builder.CreateRet(
                builder.CreateFAdd(plus_fn->getArg(0), plus_fn->getArg(1)));
        operators.add_binary_detail_for(TokenType::Plus, t, t, t);

        builder.SetInsertPoint(
            llvm::BasicBlock::Create(ctx, "entry", minus_fn));
        if (t.is_integral())
            builder.CreateRet(
                builder.CreateSub(minus_fn->getArg(0), minus_fn->getArg(1)));
        else
            builder.CreateRet(
                builder.CreateFSub(minus_fn->getArg(0), minus_fn->getArg(1)));
        operators.add_binary_detail_for(TokenType::Minus, t, t, t);

        builder.SetInsertPoint(llvm::BasicBlock::Create(ctx, "entry", mul_fn));
        if (t.is_integral())
            builder.CreateRet(
                builder.CreateMul(mul_fn->getArg(0), mul_fn->getArg(1)));
        else
            builder.CreateRet(
                builder.CreateFMul(mul_fn->getArg(0), mul_fn->getArg(1)));
        ...
        if (t.is_signed_integral())
            builder.CreateRet(
                builder.CreateSDiv(div_fn->getArg(0), div_fn->getArg(1)));
        else if (t.is_unsigned_integral())
            builder.CreateRet(
                builder.CreateUDiv(div_fn->getArg(0), div_fn->getArg(1)));
        else
            builder.CreateRet(
                builder.CreateFDiv(div_fn->getArg(0), div_fn->getArg(1)));
        operators.add_binary_detail_for(TokenType::Slash, t, t, t);
    }
    // comparison operators
    for (auto& t :
         types | std::views::filter([](auto& t) { return t.is_integral(); })) {
        // integer comparison is totally ordered
        Type result{.name = "CmpOrd",
                    .module = module,
        ...
        auto cmp_fn = llvm::Function::Create(
            fn_ty, llvm::GlobalValue::ExternalLinkage, mangled_name,
            module->code.getModuleUnlocked());
        ...
        llvm::Value* is_gt = t.is_signed_integral()
                                 ? builder.CreateICmpSGT(arg0, arg1)
                                 : builder.CreateICmpUGT(arg0, arg1);
        ...
        operators.add_binary_detail_for(TokenType::Spaceship, t, t,
                                        std::move(result));
    }
    for (auto& t : types | std::views::filter(
                               [](auto& t) { return t.is_floating_point(); })) {
        // float comparison is partially ordered because of NaN
        Type result{.name = "CmpPartOrd",
                    .module = module,
                    .block_hash = module->module_hash};
        std::string mangled_name = "__operator__cmp__" + t.name + "__" + t.name;
        auto as_llvm = module->ToLLVMType(t, "", nullptr, {});
    ...
    for (auto& t : std::ranges::subrange(types.begin(), types.begin() + 6)) {
        ...
        auto fn = llvm::Function::Create(
            fn_ty, llvm::GlobalValue::ExternalLinkage,
            mangled_name_for("un_neg"), module->code.getModuleUnlocked());
        builder.SetInsertPoint(llvm::BasicBlock::Create(ctx, "entry", fn));
        if (t.is_integral())
            builder.CreateRet(builder.CreateNeg(fn->getArg(0)));
        else
            builder.CreateRet(builder.CreateFNeg(fn->getArg(0)));
    ...
    for (auto& t : std::ranges::subrange(types.begin() + 2, types.end())) {
        ...
        auto mod_fn = llvm::Function::Create(
            fn_ty, llvm::GlobalValue::ExternalLinkage, mangled_name_for("mod"),
            module->code.getModuleUnlocked());
        builder.SetInsertPoint(llvm::BasicBlock::Create(ctx, "entry", mod_fn));
        if (t.is_signed_integral())
            builder.CreateRet(
                builder.CreateSRem(mod_fn->getArg(0), mod_fn->getArg(1)));
        else
            builder.CreateRet(
                builder.CreateURem(mod_fn->getArg(0), mod_fn->getArg(1)));
        operators.add_binary_detail_for(TokenType::Percent, t, t, t);

        auto sh_fn_ty =
            llvm::FunctionType::get(as_llvm, {as_llvm, as_llvm}, false);
        auto rsh_fn = llvm::Function::Create(
            sh_fn_ty, llvm::GlobalValue::ExternalLinkage,
            mangled_name_for("shr"), module->code.getModuleUnlocked());
        builder.SetInsertPoint(llvm::BasicBlock::Create(ctx, "entry", rsh_fn));
        builder.CreateRet(
            builder.CreateLShr(rsh_fn->getArg(0), rsh_fn->getArg(1)));
        operators.add_binary_detail_for(TokenType::DoubleGreater, t, t, t);

        auto lsh_fn = llvm::Function::Create(
            sh_fn_ty, llvm::GlobalValue::ExternalLinkage,
            mangled_name_for("shl"), module->code.getModuleUnlocked());
        builder.SetInsertPoint(llvm::BasicBlock::Create(ctx, "entry", lsh_fn));
        builder.CreateRet(
            builder.CreateShl(lsh_fn->getArg(0), lsh_fn->getArg(1)));
    ...
    sig.parameters.emplace_back(FunctionParameter{
        .type = Type{"__ref_mut", {Type{"This"}}},
          .name = "this"
    });
    iterator->clause = GenericClause{{"OutputTy"}};
    iterator->name = "Iterator";
    iterator->methods.emplace_back(
        std::make_unique<FunctionDeclaration>("next", std::move(sig), nullptr));
    module->generic_interfaces[module->module_hash].emplace_back(
        std::move(iterator));
    addTrigFunctions(module, std::span{types.begin(), types.begin() + 2},
                     builder);
    addFloatConsts(module, std::span{types.begin(), types.begin() + 2});
    addRangeIteratorMethods(module, std::span{types.begin() + 2, types.end()},
                            builder);
    ...
    module->functions["str::"].emplace_back(ModuleBase::FunctionDetails{
        ...
        .sig =
            FunctionSignature{
                    ...
                    .type = Type{.name = "__ref",
                                 .subtypes = {Type{.name = "str",
                                                   .module = module}},
                                 .module = module},
                    .name = "this"}}},
    ...
    auto c_str_fn =
        llvm::Function::Create(c_str_sig, llvm::GlobalValue::ExternalLinkage,
                               "str::c_str", module->code.getModuleUnlocked());
    builder.SetInsertPoint(llvm::BasicBlock::Create(ctx, "entry", c_str_fn));
    builder.CreateRet(builder.CreateLoad(
        ptr_ty, builder.CreateStructGEP(string_ty, c_str_fn->getArg(0), 0)));
}
void addFloatConsts(LLModule* md, std::span<Type> types) {
    ...
    for (auto& type : types) {
        ...
        md->constants[type.name + "::"].emplace_back(
            type, "QNAN", llvm::ConstantFP::getQNaN(as_llvm));
        md->constants[type.name + "::"].emplace_back(
            type, "SNAN", llvm::ConstantFP::getSNaN(as_llvm));
        md->constants[type.name + "::"].emplace_back(
            type, "PI", llvm::ConstantFP::get(as_llvm, std::numbers::pi));
        md->constants[type.name + "::"].emplace_back(
            type, "INV_PI",
            llvm::ConstantFP::get(as_llvm, std::numbers::inv_pi));
        md->constants[type.name + "::"].emplace_back(
            type, "E", llvm::ConstantFP::get(as_llvm, std::numbers::e));
        md->constants[type.name + "::"].emplace_back(
            type, "LN2", llvm::ConstantFP::get(as_llvm, std::numbers::ln2));
        md->constants[type.name + "::"].emplace_back(
            type, "LN10", llvm::ConstantFP::get(as_llvm, std::numbers::ln10));
        md->constants[type.name + "::"].emplace_back(
            type, "SQRT2", llvm::ConstantFP::get(as_llvm, std::numbers::sqrt2));
        md->constants[type.name + "::"].emplace_back(
            type, "SQRT3", llvm::ConstantFP::get(as_llvm, std::numbers::sqrt3));
    }
}
// integer types only
void addRangeIteratorMethods(LLModule* md, std::span<Type> types,
                             llvm::IRBuilder<>& bld) {
    ...
    for (auto& type : types) {
        ...
        auto range_as_llvm =
            llvm::StructType::get(ctx, {subtype_llvm, subtype_llvm}, false);
        auto range_ty = Type{
            .name = "range_" + type.name, .module = md, .block_hash = "core::"};

        std::vector<std::unique_ptr<FunctionDeclaration>> methods;
        methods.emplace_back(std::make_unique<FunctionDeclaration>(
            "next",
            FunctionSignature{
                .returnType =
                    Type{.name = "__opt", .subtypes = {type}, .module = md},
                .parameters = {FunctionParameter{
                    .type = Type{.name = "__ref_mut", .subtypes = {range_ty}},
                    .name = "this"}}
        },
            nullptr));
        std::vector<InterfaceImplementation> impl;
        impl.emplace_back(Type{.name = "Iterator",
                               .subtypes = {type},
                               .module = md,
                               .block_hash = "core::"},
                          SourceSpan{}, std::move(methods));
        auto decl_ptr = std::make_unique<ClassDeclaration>(
            Token{.text = range_ty.name}, std::vector<ClassVariable>{},
            std::vector<std::unique_ptr<Statement>>{}, Ownership::Owning,
            std::move(impl));

        md->classes["core::"].emplace_back("core::range_" + type.name + "::",
                                           range_as_llvm, std::move(decl_ptr));

        auto ptr_ty = llvm::PointerType::get(bld.getContext(), 0);
        // for the .next() it translates to fn(:&mut range_ty) -> ty?
        auto iter_sig = llvm::FunctionType::get(
            llvm::Type::getVoidTy(bld.getContext()), {ptr_ty, ptr_ty}, false);

        auto iter_fn = llvm::Function::Create(
            iter_sig, llvm::GlobalValue::ExternalLinkage,
            "core::range_" + type.name + "::core::Iterator::<" + type.name +
                ">::next",
            md->code.getModuleUnlocked());

        iter_fn->addAttributeAtIndex(
            1, llvm::Attribute::get(ctx, llvm::Attribute::StructRet,
                                    return_as_llvm));

        bld.SetInsertPoint(llvm::BasicBlock::Create(ctx, "entry", iter_fn));
        auto start_ptr =
            bld.CreateStructGEP(range_as_llvm, iter_fn->getArg(1), 0);
        auto end_val = bld.CreateLoad(
            subtype_llvm,
            bld.CreateStructGEP(range_as_llvm, iter_fn->getArg(1), 1));
        auto old_start_val = bld.CreateLoad(subtype_llvm, start_ptr);
        auto start_val = bld.CreateAdd(old_start_val,
                                       llvm::ConstantInt::get(subtype_llvm, 1));
        bld.CreateStore(start_val, start_ptr);
        llvm::Value* cmp_res;
        if (type.is_signed_integral())
            cmp_res = bld.CreateICmpSLT(old_start_val, end_val);
        else
            cmp_res = bld.CreateICmpULT(old_start_val, end_val);
        bld.CreateStore(cmp_res, bld.CreateStructGEP(return_as_llvm,
                                                     iter_fn->getArg(0), 1));
        bld.CreateStore(
            old_start_val,
            bld.CreateStructGEP(return_as_llvm, iter_fn->getArg(0), 0));
        bld.CreateRetVoid();
    }
}
void addTrigFunctions(LLModule* md, std::span<Type> types,
                      llvm::IRBuilder<>& bld) {
    auto& ctx = *md->code.getContext().getContext();
    for (auto& type : types) {
        FunctionSignature sig;
        ...
        ModuleBase::FunctionDetails sdets{
            .name = "sin",
            .sig = sig,
            .attributes = std::vector{Attribute{"public"}}};
        ModuleBase::FunctionDetails cdets{
            .name = "cos",
            .sig = sig,
            .attributes = std::vector{Attribute{"public"}}};
        ModuleBase::FunctionDetails tdets{
            .name = "tan",
            .sig = sig,
            .attributes = std::vector{Attribute{"public"}}};
        ...
        auto sin_func = llvm::Function::Create(
            f_ty, llvm::GlobalValue::ExternalLinkage, type.name + "::" + "sin",
            md->code.getModuleUnlocked());
        bld.SetInsertPoint(llvm::BasicBlock::Create(ctx, "entry", sin_func));
        bld.CreateRet(bld.CreateIntrinsic(as_llvm, llvm::Intrinsic::sin,
                                          {sin_func->getArg(0)}));

        auto cos_func = llvm::Function::Create(
            f_ty, llvm::GlobalValue::ExternalLinkage, type.name + "::" + "cos",
            md->code.getModuleUnlocked());
        bld.SetInsertPoint(llvm::BasicBlock::Create(ctx, "entry", cos_func));
        bld.CreateRet(bld.CreateIntrinsic(as_llvm, llvm::Intrinsic::cos,
                                          {cos_func->getArg(0)}));

        // auto tan_func = llvm::Function::Create(f_ty,
        // llvm::GlobalValue::ExternalLinkage, type.name + "::" + "tan",
        // md->code.getModuleUnlocked());
        // bld.SetInsertPoint(llvm::BasicBlock::Create(ctx, "entry",
        // tan_func)); bld.CreateRet(bld.CreateIntrinsic(as_llvm,
        // llvm::Intrinsic::tan, { tan_func->getArg(0) }));
    }
}
...
std::pair<std::string, std::pair<UnionDeclaration*, llvm::StructType*>>
    LLModule::findUnionWithType(const std::string& block,
    ...
    for (auto& [hash, unn_list] : unions) {
        ...
        for (auto& unn : unn_list)
            if (unn->name == name)
                return {
                    hash, {unn.get(), union_types[unn.get()]}
                };
    }
    return {
        "", {nullptr, nullptr}
