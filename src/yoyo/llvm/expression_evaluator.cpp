...
void stealUsages(std::span<llvm::Value*> args, llvm::Value* out) {
    ...
    for (auto value : args) {
        if (value->getName().starts_with("__del_parents___")) {
            ...
            auto& lifetimes =
                **reinterpret_cast<ExtendedLifetimes* const*>(nm.data() + 16);
            ext->objects.insert(
                ext->objects.end(),
...
void clearUsages(std::vector<llvm::Value*>& args,
                 const LLVMExpressionEvaluator& eval) {
    for (auto value : args) {
        if (!value) continue;
        if (value->getName().starts_with("__del_parents___")) {
            auto nm = value->getName();
            auto& lifetimes =
                **reinterpret_cast<ExtendedLifetimes* const*>(nm.data() + 16);
            for (auto& [type, obj] : lifetimes.objects) eval.destroy(obj, type);
            delete &lifetimes;
        }
    }
}
llvm::Function* declareFunction(const std::string& mangled_name,
                                LLVMIRGenerator* irgen,
                                FunctionSignature& fn_sig) {
    llvm::Function* fn = nullptr;
    irgen->saturateSignature(fn_sig, irgen->module);
    fn = llvm::Function::Create(irgen->ToLLVMSignature(fn_sig),
    ...
    if (fn_sig.returnType.should_sret()) {
        ...
        fn->addAttributeAtIndex(
            1, llvm::Attribute::get(irgen->context, llvm::Attribute::StructRet,
                                    return_as_llvm));
    }
    return fn;
}
...
llvm::Value* LLVMExpressionEvaluator::implicitConvert(Expression* xp,
                                                      llvm::Value* val,
                                                      const Type& src,
                                                      const Type& dst,
                                                      llvm::Value* out) const {
    llvm::Type* int_ty = llvm::Type::getInt32Ty(irgen->context);
    if (dst.is_equal(src)) {
        return clone(xp, val, src, out);
    }
    if (!dst.is_assignable_from(src, irgen)) {
        irgen->error(Error(
            xp,
            std::format("Expression of type {} cannot be converted to type {}",
                        src.pretty_name(irgen->block_hash),
                        dst.pretty_name(irgen->block_hash))));
        return nullptr;
    }
    ...
    if (src.name == "ilit") {
        ...
        if (dst.is_integral()) {
            if (as_int->isNegative()) {
                if (as_int->getSExtValue() >= min_bound &&
                    as_int->getSExtValue() <= max_bound) {
                    val = irgen->builder->CreateSExtOrTrunc(val, dst_as_llvm);
                    irgen->builder->CreateStore(val, out);
                    return val;
                }
                irgen->error(
                    Error(xp, "Literal out of bounds for destination type"));
                return nullptr;
            }
            ...
            irgen->error(
                Error(xp, "Literal out of bound for destination type"));
            return nullptr;
        }
    ...
    if (dst.is_signed_integral()) {
        if (!dst.is_equal(src))
            if (src.is_signed_integral())
                val =
                    irgen->builder->CreateSExt(val, dst_as_llvm, "assign_sext");
            else if (src.is_unsigned_integral())
                val =
                    irgen->builder->CreateZExt(val, dst_as_llvm, "assign_zext");
        irgen->builder->CreateStore(val, out);
        return val;
    }
    if (dst.is_floating_point()) {
        if (!dst.is_equal(src))
            if (src.is_floating_point())
                val = irgen->builder->CreateFPExt(val, dst_as_llvm,
                                                  "assign_fpext");
            else if (src.is_unsigned_integral())
                val = irgen->builder->CreateUIToFP(val, dst_as_llvm,
                                                   "assign_uitofp");
    ...
    if (src.is_gc_reference()) {
        std::string error_str =
            std::format("Borrow check failed at {}:{}:{}",
                        irgen->view->filename, xp->beg.line, xp->beg.column);
        ...
        auto gcref_subtype = llvm::StructType::get(
            irgen->context, {irgen->ToLLVMType(src.deref(), false), i64_ty},
            false);
        auto gc_borrow_count_ptr =
            irgen->builder->CreateStructGEP(gcref_subtype, val, 1);
        auto gc_object_ptr =
            irgen->builder->CreateStructGEP(gcref_subtype, val, 0);
        auto gc_borrow_count =
            irgen->builder->CreateLoad(i64_ty, gc_borrow_count_ptr);
        ...
        auto invalid_borrow = llvm::BasicBlock::Create(
            irgen->context, "borrow_check_fail", fn, irgen->returnBlock);
        auto valid_borrow = llvm::BasicBlock::Create(
            irgen->context, "borrow_check_pass", fn, irgen->returnBlock);
        if (dst.is_mutable_reference()) {
            type_name += "_mut";
            irgen->builder->CreateCondBr(
                irgen->builder->CreateICmpNE(const_zero, gc_borrow_count),
                invalid_borrow, valid_borrow);
            new_count = const_min_1;
        } else {
            new_count = irgen->builder->CreateAdd(
                gc_borrow_count, llvm::ConstantInt::get(i64_ty, 1));
            irgen->builder->CreateCondBr(
                irgen->builder->CreateICmpEQ(const_min_1, gc_borrow_count),
                invalid_borrow, valid_borrow);
        }
    ...
    if (dst.is_variant()) {
        ...
        for (auto& sub : subtypes) {
            ...
            implicitConvert(
                ...
                irgen->builder->CreateStructGEP(dst_as_llvm, out, 0));
            irgen->builder->CreateStore(
                llvm::ConstantInt::get(llvm::Type::getInt32Ty(irgen->context),
                                       i),
                type_idx_ptr);
            break;
        }
    ...
    if (dst.name == "__called_fn") {
        if (src.is_function()) {
            irgen->builder->CreateStore(
                ...
                irgen->builder->CreateStructGEP(dst_as_llvm, out, 0));
            irgen->builder->CreateStore(
                val, irgen->builder->CreateStructGEP(dst_as_llvm, out, 1));
            return out;
        }
        if (src.is_lambda()) {
            irgen->builder->CreateStore(
                val, irgen->builder->CreateStructGEP(dst_as_llvm, out, 0));
            irgen->builder->CreateStore(
                irgen->code->getFunction(src.module->module_hash + src.name),
                irgen->builder->CreateStructGEP(dst_as_llvm, out, 1));
            return out;
        }
    }
    if (dst.is_view()) {
        if (src.is_view()) {
            ...
            irgen->builder->CreateMemCpy(out, std::nullopt, val, std::nullopt,
                                         size);
            return out;
        }
        auto& viewed = dst.subtypes[0];
        auto [name, intf] =
            viewed.module->findInterface(viewed.block_hash, viewed.name);
        std::string full_name = name + intf->name;
        if (auto cls = src.deref().get_decl_if_class(irgen)) {
            auto details =
                src.deref()
                    .module->findClass(src.deref().block_hash, src.deref().name)
                    .second;
            auto this_ptr =
                irgen->builder->CreateStructGEP(dst_as_llvm, out, 0);
            ...
            for (auto& method : intf->methods) {
                idx++;
                auto fn_ptr =
                    irgen->builder->CreateStructGEP(dst_as_llvm, out, idx);
                std::string method_name =
                    std::get<0>(*details) + full_name + "::" + method->name;
...
/// The @c clone function's signature looks like @code clone: (&this) ->
/// This @endcode \n Without it the type is cannot be copied I think it
/// should be implicit for union types
llvm::Value* LLVMExpressionEvaluator::clone(Expression* xp, llvm::Value* value,
                                            const Type& left_type,
                                            llvm::Value* into) const {
    ...
    if (!left_type.is_lvalue) {
        ...
        irgen->builder->CreateMemCpy(into, std::nullopt, value, std::nullopt,
                                     size);
        return into;
    }
    ...
    if (left_type.is_tuple()) {
        ...
        for (auto& sub : left_type.subtypes) {
            ...
            if (!sub.should_sret())
                val = irgen->builder->CreateLoad(sub_as_llvm, val);
        ...
        // opt is {data, bool}
        auto has_value = irgen->builder->CreateLoad(
            llvm::Type::getInt1Ty(irgen->context),
            irgen->builder->CreateStructGEP(opt_ty, value, 1));
        auto fn = irgen->builder->GetInsertBlock()->getParent();
        auto is_valid = llvm::BasicBlock::Create(
            irgen->context, "opt_valid_assign", fn, irgen->returnBlock);
        auto opt_assign_cont = llvm::BasicBlock::Create(
            irgen->context, "opt_assign_cont", fn, irgen->returnBlock);
        ...
        if (!left_type.subtypes[0].should_sret()) {
            auto subtype =
                llvm::dyn_cast<llvm::StructType>(opt_ty)->getElementType(0);
            sub_value = irgen->builder->CreateLoad(subtype, sub_value);
        }
        clone(xp, sub_value, left_type.subtypes[0],
              irgen->builder->CreateStructGEP(opt_ty, into, 0));
        irgen->builder->CreateBr(opt_assign_cont);

        irgen->builder->SetInsertPoint(opt_assign_cont);
        irgen->builder->CreateStore(
            has_value, irgen->builder->CreateStructGEP(opt_ty, into, 1));
    } else if (left_type.is_str()) {
        auto size = irgen->builder->CreateStructGEP(as_llvm, value, 1);
        size = irgen->builder->CreateLoad(
            llvm::Type::getInt64Ty(irgen->context), size);
        auto mem = irgen->builder->CreateStructGEP(as_llvm, value, 0);
        mem = irgen->builder->CreateLoad(
            llvm::PointerType::get(irgen->context, 0), mem);
        ...
        irgen->builder->CreateMemCpy(ptr, std::nullopt, mem, std::nullopt,
                                     size);
        ...
        llvm::BasicBlock* def = llvm::BasicBlock::Create(
            irgen->context, "variant_default", fn, irgen->returnBlock);
        auto sw = irgen->builder->CreateSwitch(type_idx, def,
                                               left_type.subtypes.size());
        uint32_t idx = 0;
        for (auto& sub : subtypes) {
            llvm::BasicBlock* blk =
                llvm::BasicBlock::Create(irgen->context, sub.name, fn, def);
            sw->addCase(llvm::ConstantInt::get(
                            llvm::Type::getInt32Ty(irgen->context), idx),
                        blk);
            ...
            if (!sub.should_sret())
                sub_value = irgen->builder->CreateLoad(sub_as_llvm, sub_value);

            clone(xp, sub_value, sub,
                  irgen->builder->CreateStructGEP(llvm_ty, into, 1));
    ...
    // class types can define custom clone
    else if (auto decl_tup =
                 left_type.module
                     ->findClass(left_type.block_hash, left_type.name)
                     .second) {
        constexpr auto asdf = std::is_same_v<decltype(std::get<1>(*decl_tup)),
                                             decltype(std::get<10>(*decl_tup))>;
        auto decl = std::get<2>(*decl_tup).get();
        if (!decl->has_clone) {
            irgen->error(Error(xp, "Expression cannot be cloned"));
            return nullptr;
        }
        ...
        if (candidate != decl->stats.end() &&
            dynamic_cast<FunctionDeclaration*>(candidate->get())) {
            auto decl =
                reinterpret_cast<FunctionDeclaration*>(candidate->get());
            ...
            for (auto& var : decl->vars) {
                auto sub_val_ptr =
                    irgen->builder->CreateStructGEP(as_llvm, value, idx);
                auto sub_into_ptr =
                    irgen->builder->CreateStructGEP(as_llvm, into, idx);
                if (!var.type.should_sret())
                    sub_val_ptr = irgen->builder->CreateLoad(
                        irgen->ToLLVMType(var.type, false), sub_val_ptr);
...
void destroyNonOwningParents(const LLVMExpressionEvaluator& eval,
                             llvm::Value* value, const Type& tp) {
    if (value->getName().starts_with("__del_parents___")) {
        auto nm = value->getName();
        auto& lifetimes =
            **reinterpret_cast<ExtendedLifetimes* const*>(nm.data() + 16);
...
extern "C" void string_destroy_debug() { printf("string destroyed\n"); }
llvm::Function* getStringDestroy(LLVMIRGenerator* irgen) {
    auto fn = irgen->code->getFunction("string_destroy_debug");
    if (fn) return fn;
    auto ty =
        llvm::FunctionType::get(llvm::Type::getVoidTy(irgen->context), false);
    return llvm::Function::Create(ty, llvm::GlobalValue::ExternalLinkage,
                                  "string_destroy_debug", irgen->code);
}
void LLVMExpressionEvaluator::destroy(llvm::Value* value,
    ...
    if (type.is_optional()) {
        ...
        auto should_destroy = llvm::BasicBlock::Create(
            irgen->context, "destroy_opt", parent, irgen->returnBlock);
        auto destroy_cont = llvm::BasicBlock::Create(
            irgen->context, "destroy_cont", parent, should_destroy);
        irgen->builder->CreateCondBr(
            irgen->builder->CreateLoad(llvm::Type::getInt1Ty(irgen->context),
                                       is_valid),
            should_destroy, destroy_cont);
        ...
        irgen->builder->CreateStore(
            irgen->builder->CreateSub(new_val,
                                      llvm::ConstantInt::get(i64_ty, 1)),
            value);
    } else if (type.name == "__gc_refcell_borrow_mut") {
        irgen->builder->CreateStore(
            llvm::ConstantInt::get(llvm::Type::getInt64Ty(irgen->context), 0),
            value);
        ...
        llvm::BasicBlock* def = llvm::BasicBlock::Create(
            irgen->context, "variant_default", fn, irgen->returnBlock);
        auto sw =
            irgen->builder->CreateSwitch(type_idx, def, type.subtypes.size());
        uint32_t idx = 0;
        for (auto& sub : subtypes) {
            llvm::BasicBlock* blk =
                llvm::BasicBlock::Create(irgen->context, sub.name, fn, def);
        ...
        irgen->Free(irgen->builder->CreateLoad(
            llvm::PointerType::get(irgen->context, 0), mem_ptr));
        // irgen->builder->CreateCall(getStringDestroy(irgen));
    } else if (auto dets =
                   type.module->findClass(type.block_hash, type.name).second) {
        auto decl = std::get<2>(*dets).get();
        if (!decl->destructor_name.empty()) {
            auto fn = irgen->code->getFunction(std::get<0>(*dets) +
                                               decl->destructor_name);
            irgen->builder->CreateCall(fn, {value});
        }
        // even after calling the destructor, we still do member wise
        // destruction
        size_t idx = 0;
        for (auto& var : decl->vars) {
            auto sub = irgen->builder->CreateStructGEP(as_llvm, value, idx);
    ...
    if (type.is_non_owning(irgen)) destroyNonOwningParents(*this, value, type);
}

llvm::Value* LLVMExpressionEvaluator::doDot(Expression* lhs, Expression* rhs,
                                            const Type& left_type,
                                            bool load_primitive) {
    ...
    if (left_type.deref().is_tuple()) {
        if (auto idx = dynamic_cast<IntegerLiteral*>(rhs)) {
            ...
            auto ptr =
                irgen->builder->CreateStructGEP(llvm_t, left_ptr, idx_int);
            if (load_primitive && !out_type.should_sret())
                return irgen->builder->CreateLoad(
                    irgen->ToLLVMType(out_type, false), ptr);
            // no partial move allowed, so rvalue binary operations clone
            // the field and destroy the object
            if (!left_type.is_lvalue) {
    ...
    if (auto cls = left_type.deref().get_decl_if_class(irgen)) {
        if (auto* name_expr = dynamic_cast<NameExpression*>(rhs)) {
            ...
            if (auto var = std::ranges::find_if(
                    cls->vars,
                    [&name](ClassVariable& v) { return name == v.name; });
                var != cls->vars.end()) {
                auto llvm_t = irgen->ToLLVMType(left_type.deref(), false);
                ...
                ptr = irgen->builder->CreateStructGEP(
                    llvm_t, lalloc, std::distance(cls->vars.begin(), var));
 ...
 * flit op flit = flit
 */
void convertLiterals(llvm::Value** lhs, llvm::Value** rhs,
    ...
    if (left_type.name == "ilit") {
        ...
        if (right_type.name == "filt") {
            if (as_int->isNegative())
                *lhs = irgen->builder->CreateSIToFP(
                    *lhs, llvm::Type::getDoubleTy(irgen->context));
            else
                *lhs = irgen->builder->CreateUIToFP(
                    *lhs, llvm::Type::getDoubleTy(irgen->context));
            return;
        }
...
llvm::FunctionType* BinOverloadToLLVMSig(LLVMIRGenerator* irgen,
                                         OverloadDetailsBinary* bin) {
    ...
    args.push_back(
        bin->left.should_sret() ? ptr_ty : irgen->ToLLVMType(bin->left, false));
    args.push_back(bin->right.should_sret()
                       ? ptr_ty
                       : irgen->ToLLVMType(bin->right, false));
    return llvm::FunctionType::get(res, args, false);
}
llvm::Function* getOperatorFunction(TokenType t, LLVMIRGenerator* irgen,
                                    OverloadDetailsBinary* target) {
    auto fn_name = target->mangled_name(t);
    auto fn = irgen->code->getFunction(fn_name);
    if (!fn) {
        fn = llvm::Function::Create(BinOverloadToLLVMSig(irgen, target),
                                    llvm::GlobalValue::ExternalLinkage, fn_name,
                                    irgen->code);
        if (target->result.should_sret())
            fn->addAttributeAtIndex(
                1,
                llvm::Attribute::get(irgen->context, llvm::Attribute::StructRet,
                                     irgen->ToLLVMType(target->result, false)));
    }
    return fn;
}
llvm::Value* LLVMExpressionEvaluator::doAddition(Expression* lhs,
                                                 Expression* rhs,
                                                 const Type& left_type,
...
llvm::Value* LLVMExpressionEvaluator::doMinus(Expression* lhs, Expression* rhs,
                                              const Type& left_type,
                                              const Type& right_type) {
    auto lhs_e = std::visit(*this, lhs->toVariant());
    auto rhs_e = std::visit(*this, rhs->toVariant());
    if (left_type.name == "ilit" && right_type.name == "ilit")
        return irgen->builder->CreateSub(lhs_e, rhs_e);
...
llvm::Value* LLVMExpressionEvaluator::doMult(Expression* lhs, Expression* rhs,
                                             const Type& left_type,
                                             const Type& right_type) {
    auto lhs_e = std::visit(*this, lhs->toVariant());
    auto rhs_e = std::visit(*this, rhs->toVariant());
    if (left_type.name == "ilit" && right_type.name == "ilit")
        return irgen->builder->CreateMul(lhs_e, rhs_e);
...
llvm::Value* LLVMExpressionEvaluator::doRange(Expression* lhs, Expression* rhs,
                                              const Type& left_type,
                                              const Type& right_type,
                                              const Type& result) {
    using namespace std::string_view_literals;
    auto subtype =
        Type{.name = std::string(result.name.begin() + "range_"sv.size(),
                                 result.name.end())};
    auto type = irgen->ToLLVMType(result, false);
    auto ptr = irgen->Alloca("range_literal", type);
    implicitConvert(lhs, std::visit(*this, lhs->toVariant()), left_type,
                    subtype, irgen->builder->CreateStructGEP(type, ptr, 0));
    implicitConvert(lhs, std::visit(*this, rhs->toVariant()), right_type,
                    subtype, irgen->builder->CreateStructGEP(type, ptr, 1));
    return ptr;
}
llvm::Value* LLVMExpressionEvaluator::doDiv(Expression* lhs, Expression* rhs,
                                            const Type& left_type,
                                            const Type& right_type) {
    auto lhs_e = std::visit(*this, lhs->toVariant());
    auto rhs_e = std::visit(*this, rhs->toVariant());
    if (left_type.name == "ilit" && right_type.name == "ilit")
        return irgen->builder->CreateSDiv(lhs_e, rhs_e);
...
llvm::Value* LLVMExpressionEvaluator::doRem(Expression* lhs, Expression* rhs,
                                            const Type& left_type,
                                            const Type& right_type) {
    auto lhs_e = std::visit(*this, lhs->toVariant());
    auto rhs_e = std::visit(*this, rhs->toVariant());
    if (left_type.name == "ilit" && right_type.name == "ilit")
        return irgen->builder->CreateSRem(lhs_e, rhs_e);
...
llvm::Value* LLVMExpressionEvaluator::doShl(Expression* lhs, Expression* rhs,
                                            const Type& left_type,
                                            const Type& right_type) {
    auto lhs_e = std::visit(*this, lhs->toVariant());
    auto rhs_e = std::visit(*this, rhs->toVariant());
    if (left_type.name == "ilit" && right_type.name == "ilit")
        return irgen->builder->CreateShl(lhs_e, rhs_e);
...
llvm::Value* LLVMExpressionEvaluator::doShr(Expression* lhs, Expression* rhs,
                                            const Type& left_type,
                                            const Type& right_type) {
    auto lhs_e = std::visit(*this, lhs->toVariant());
    auto rhs_e = std::visit(*this, rhs->toVariant());
    if (left_type.name == "ilit" && right_type.name == "ilit")
        return irgen->builder->CreateLShr(lhs_e, rhs_e);
...
llvm::Value* LLVMExpressionEvaluator::doCmp(ComparisonPredicate p,
    ...
    if ((p == EQ || p == NE) && left_type.is_equal(right_type)) {
        if (left_type.get_decl_if_enum()) {
            return (p == EQ)
                       ? irgen->builder->CreateICmpEQ(lhs_e, rhs_e, "enum_cmp")
    ...
    if (p == EQ)
        return irgen->builder->CreateICmpEQ(ret,
                                            llvm::ConstantInt::get(i32, eq));
    if (p == GT)
        return irgen->builder->CreateICmpEQ(
            ret, llvm::ConstantInt::get(i32, greater));
    if (p == LT)
        return irgen->builder->CreateICmpEQ(ret,
                                            llvm::ConstantInt::get(i32, less));
    if (p == NE) {
        return irgen->builder->CreateICmpNE(ret,
                                            llvm::ConstantInt::get(i32, eq));
    }
    if (p == EQ_GT) {
        return irgen->builder->CreateOr(
            irgen->builder->CreateICmpEQ(ret,
                                         llvm::ConstantInt::get(i32, greater)),
...
llvm::Value* LLVMExpressionEvaluator::doUnionVar(CallOperation* op, Type& t) {
    ...
    std::string variant =
        std::string(t.name.begin() + dollar_off + 1, t.name.end());
    std::string type_name = std::string(t.name.begin() + "__union_var"sv.size(),
                                        t.name.begin() + dollar_off);
    t.name = type_name;
    auto as_llvm = irgen->ToLLVMType(t, false);
    auto ret = irgen->Alloca("union_obj", as_llvm);
    auto decl = t.get_decl_if_union();
    auto arg =
        std::visit(LLVMExpressionEvaluator{irgen, decl->fields.at(variant)},
                   op->arguments[0]->toVariant());
    auto arg_ty =
        std::visit(ExpressionTypeChecker{irgen, decl->fields.at(variant)},
                   op->arguments[0]->toVariant())
            .value_or_error();
    implicitConvert(op->arguments[0].get(), arg, arg_ty,
                    decl->fields.at(variant),
                    irgen->builder->CreateStructGEP(as_llvm, ret, 0));
    irgen->builder->CreateStore(
        llvm::ConstantInt::get(
            llvm::Type::getInt32Ty(irgen->context),
            std::distance(decl->fields.begin(), decl->fields.find(variant))),
        irgen->builder->CreateStructGEP(as_llvm, ret, 1));
    return ret;
}

llvm::Value* LLVMExpressionEvaluator::LValueEvaluator::operator()(
    NameExpression* nm) {
    std::string name(nm->text);
    for (size_t i = irgen->variables.size(); i > 0; --i) {
        size_t idx = i - 1;
        if (auto var = irgen->variables[idx].find(name);
            var != irgen->variables[idx].end()) {
...
llvm::Value* LLVMExpressionEvaluator::LValueEvaluator::operator()(
    ...
    auto left_t =
        std::visit(ExpressionTypeChecker{irgen}, bop->lhs->toVariant());
    if (!left_t) return nullptr;
    if (!left_t->is_mutable && !left_t->is_reference()) return nullptr;
    return LLVMExpressionEvaluator{irgen}.doDot(bop->lhs.get(), bop->rhs.get(),
                                                *left_t, false);
}
...
llvm::Value* LLVMExpressionEvaluator::LValueEvaluator::operator()(
    ...
    if (op->op.type == TokenType::Star) {
        return std::visit(LLVMExpressionEvaluator{irgen},
                          op->operand->toVariant());
    }
}
...
llvm::Value* LLVMExpressionEvaluator::operator()(TupleLiteral* tup) {
    ...
    for (auto& expr : tup->elements) {
        auto idx_const =
            llvm::ConstantInt::get(llvm::Type::getInt32Ty(irgen->context), idx);
        auto idx_ptr = irgen->builder->CreateGEP(
            llvm_t, tuple_tmp, {zero_const, idx_const}, "tuple_elem");
        auto type = std::visit(ExpressionTypeChecker{irgen}, expr->toVariant());
        implicitConvert(expr.get(), std::visit(*this, expr->toVariant()), *type,
                        tuple_t->subtypes[idx], idx_ptr);
        idx++;
    }
    return tuple_tmp;
}
llvm::Value* LLVMExpressionEvaluator::operator()(ArrayLiteral* lit) {
    using repeat_notation =
    ...
    if (type->is_static_array()) {
        if (std::holds_alternative<repeat_notation>(lit->elements)) {
            ...
            if (target && target->is_array())
                elem = std::visit(
                    LLVMExpressionEvaluator{irgen, target->subtypes[0]},
                    rpn.first->toVariant());
            else
                elem = std::visit(*this, rpn.first->toVariant());
            for (size_t i = 0; i < type->static_array_size(); i++) {
                auto ptr =
                    irgen->builder->CreateConstGEP2_32(as_llvm, val, 0, i);
        ...
        for (size_t i = 0; i < elements.size(); ++i) {
            ...
            implicitConvert(elements[i].get(),
                            ...
                            tp.value(), type->subtypes[0], elem);
        }
    }
    return val;
}
llvm::Value* LLVMExpressionEvaluator::operator()(RealLiteral* lit) {
    return llvm::ConstantFP::get(llvm::Type::getDoubleTy(irgen->context),
                                 lit->token.text);
}
...
llvm::Value* LLVMExpressionEvaluator::operator()(StringLiteral* lit) {
    ...
    if (auto tp = type_checker(lit); !tp) {
        irgen->error(tp.error());
        return nullptr;
    }
    ...
    for (auto& substr : lit->literal) {
        if (std::holds_alternative<std::string>(substr)) {
            ...
            auto str_size = llvm::ConstantInt::get(
                llvm::Type::getInt64Ty(irgen->context), as_str.size());
    ...
    for (auto& pair :
         std::ranges::subrange(substrings.begin() + 1, substrings.end()))
        final_len = irgen->builder->CreateAdd(final_len, pair.second);
    // add 1 for null termination
    llvm::Value* final_buffer = irgen->Malloc(
        "string_buffer",
        irgen->builder->CreateAdd(
            final_len,
            llvm::ConstantInt::get(llvm::Type::getInt64Ty(irgen->context), 1)));
    llvm::Value* offset =
        llvm::ConstantInt::get(llvm::Type::getInt64Ty(irgen->context), 0);
    ...
    for (auto& pair : substrings) {
        ...
        irgen->builder->CreateMemCpy(current_pointer, std::nullopt, pair.first,
                                     std::nullopt, pair.second);
        if (!llvm::isa<llvm::GlobalVariable>(pair.first))
...
llvm::Value* LLVMExpressionEvaluator::operator()(NameExpression* nm) {
    ...
    for (size_t i = irgen->variables.size(); i > 0; --i) {
        ...
        if (auto var = irgen->variables[idx].find(name);
            ...
            if (!std::get<1>(var->second).should_sret()) {
                return irgen->builder->CreateLoad(
                    irgen->ToLLVMType(std::get<1>(var->second), false),
                    std::get<0>(var->second), name);
            }
            if (!ret_type->is_lvalue) {
                ...
                if (drop_flag)
                    irgen->builder->CreateStore(
                        llvm::ConstantInt::getFalse(irgen->context), drop_flag);
            }
            return std::get<0>(var->second);
        }
    }
    if (auto [name_prefix, fn] =
            irgen->module->findFunction(irgen->block_hash, name);
        fn) {
    ...
    if (ret_type->is_integral() || ret_type->is_char() ||
        ret_type->is_boolean() || ret_type->get_decl_if_enum())
        return std::visit(
            [as_llvm]<typename T>(T& val) {
        ...
        return reinterpret_cast<llvm::Constant*>(
            std::get<void*>(cnst.internal_repr));
}

llvm::Value* LLVMExpressionEvaluator::operator()(GenericNameExpression* nm) {
    if (auto [hash, fn] =
            irgen->module->findGenericFn(irgen->block_hash, nm->text);
        fn) {
        std::string mangled_name_suffix =
            nm->text + IRGenerator::mangleGenericArgs(nm->arguments);
        // it has not already been instantiated
        if (auto [name_prefx, ifn] = irgen->module->findFunction(
                irgen->block_hash, mangled_name_suffix);
            !ifn)
            irgen->generateGenericFunction(irgen->module, hash, fn,
                                           nm->arguments);
...
llvm::Value* LLVMExpressionEvaluator::operator()(PrefixOperation* op) {
    ...
    case TokenType::Star: {
        ...
        if (!target->should_sret())
            return irgen->builder->CreateLoad(irgen->ToLLVMType(*target, false),
                                              operand);
        return operand;
    }
    case TokenType::Ampersand: {
        ...
        if (!target->subtypes[0].should_sret()) {
            auto val = irgen->Alloca(
                "temp", irgen->ToLLVMType(target->subtypes[0], false));
            irgen->builder->CreateStore(operand, val);
            return val;
        }
        // taking the address of the lvalue causes the lifetime to be
        // extended till the ref dies
        if (!target->deref().is_trivially_destructible(irgen) &&
            !target->deref().is_lvalue) {
            auto lf = new ExtendedLifetimes;
...
llvm::Value* LLVMExpressionEvaluator::operator()(BinaryOperation* op) {
    ...
    case Plus:
        return doAddition(lhs, rhs, *left_t, *right_t);
    case Minus:
        return doMinus(lhs, rhs, *left_t, *right_t);
    case Star:
        return doMult(lhs, rhs, *left_t, *right_t);
    case Slash:
        return doDiv(lhs, rhs, *left_t, *right_t);
    case Percent:
        return doRem(lhs, rhs, *left_t, *right_t);
    case DoubleGreater:
        return doShr(lhs, rhs, *left_t, *right_t);
    case DoubleLess:
        return doShl(lhs, rhs, *left_t, *right_t);
    case Greater:
        return doCmp(GT, lhs, rhs, *left_t, *right_t, *res);
    case Less:
        return doCmp(LT, lhs, rhs, *left_t, *right_t, *res);
    case GreaterEqual:
        return doCmp(EQ_GT, lhs, rhs, *left_t, *right_t, *res);
    case LessEqual:
        return doCmp(EQ_LT, lhs, rhs, *left_t, *right_t, *res);
    case BangEqual:
        return doCmp(NE, lhs, rhs, *left_t, *right_t, *res);
    case DoubleEqual:
        return doCmp(EQ, lhs, rhs, *left_t, *right_t, *res);
    case Spaceship:
        return doCmp(SPACE, lhs, rhs, *left_t, *right_t, *res);
    case Dot:
        return doDot(op->lhs.get(), op->rhs.get(), *left_t);
    case DoubleDotEqual:
        irgen->error(Error(op, "Not implemented yet"));
        return nullptr;
    case DoubleDot:
        return doRange(lhs, rhs, *left_t, *right_t, *res);
    case Equal:
        return implicitConvert(rhs, std::visit(*this, r_as_var), *right_t,
                               *left_t,
...
llvm::Value* LLVMExpressionEvaluator::operator()(LogicalOperation* op) {
    ...
    if (!left) return left;
    auto left_ext = llvm::BasicBlock::Create(irgen->context, "lft_ext", fn,
                                             irgen->returnBlock);
    auto cont = llvm::BasicBlock::Create(irgen->context, "logi_cont", fn,
                                         irgen->returnBlock);
    if (op->op.type == TokenType::DoubleAmpersand) {
        ...
        auto result = irgen->builder->CreatePHI(
            llvm::Type::getInt1Ty(irgen->context), 2, "logi_result");
        result->addIncoming(llvm::ConstantInt::getFalse(irgen->context),
                            orig_block);
    ...
    if (op->op.type == TokenType::DoublePipe) {
        ...
        auto result = irgen->builder->CreatePHI(
            llvm::Type::getInt1Ty(irgen->context), 2, "logi_result");
        result->addIncoming(llvm::ConstantInt::getTrue(irgen->context),
                            orig_block);
        result->addIncoming(right, current_block);
        return result;
    }
...
llvm::Value* LLVMExpressionEvaluator::operator()(PostfixOperation*) {
    return nullptr;
}
llvm::Value* LLVMExpressionEvaluator::fillArgs(
    bool uses_sret, const FunctionSignature& sig,
    std::vector<llvm::Value*>& args, llvm::Value* first,
    ...
    for (size_t i = 0; i < exprs.size(); i++) {
        ...
        auto tp =
            std::visit(ExpressionTypeChecker{irgen}, exprs[i]->toVariant());
        if (!tp) continue;

        args[i + is_bound + uses_sret] = implicitConvert(
            exprs[i].get(), arg, *tp, sig.parameters[i + is_bound].type);
    }
    return return_value;
}

llvm::Value* LLVMExpressionEvaluator::doInvoke(CallOperation* op,
                                               const Type& left_t) {
    ...
    auto ctx_ptr = irgen->builder->CreateGEP(
        as_llvm_t, fn_store, {const_zero, const_zero}, "fn_context_ptr_ptr");
    auto fn_ptr = irgen->builder->CreateGEP(
        as_llvm_t, fn_store,
        {const_zero,
    ...
    auto return_val =
        fillArgs(uses_sret, *left_t.signature, args, nullptr, op->arguments);
    auto args_w_lambda = args;
    args_w_lambda.push_back(ctx);
    auto ctx_as_int = irgen->builder->CreatePtrToInt(
        ctx, llvm::Type::getInt64Ty(irgen->context), "fn_ctx_as_int");
    auto zero64 =
        llvm::ConstantInt::get(llvm::Type::getInt64Ty(irgen->context), 0);
    auto is_lambda =
        irgen->builder->CreateICmpEQ(ctx_as_int, zero64, "is_lambda");
    auto current_fn = irgen->builder->GetInsertBlock()->getParent();
    auto then_bb = llvm::BasicBlock::Create(irgen->context, "lambda_call",
                                            current_fn, irgen->returnBlock);
    auto else_bb = llvm::BasicBlock::Create(irgen->context, "fn_call",
                                            current_fn, irgen->returnBlock);
    auto cont_bb = llvm::BasicBlock::Create(irgen->context, "after_call",
                                            current_fn, irgen->returnBlock);
    ...
    auto return_as_llvm =
        irgen->ToLLVMType(left_t.signature->returnType, false);
...
llvm::Value* LLVMExpressionEvaluator::operator()(CallOperation* op) {
    ExpressionTypeChecker type_checker{irgen};
    // type checker is allowed to modify generic function nodes to do
    // argument deduction
    auto return_t = type_checker(op);
    if (!return_t) {
        irgen->error(return_t.error());
        return nullptr;
    }
    ...
    if (!t || !(t->is_function() || t->is_lambda())) {
        irgen->error(t.error());
        return nullptr;
    }
    if (!return_t) {
    ...
    if (fn.is_bound) {
        ...
        auto left_t =
            std::visit(ExpressionTypeChecker{irgen}, expr->lhs->toVariant());
        ...
        if (auto rhs = dynamic_cast<NameExpression*>(expr->rhs.get())) {
            ...
            if (block == this_block && fn->sig.parameters[0].name == "this") {
                ...
                std::vector<llvm::Value*> args(
                    op->arguments.size() + 1 +
                    uses_sret);  // +1 because its bound
                auto first = std::visit(*this, expr->lhs->toVariant());
                if (left_t->deref() != fn->sig.parameters[0].type.deref())
                    first = implicitConvert(expr->lhs.get(), first, *left_t,
                                            fn->sig.parameters[0].type);
                llvm::Value* return_value =
                    fillArgs(uses_sret, fn->sig, args, first, op->arguments);
        ...
        if (left_t->deref().is_view()) {
            ...
            auto [hsh, interface] =
                viewed.module->findInterface(viewed.block_hash, viewed.name);
            if (auto* name_expr =
                    dynamic_cast<NameExpression*>(expr->rhs.get());
                name_expr && interface) {
                auto it = std::ranges::find_if(
                    interface->methods, [name_expr](auto& mth) {
                ...
                std::vector<llvm::Value*> args(op->arguments.size() + 1 +
                                               uses_sret);
                ...
                auto param = irgen->builder->CreateLoad(
                    pointer_type,
                    irgen->builder->CreateStructGEP(as_llvm_type, value, 0));
                auto callee = irgen->builder->CreateLoad(
                    pointer_type,
                    irgen->builder->CreateStructGEP(
                        as_llvm_type, value,
                        1 + std::distance(interface->methods.begin(), it)));

                llvm::Value* return_value =
                    fillArgs(uses_sret, t->sig, args, param, op->arguments);
        ...
        auto right_t =
            std::visit(ExpressionTypeChecker{irgen}, expr->rhs->toVariant());
        if (right_t->is_interface_function() && cls) {
            using namespace std::string_view_literals;
            auto pos = right_t->name.find_first_of('$');
            std::string name = right_t->name.substr(
                "__interface_fn"sv.size(), pos - "__interface_fn"sv.size());
            std::string fn_name = right_t->name.substr(pos + 1);
            auto dets = irgen->module
                            ->findClass(left_t->deref().block_hash,
                                        left_t->deref().full_name_no_block())
                            .second;
            ...
            llvm::Value* return_value = fillArgs(
                uses_sret, t->sig, args,
                std::visit(*this, expr->lhs->toVariant()), op->arguments);
            auto call_val = irgen->builder->CreateCall(callee, args);
            if (!uses_sret) return_value = call_val;
            if (return_t->is_non_owning(irgen))
                stealUsages(args, return_value);
        ...
        if (!callee) { /* return an llvm invalid value*/
            return nullptr;
        }
        const auto* sig = is_lambda
                              ? &reinterpret_cast<LLModule*>(right_t->module)
                                     ->lambdas[right_t->name]
                                     .second->sig
                              : &right_t->sig;
        bool uses_sret = callee->hasStructRetAttr();
        ...
        if (left_t->deref() != sig->parameters[0].type.deref()) {
            first = implicitConvert(expr->lhs.get(), first, *left_t,
                                    sig->parameters[0].type, nullptr);
        }
        llvm::Value* return_value =
            fillArgs(uses_sret, fn.sig, args, first, op->arguments);
        if (is_lambda)
            args.push_back(std::visit(*this, expr->rhs->toVariant()));
        auto call_val =
            irgen->builder->CreateCall(callee->getFunctionType(), callee, args);
        if (!uses_sret) return_value = call_val;
        if (return_t->is_non_owning(irgen))
            stealUsages(args, return_value);
        else
    ...
    std::vector<llvm::Value*> args(op->arguments.size() + uses_sret);
    llvm::Value* return_value =
        fillArgs(uses_sret, fn.sig, args, nullptr, op->arguments);
...
llvm::Value* LLVMExpressionEvaluator::operator()(SubscriptOperation* op) {
    auto obj_ty =
        std::visit(ExpressionTypeChecker{irgen}, op->object->toVariant());
    if (!obj_ty) {
        irgen->error(obj_ty.error());
        return nullptr;
    }
    ...
    if (obj_ty->is_slice()) {
        ...
        data_ptr = irgen->builder->CreateLoad(
            llvm::PointerType::get(irgen->context, 0), data_ptr);
...
llvm::Value* LLVMExpressionEvaluator::operator()(GCNewExpression* expr) {
    ...
    auto llvm_ty = llvm::StructType::get(
        irgen->context, {internal_ty, llvm::Type::getInt64Ty(irgen->context)},
        false);
    size_t type_size = irgen->code->getDataLayout().getTypeAllocSize(llvm_ty);
    size_t internal_size =
        irgen->code->getDataLayout().getTypeAllocSize(internal_ty);
    auto memory = irgen->GCMalloc(type_size);
    auto copy_to = irgen->builder->CreateStructGEP(llvm_ty, memory, 0);
    irgen->builder->CreateMemCpy(copy_to, std::nullopt, value, std::nullopt,
                                 internal_size);
    irgen->builder->CreateStore(
        llvm::ConstantInt::get(llvm::Type::getInt64Ty(irgen->context), 0),
        irgen->builder->CreateStructGEP(llvm_ty, memory, 1));
    return memory;
}
...
llvm::Value* LLVMExpressionEvaluator::operator()(LambdaExpression* expr) {
    ...
    auto context =
        reinterpret_cast<LLModule*>(t->module)->lambdas.at(t->name).first;
    auto ctx_object = irgen->Alloca("lambda_context", context);
    // copy types into the context
    size_t idx = 0;
    if (!reinterpret_cast<LLModule*>(irgen->module)
             ->lambdas.contains(t->name)) {
        ...
        for (auto& capture : expr->captures) {
            ...
            llvm::Type* tp = capture.cp_type == Ownership::Owning
                                 ? irgen->ToLLVMType(*type, false)
                                 : llvm::PointerType::get(irgen->context, 0);
            context_types.push_back(tp);
        }
        // was considering stealing the unique ptr, but cloning is much more
        // convenient also we can choose not to store the body and free
        // memory
        ...
        expr->body.swap(body);
        std::unique_ptr<LambdaExpression> new_expr(
            reinterpret_cast<LambdaExpression*>(new_expr_generic.get()));
        new_expr_generic.release();
        reinterpret_cast<LLModule*>(irgen->module)->lambdas[t->name] = {
            context, std::move(new_expr)};
    }
    for (auto& capture : expr->captures) {
        NameExpression name(capture.name);
        auto type = ExpressionTypeChecker{irgen}(&name);
        bool is_last_use =
            irgen->function_cfgs.back().last_uses.at(name.text).contains(expr);
        type->is_lvalue = !is_last_use;
        llvm::Value* lhs =
            irgen->builder->CreateStructGEP(context, ctx_object, idx);
    ...
    sig.parameters.push_back(FunctionParameter{
        .type = Type{.name = name, .module = irgen->module}
    });
...
llvm::Value* LLVMExpressionEvaluator::operator()(ScopeOperation* op) {
    ...
    if (!ty) {
        irgen->error(ty.error());
        return nullptr;
    }
    if (ty->is_function()) {
        std::string mangled_name =
            ty->block_hash + UnsaturatedTypeIterator(op->type).last().name;
    ...
    if (auto enm = ty->get_decl_if_enum()) {
        ...
        return llvm::ConstantInt::get(llvm::Type::getInt32Ty(irgen->context),
                                      enm->values[name]);
    }
    ...
    if (ty->is_integral() || ty->is_char() || ty->is_boolean() ||
        ty->get_decl_if_enum())
        return std::visit(
            [as_llvm]<typename T>(T& val) {
                if constexpr (std::is_integral_v<T>)
...
llvm::Value* LLVMExpressionEvaluator::operator()(ObjectLiteral* lit) {
    ...
    for (size_t i = 0; i < decl->vars.size(); i++) {
        auto idx_const =
            llvm::ConstantInt::get(llvm::Type::getInt32Ty(irgen->context), i);
        auto& var = decl->vars[i];
        auto val_ty = std::visit(ExpressionTypeChecker{irgen, var.type},
                                 lit->values[var.name]->toVariant());
        if (!val_ty) {
            irgen->error(val_ty.error());
            continue;
        }
        auto val = std::visit(LLVMExpressionEvaluator{irgen, var.type},
                              lit->values[var.name]->toVariant());

        auto mem_ptr = irgen->builder->CreateGEP(as_llvm_type, value,
                                                 {zero_const, idx_const});
        implicitConvert(lit->values[var.name].get(), val, *val_ty, var.type,
                        mem_ptr);
    }
    return value;
}
...
llvm::Value* LLVMExpressionEvaluator::operator()(AsExpression* expr) {
    ...
    if (ty->is_variant()) {
        ...
        auto internal_ptr =
            irgen->builder->CreateStructGEP(internal_as_llvm, internal, 0);
        irgen->builder->CreateStore(internal_ptr, ptr);
        auto is_valid_ptr = irgen->builder->CreateStructGEP(as_llvm, val, 1);
        auto idx_ptr =
            irgen->builder->CreateStructGEP(internal_as_llvm, internal, 1);

        auto i32_ty = llvm::Type::getInt32Ty(irgen->context);
        auto this_idx = llvm::ConstantInt::get(
            i32_ty, std::distance(subtypes.begin(), subtypes.find(expr->dest)));
        ...
        if (!ty->deref().is_trivially_destructible(irgen) &&
            !ty->deref().is_lvalue) {
            auto lf = new ExtendedLifetimes;
    ...
    if (final_ty->is_view()) {
        ...
        if (auto cls = ty->deref().get_decl_if_class(irgen)) {
            auto details =
                ty->deref()
                    .module->findClass(ty->deref().block_hash, ty->deref().name)
                    .second;
            ...
            for (auto& method : intf->methods) {
                idx++;
                auto fn_ptr =
                    irgen->builder->CreateStructGEP(as_llvm, result, idx);
                std::string method_name = std::get<0>(*details) +
                                          "__interface" + full_name + "__%" +
                                          method->name;
                auto fn = irgen->code->getFunction(method_name);
                irgen->builder->CreateStore(fn, fn_ptr);
            }
