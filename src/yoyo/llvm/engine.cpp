...
struct ForwardDeclaratorPass1 {
    ...
    bool operator()(FunctionDeclaration* decl) const {
        ...
        std::visit(ForwardDeclaratorPass1{md, decl->body, new_blk},
                   decl->body->toVariant());
        return true;
    }
    ...
    bool operator()(ClassDeclaration* decl) const {
        ...
        for (auto& stt : decl->stats) {
            std::visit(ForwardDeclaratorPass1{md, stt, mangled_name_prefix},
                       stt->toVariant());
        }
    ...
    bool operator()(ModuleImport* imp) {
        reinterpret_cast<LLVMEngine*>(md->engine)
            ->addModule(imp->module_path, "");
        md->modules[imp->module_name] =
            md->engine->modules[imp->module_path].get();
        return true;
    }
    bool operator()(EnumDeclaration* decl) {
        ...
        for (auto& stt : decl->stats) {
            std::visit(ForwardDeclaratorPass1{md, stt, new_blk},
                       stt->toVariant());
        }
    ...
    bool operator()(UnionDeclaration* decl) {
        ...
        md->unions[block].emplace_back(std::unique_ptr<UnionDeclaration>{decl},
                                       nullptr);
    ...
    bool operator()(BlockStatement* stat) {
        for (auto& sub : stat->statements) {
            std::visit(ForwardDeclaratorPass1{md, sub, block},
                       sub->toVariant());
        }
        return false;
    }
...
LLVMAppModule* LLVMEngine::addAppModule(const std::string& name) {
    ...
    md->module_hash = name + "::";
    md->engine = this;
    reinterpret_cast<LLModule*>(md.get())->code = llvm::orc::ThreadSafeModule(
        std::make_unique<llvm::Module>(name, ctx), llvm_context);
...
void LLVMEngine::addModule(const std::string& module_name, std::string source) {
    ...
    for (auto& stat : prog) {
        if (!std::visit(ForwardDeclaratorPass1{md.get(), stat, md->module_hash},
                        stat->toVariant())) {
...
void LLVMEngine::compile() {
    ...
    for (auto& mod : modules) {
        ...
        if (!llvm_mod->code) {
            ...
            irgen.view = &vw;
            if (!irgen.GenerateIR(mod.first, std::move(src.mapped().second),
                                  llvm_mod, this))
...
void LLVMEngine::addStaticLibrary(std::string_view path) {
    ...
    auto err =
        jit->linkStaticLibraryInto(jit->getMainJITDylib(), path_str.c_str());
    if (err) debugbreak();
}
void LLVMEngine::addDynamicLibrary(std::string_view path) {
    ...
    std::string err;
    auto lib =
        llvm::sys::DynamicLibrary::getPermanentLibrary(path_str.c_str(), &err);
    if (!lib.isValid()) {
        debugbreak();
    }
    auto ptr = std::make_unique<llvm::orc::DynamicLibrarySearchGenerator>(
        lib, jit->getDataLayout().getGlobalPrefix());
    jit->getMainJITDylib().addGenerator(std::move(ptr));
    // if (auto DLSGOrErr =
    //     llvm::orc::DynamicLibrarySearchGenerator::Load(path_str.c_str(),
...
void* LLVMEngine::createGlobalConstant(const Type& type,
    ...
    if (type.name == "str") {
        auto str_ptr =
            static_cast<char*>(std::get<void*>(args[0].internal_repr));
        auto str_sz = strlen(str_ptr);
        auto string = irgen->builder->CreateGlobalString(str_ptr);
        auto str_size = llvm::ConstantInt::get(
            llvm::Type::getInt64Ty(irgen->context), str_sz);

        return new llvm::GlobalVariable(
            llvm_t, true, llvm::GlobalValue::ExternalLinkage,
            llvm::ConstantStruct::get(
                reinterpret_cast<llvm::StructType*>(llvm_t),
                {string, str_size, str_size}));
    }
    std::vector<llvm::Constant*> consts;
    consts.reserve(args.size());
    size_t n = 0;
    ...
    for (auto& constant : args) {
        ...
        if (auto as_int = llvm::dyn_cast<llvm::IntegerType>(elem_ty)) {
            consts.push_back(std::visit(
                [elem_ty]<typename T>(T& val) {
