...
llvm::FunctionType* toLLVMSignature(const FunctionSignature& sig,
    ...
    // sret
    auto return_t =
        module->ToLLVMType(sig.returnType, module->module_hash, nullptr, {});
    ...
    for (const auto& p : sig.parameters) {
        if (p.type.should_sret())
            ...
            argTypes.push_back(
                module->ToLLVMType(p.type, module->module_hash, nullptr, {}));
    }
    return llvm::FunctionType::get(return_t, argTypes, false);
}
void AppModule::addFunction(FunctionSignature sig, void* func,
                            std::string name) {
    ...
    functions[module_hash].emplace_back(name, std::move(sig),
                                        std::vector{Attribute{"public"}});

    auto fn =
        llvm::Function::Create(llvm_sig, llvm::GlobalValue::ExternalLinkage,
                               mangled_name, code.getModuleUnlocked());
    if (uses_sret)
    ...
    auto addr =
        llvm::ConstantInt::get(llvm::Type::getIntNTy(ctx, sizeof func * 8),
                               reinterpret_cast<std::uintptr_t>(func));
    auto addr_as_ptr =
        builder.CreateIntToPtr(addr, llvm::PointerType::get(ctx, 0));
...
Result AppModule::addEnum(std::string enum_name,
    ...
    enums[module_hash].emplace_back(std::make_unique<EnumDeclaration>(
        std::move(enum_name), std::move(values),
        std::vector<std::unique_ptr<Statement>>{}));
    return Result::Success;
}
Result AppModule::addClass(std::string class_name,
                           std::vector<ClassVariable> variables, Ownership sh,
                           std::string block_hash) {
    auto hash = module_hash + block_hash;
    classes[hash].emplace_back(
        hash + class_name + "::", nullptr,
        std::make_unique<ClassDeclaration>(
            Token{.text = class_name}, std::move(variables),
            std::vector<std::unique_ptr<Statement>>{}, sh,
