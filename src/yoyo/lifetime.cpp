...
void Yoyo::BorrowChecker::drop_object(const std::string& obj, ASTNode* expr) {
    current_block->insts.push_back(
        Instruction{.data = obj, .type = Instruction::Drop, .expr = expr});
}
void BorrowChecker::set_block(BorrowCheckerBlock* block) {
    current_block = block;
}
std::string BorrowChecker::borrow_values(
    std::span<const std::pair<std::string, BorrowType>> arg, Expression* expr) {
    std::string into = std::to_string(counter++);
    for (auto& [value, type] : arg) {
        current_block->insts.push_back(Instruction{
            .data =
                Instruction::BorrowInstruction{.into = into, .operand = value},
            .type = type == BorrowType::Const ? Instruction::Borrow
...
void check_block(BorrowCheckerBlock* block, BorrowState stt,
                 BorrowCheckerBlock* from, IRGenerator* irgen) {
    using namespace std::ranges;
    using namespace std::views;
    if (from) {
        block->preds.at(from) = std::move(stt);
    }
    if (!all_of(block->preds | values,
                [](auto& param) { return param.has_value(); })) {
        return;
    }
    auto state = BorrowState::aggregate(
        block->preds | values |
        views::transform([](auto& value) { return value.value(); }));
    block->is_checked = true;
    for (auto& inst : block->insts) {
        if (inst.type == Instruction::Br) {
            return check_block(std::get<BorrowCheckerBlock*>(inst.data),
                               std::move(state), block, irgen);
        } else if (inst.type == Instruction::CondBr) {
            auto& br_inst =
                std::get<std::pair<BorrowCheckerBlock*, BorrowCheckerBlock*>>(
                    inst.data);
            ...
            if (state.mutable_borrow.contains(cinst.operand)) {
                ...
                auto error = Error(inst.expr,
                                   "Attempt to borrow a value that has "
                                   "been mutably borrowed");
                error.markers.emplace_back(
                    SourceSpan{elem->expr->beg, elem->expr->end},
            ...
            state.is_borrowing[cinst.into].emplace_back(
                cinst.operand, BorrowType::Const, &inst);
            ...
            if (state.immutable_borrows.contains(cinst.operand) ||
                ...
                irgen->error(Error(inst.expr,
                                   "Attempt to mutably borrow already borrowed "
                                   "value TODO show other borrow"));
                return;
            }
            // this instruction is where the borrow occurs
            state.mutable_borrow[cinst.operand] = &inst;
            // "into" borrows "operand" as immutable in this instruction
            state.is_borrowing[cinst.into].emplace_back(cinst.operand,
                                                        BorrowType::Mut, &inst);
            ...
            if (state.immutable_borrows.contains(var) ||
                ...
                irgen->error(Error(inst.expr,
                                   "Attempt to drop already borrowed value "
                                   "TODO show other borrow"));
                return;
            }
            ...
            if (state.is_borrowing.contains(var)) {
                for (auto& borrow : state.is_borrowing.at(var)) {
                    if (std::get<1>(borrow) == BorrowType::Mut) {
                        ...
                        auto& imm =
                            state.immutable_borrows.at(std::get<0>(borrow));
                        imm.erase(std::get<2>(borrow));
                        if (imm.empty())
                            state.immutable_borrows.erase(std::get<0>(borrow));
                    }
                }
...
void BorrowChecker::create_cond_br(BorrowCheckerBlock* success,
                                   BorrowCheckerBlock* fail) {
    current_block->insts.push_back(Instruction{
...
void BorrowChecker::create_br(BorrowCheckerBlock* block) {
    current_block->insts.push_back(
        Instruction{.data = block, .type = Instruction::Br, .expr = nullptr});
}
...
std::string BorrowChecker::move_object(const std::string& from,
                                       Expression* expr) {
    auto obj = make_object();
    move_into(from, obj, expr);
    return obj;
}
void BorrowChecker::move_into(const std::string& from, const std::string& to,
                              Expression* expr) {
    current_block->insts.push_back(Instruction{
        .data = std::pair{from, to},
          .type = Instruction::Move, .expr = expr
    });
}
std::string BorrowChecker::borrow_as_temporary(const std::string& value,
                                               Expression* expr) {
    std::string into = make_object();
    current_block->insts.push_back(Instruction{
        .data = Instruction::BorrowInstruction{.into = into, .operand = value},
