...
Constant ConstantEvaluator::operator()(IntegerLiteral* lit) {
    if (target && target->is_signed_integral())
        return static_cast<int64_t>(std::stoll(lit->text));
...
Constant ConstantEvaluator::operator()(NameExpression* nexpr) {
    ...
    if (!dets) {
        irgen->error(Error(nexpr, nexpr->text + " does not name a constant"));
        return nullptr;
    }
    // check if its recursive
    if (auto it = std::ranges::find(disallowed_consts,
                                    std::pair{blk, std::get<1>(*dets)});
...
Constant ConstantEvaluator::constConvert(const Constant& src,
                                         const Type& source,
...
Constant ConstantEvaluator::operator()(ObjectLiteral* lit) {
    if (!is_initial)
        irgen->error(Error(lit,
                           "Structural constants are only allowed as top "
                           "level constants"));
    is_initial = false;
...
Constant ConstantEvaluator::operator()(StringLiteral* lit) {
    if (lit->literal.size() != 1) {
        irgen->error(Error(
            lit, "String interpolation is not supported in constant strings"));
        return nullptr;
    }
    if (std::holds_alternative<std::string>(lit->literal[0])) {
        auto& as_str = std::get<std::string>(lit->literal[0]);
        return irgen->module->engine->createGlobalConstant(
            Type{"str"}, {as_str.c_str()}, irgen);
    } else {
        irgen->error(Error(
            lit, "String interpolation is not supported in constant strings"));
        return nullptr;
...
Constant ConstantEvaluator::operator()(BinaryOperation* bop) {
    ...
    switch (bop->op.type) {
        ...
        // Implicit requirement for the machine / compiler to use twos
        // complement
    case Plus:
        return is_float ? Constant{std::get<double>(left_repr) +
                                   std::get<double>(right_repr)}
                        : Constant{std::get<uint64_t>(left_repr) +
                                   std::get<uint64_t>(right_repr)};
    case Star:
        return is_float ? Constant{std::get<double>(left_repr) *
                                   std::get<double>(right_repr)}
                        : Constant{std::get<uint64_t>(left_repr) *
    ...
    case Slash: {
        if (is_float)
            return std::get<double>(left_repr) / std::get<double>(right_repr);
        else if (type->is_signed_integral())
            return std::get<int64_t>(left_repr) / std::get<int64_t>(right_repr);
        else
            return std::get<uint64_t>(left_repr) /
                   std::get<uint64_t>(right_repr);
    }
    case Percent: {
    ...
    case DoubleGreater:
        return std::get<uint64_t>(left_repr) >> std::get<uint64_t>(right_repr);
    case DoubleLess:
        return std::get<uint64_t>(left_repr) << std::get<uint64_t>(right_repr);
    /*
    case DoubleEqual:
    ...
    */
    default:
        irgen->error(Error(bop, "Invalid constant operation"));
    }
    return nullptr;
}
...
Constant ConstantEvaluator::operator()(ScopeOperation* scp) {
    ...
    while (!iterator.is_end()) {
        ...
        if (!advanceScope(type, md, hash, irgen, first)) {
            irgen->error(Error(scp, "The name '" + type.name +
    ...
    if (!dets) {
        irgen->error(Error(scp, c_name + " does not name a constant"));
        return nullptr;
    }
