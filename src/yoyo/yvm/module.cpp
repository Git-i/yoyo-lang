...
std::pair<std::string,
          std::tuple<std::string, StructNativeTy*, ClassDeclaration*>>
    YVMModule::findClassWithType(const std::string& block,
                                 const std::string& name) {
    if (auto cls = findClass(block, name); cls.second) {
        return {
            cls.first,
            {cls.second->first, classes_types[cls.second->second.get()],
                    cls.second->second.get()}
        };
    }
    return {
        "", {"", nullptr, nullptr}
...
std::pair<std::string, std::pair<UnionDeclaration*, StructNativeTy*>>
    YVMModule::findUnionWithType(const std::string& block,
                                 const std::string& name) {
    if (auto unn = findUnion(block, name); unn.second) {
        return {
            unn.first, {unn.second, union_types[unn.second]}
        };
    }
    return {
        "", {nullptr, nullptr}
...
NativeTy* YVMModule::toNativeType(const Type& type, const std::string& hash,
                                  IRGenerator* irgen,
    ...
    if (type.is_integral()) {
        ...
        case 8:
            return type.is_unsigned_integral() ? NativeType::getU8()
                                               : NativeType::getI8();
        case 16:
            return type.is_unsigned_integral() ? NativeType::getU16()
                                               : NativeType::getI16();
        case 32:
            return type.is_unsigned_integral() ? NativeType::getU32()
                                               : NativeType::getI32();
        case 64:
            return type.is_unsigned_integral() ? NativeType::getU64()
                                               : NativeType::getI64();
        default:
            return nullptr;
        }
    ...
    if (type.is_floating_point())
        return *type.float_width() == 32 ? NativeType::getF32()
                                         : NativeType::getF64();
    if (type.is_boolean()) return NativeType::getU8();
    if (type.name == "void") return NativeType::getVoid();
    if (type.is_gc_reference()) {
        return eng.struct_manager.get_struct_type({
            {reinterpret_cast<YVMModule*>(type.deref().module)
                 ->toNativeType(type.deref(), hash, irgen, disallowed_types),
             NativeType::getI64()}
        });
    }
    if (type.is_opaque_pointer() || type.is_reference())
        return NativeType::getPtrTy();
    if (type.is_char()) return NativeType::getU32();
    ...
    if (type.is_tuple()) {
        ...
        for (auto& subtype : type.subtypes) {
            auto ty =
                reinterpret_cast<YVMModule*>(subtype.module)
                    ->toNativeType(subtype, hash, irgen, disallowed_types);
    ...
    if (type.is_lambda()) {
        return reinterpret_cast<YVMModule*>(type.module)
    ...
    if (type.is_optional()) {
        ...
        args[0] =
            reinterpret_cast<YVMModule*>(type.subtypes[0].module)
                ->toNativeType(type.subtypes[0], hash, irgen, disallowed_types);
    ...
    if (type.is_result()) {
        ...
        args[0] = eng.union_manager.get_union_type({
            {reinterpret_cast<YVMModule*>(type.subtypes[0].module)
                 ->toNativeType(type.subtypes[0], hash, irgen,
             disallowed_types),
             reinterpret_cast<YVMModule*>(type.subtypes[1].module)
                 ->toNativeType(type.subtypes[1], hash, irgen,
             disallowed_types)}
        });
        args[1] = NativeType::getU8();
        return eng.struct_manager.get_struct_type(args);
    ...
    if (type.is_variant()) {
        ...
        //{
        //     auto sub_t =
        //     reinterpret_cast<LLModule*>(subtype.module)->ToLLVMType(subtype,
        //     hash, irgen, disallowed_types); auto as_struct =
        //     llvm::dyn_cast_or_null<llvm::StructType>(sub_t); size_t sz =
        //     0; if (!as_struct) sz = sub_t->getPrimitiveSizeInBits() / 8;
        //     else sz =
        //     layout.getStructLayout(as_struct)->getSizeInBytes(); if (sz >
        //     size) size = sz;
        // }
        // args[0] = llvm::ArrayType::get(llvm::Type::getInt8Ty(context),
        // size); args[1] = llvm::Type::getInt32Ty(context); // 2^32 is a
        // reasonable amount of variant subtypes return
        // llvm::StructType::get(context, args);
    }
    ...
    if (type.is_ref_conversion_result()) {
        return eng.struct_manager.get_struct_type({
            {NativeType::getPtrTy(), NativeType::getU8()}
        });
    }
    ...
    if (type.is_view()) {
        ...
        if (intf) {
            ...
            std::vector<NativeTy*> subtypes(intf->methods.size() + 1,
                                            NativeType::getPtrTy());
    ...
    if (auto t = findUnionWithType(type.block_hash, type.full_name_no_block());
        t.second.first) {
        ...
        for (auto& [name, field_ty] : decl->fields) {
            ...
            auto ty = reinterpret_cast<YVMModule*>(field_ty.module)
                          ->toNativeType(field_ty, hash, irgen, not_allowed);
            if (!ty) return nullptr;
            args.push_back(ty);
        }
    ...
    if (auto t = findClassWithType(type.block_hash, type.full_name_no_block());
        std::get<2>(t.second)) {
        ...
        for (auto& subvar : decl->vars) {
            ...
            auto ty = reinterpret_cast<YVMModule*>(subtype.module)
                          ->toNativeType(subtype, hash, irgen, not_allowed);
            if (!ty) return nullptr;
            args.push_back(ty);
        }
        return eng.struct_manager.get_struct_type(args);
...
void YVMModule::makeBuiltinModule(YVMEngine* eng) {
    ...
    for (auto& t : std::ranges::subrange(types.begin() + 2, types.end())) {
        Type result{
            .name = "CmpOrd", .module = mod, .block_hash = mod->module_hash};
...
std::string YVMModule::dumpIR() {
    ...
    for (auto& [name, body] : code.code) {
        final += name + ":\n\n";
        final += Yvm::Disassembler::disassemble(
            body, &reinterpret_cast<YVMEngine*>(engine)->vm);
    }
    return final;
}
void YVMModule::registerStringDestructor(YVMModule* mod) {
    Yvm::Emitter em(false);
    auto str_ty = reinterpret_cast<StructNativeTy*>(
        mod->toNativeType(Type{"str"}, "", nullptr, {}));
