...
void YVMExpressionEvaluator::implicitConvert(Expression* xp, const Type& src,
                                             const Type& dst, bool on_stack,
                                             bool do_load) {
    if (dst.is_equal(src)) {
        return clone(xp, src, on_stack, do_load);
    }
    ...
    if (src.name == "ilit") {
        // There's no way to check the value of the constant till folding is
        // implemented in Yvm::Emitter
    ...
    if (src.name == "flit") {
        if (dst.is_floating_point()) {
            ...
            if (do_load && on_stack) {
                irgen->error(
                    Error(xp, "Internal, error do_load and on_stack set"));
            } else if (on_stack)
                irgen->builder->write_2b_inst(OpCode::Store,
                                              irgen->toTypeEnum(dst));
    ...
    if (dst.is_unsigned_integral()) {
        ...
        irgen->builder->write_3b_inst(OpCode::UConv, *src.integer_width(),
                                      *dst.integer_width());
        if (do_load && on_stack) {
    ...
    if (dst.is_signed_integral()) {
        if (src.is_signed_integral()) {
            irgen->builder->write_3b_inst(OpCode::SConv, *src.integer_width(),
                                          *dst.integer_width());
            if (do_load && on_stack) {
                irgen->error(
                    Error(xp, "Internal, error do_load and on_stack set"));
    ...
    if (dst.is_floating_point()) {
        if (src.is_floating_point()) {
            irgen->builder->write_3b_inst(OpCode::FpConv, *src.float_width(),
                                          *dst.float_width());
            if (do_load && on_stack) {
                irgen->error(
                    Error(xp, "Internal, error do_load and on_stack set"));
    ...
    if (src.is_gc_reference() && dst.is_reference()) {
        ...
        irgen->builder->write_ptr_off(NativeType::getElementOffset(
            reinterpret_cast<StructNativeTy*>(irgen->toNativeType(src)), 1));
        irgen->builder->write_1b_inst(
            OpCode::Dup);  // keep a copy of the pointer to the count to
                           // update it
        ...
        if (dst.is_mutable_reference()) {
            ...
            // since we're going to add one to the borrow value we can just
            // duplicate it rather than load again
    ...
    auto ret_alloc = 0;
    if (!on_stack) {
        irgen->builder->write_alloca(NativeType::get_size(native));
        ret_alloc = irgen->builder->last_alloc_addr();
    }
    if (dst.is_optional()) {
        // opt is {data, bool}
        ...
        irgen->builder->write_1b_inst(OpCode::Dup);
        irgen->builder->write_ptr_off(
            NativeType::getElementOffset(struct_native, 1));
        ...
        // place a pointer to the data on the stack top and do implicit
        // conversion
    ...
    if (dst.is_variant()) {
        ...
        // TODO: I should reall use `find` and `std::distance`, and do so in
        // the llvm backend
        for (auto& sub : subtypes) {
            ...
            irgen->builder->write_1b_inst(OpCode::Dup);
            irgen->builder->write_ptr_off(
                NativeType::getElementOffset(struct_native, 0));
    ...
    if (dst.is_slice()) {
        if (src.is_reference()) {
            if (src.deref().is_static_array()) {
                ...
                // [array, slice pointer]
                // we can get the slice data pointer pointer and store array
                // into it with reverse stack addressing dup, ptr_off =>
                // [array, slice pointer, slice data pointer pointer]
                // rev_stack_addr 2 => [array, slice pointer, slice data
                // pointer pointer, array] switch => [array, slice pointer,
                // array, slice data pointer pointer] store ptr => [array,
                // slice pointer] dup, ptr_off => [array, slice pointer,
                // slice size pointer] constant => [array, slice pointer,
                // slice size pointer, array size] switch => [array, slice
                // pointer, array size, slice size pointer] store u64 =>
                // [array, slice pointer] top_consume => [slice pointer]
                irgen->builder->write_1b_inst(OpCode::Dup);
                irgen->builder->write_ptr_off(
                    NativeType::getElementOffset(struct_native, 0));
                ...
                irgen->builder->write_ptr_off(
                    NativeType::getElementOffset(struct_native, 1));
                irgen->builder->write_const<uint64_t>(
    ...
    if (dst.name == "__called_fn") {
        if (src.is_function()) {
            /*irgen->builder->CreateStore(llvm::ConstantPointerNull::get(llvm::PointerType::get(irgen->context,
            0)), irgen->builder->CreateStructGEP(dst_as_llvm, out, 0));
        ...
        if (src.is_lambda()) {
            ...
            irgen->builder->CreateStore(irgen->code->getFunction(src.module->module_hash
            + src.name), irgen->builder->CreateStructGEP(dst_as_llvm, out,
            1)); return out;*/
        }
    }
    if (dst.is_view()) {
        /*if (src.is_view())
        {
            ...
            irgen->builder->CreateMemCpy(out, std::nullopt, val,
        std::nullopt, size); return out;
        }
        auto& viewed = dst.subtypes[0];
        auto [name, intf] = viewed.module->findInterface(viewed.block_hash,
        viewed.name); std::string full_name = name + intf->name; if (auto
        cls = src.deref().get_decl_if_class(irgen))
        {
            auto details =
        src.deref().module->findClass(src.deref().block_hash,
        src.deref().name).second; auto this_ptr =
        irgen->builder->CreateStructGEP(dst_as_llvm, out, 0);
            ...
            {
                idx++;
                auto fn_ptr = irgen->builder->CreateStructGEP(dst_as_llvm,
        out, idx); std::string method_name = std::get<0>(*details) +
        full_name + "::" + method->name; auto fn =
        irgen->code->getFunction(method_name);
...
void YVMExpressionEvaluator::clone(Expression* xp, const Type& left_type,
                                   bool on_stack, bool perform_load) {
    if (!left_type.should_sret()) {
        if (on_stack) {
            irgen->builder->write_1b_inst(OpCode::Switch);
            irgen->builder->write_2b_inst(OpCode::RevStackAddr, 1);
            irgen->builder->write_2b_inst(OpCode::Store,
                                          irgen->toTypeEnum(left_type));
            return;
        } else {
    ...
    if (!left_type.is_lvalue) {
        ...
        irgen->builder->write_2b_inst(OpCode::RevStackAddr,
                                      2);  // get the source object
        irgen->builder->write_2b_inst(OpCode::RevStackAddr,
    ...
    if (left_type.is_tuple()) {
        ...
        for (auto& sub : left_type.subtypes) {
            ...
            if (!sub.should_sret())
                irgen->builder->write_2b_inst(OpCode::Load, sub_yvm);
            irgen->builder->write_2b_inst(OpCode::RevStackAddr, 1);
        ...
        if (!left_type.subtypes[0].should_sret())
            irgen->builder->write_2b_inst(
                OpCode::Load, irgen->toTypeEnum(left_type.subtypes[0]));
        irgen->builder->write_2b_inst(OpCode::RevStackAddr, 2);
    ...
    // class types can define custom clone
        ...
        if (!decl->has_clone) {
            irgen->error(
                Error(xp, "Expression of type " +
                              left_type.pretty_name(irgen->block_hash) +
                              " cannot be cloned"));
            return;
        }
        ...
        if (candidate != decl->stats.end() &&
            ...
            irgen->builder->write_1b_inst(
                OpCode::Pop);  // Pop the zero call returns
            irgen->builder->write_1b_inst(OpCode::TopConsume);
        } else {
            ...
            for (auto& var : decl->vars) {
                ...
                auto elem_off =
                    NativeType::getElementOffset(struct_native, idx);
                irgen->builder->write_2b_inst(OpCode::RevStackAddr, 1);
                irgen->builder->write_ptr_off(elem_off);
...
std::vector<Type> YVMExpressionEvaluator::doDot(Expression* lhs,
                                                Expression* rhs,
    ...
    if (left_type.deref().is_tuple()) {
        if (auto idx = dynamic_cast<IntegerLiteral*>(rhs)) {
            ...
            auto native_t = reinterpret_cast<StructNativeTy*>(
                irgen->toNativeType(left_type.deref()));
            irgen->builder->write_ptr_off(
                NativeType::getElementOffset(native_t, idx_int));
            if (load_primitive && !out_type.should_sret()) {
                irgen->builder->write_2b_inst(OpCode::Load,
                                              irgen->toTypeEnum(out_type));
                return {};
            }
        ...
        if (auto* name_expr = dynamic_cast<NameExpression*>(rhs)) {
            ...
            if (auto var = std::ranges::find_if(
                ...
                auto native_t = reinterpret_cast<StructNativeTy*>(
                    irgen->toNativeType(left_type.deref()));
                irgen->builder->write_ptr_off(NativeType::getElementOffset(
                    native_t, std::distance(cls->vars.begin(), var)));

                if (load_primitive && !var->type.should_sret()) {
                    irgen->builder->write_2b_inst(OpCode::Load,
                                                  irgen->toTypeEnum(var->type));
                    return {};
                }
...
std::string getOperatorFunction(TokenType t, YVMIRGenerator* irgen,
                                OverloadDetailsBinary* target) {
    return target->mangled_name(t);
}
std::vector<Type> doBasicBinaryOp(YVMExpressionEvaluator* eval,
                                  OverloadDetailsBinary* target_ovl,
                                  TokenType toktp, BinaryOperation* op) {
    auto tok_to_str = [](TokenType tok) {
        switch (tok) {
        case TokenType::Plus:
            return "+";
        ...
        case TokenType::DoubleLess:
            return "<<";
        case TokenType::SquarePair:
            return "[]";
        case TokenType::SquarePairMut:
            return "[mut]";
        default:
            debugbreak();
            return "";
    ...
    auto lhs_e = std::visit(*eval, op->lhs->toVariant());
    eval->clone(op->lhs.get(), left_type, false, true);

    auto& right_type = op->rhs->evaluated_type;
    auto rhs_e = std::visit(*eval, op->rhs->toVariant());
    eval->clone(op->rhs.get(), right_type, false, true);

    std::string fn;
    Type& result = op->evaluated_type;
    if (op->subtypes.empty())
        fn = op->module->module_hash +
             getOperatorFunction(toktp, eval->irgen, target_ovl);
    else {
        auto fn_name =
            op->module->module_hash + "operator" + tok_to_str(toktp) +
            IRGenerator::mangleGenericArgs(op->subtypes) +
            target_ovl->left.full_name() + "&&" + target_ovl->right.full_name();
        if (!reinterpret_cast<YVMModule*>(op->module)
                 ->code.code.contains(fn_name)) {
            // Generate the overload
            std::swap(eval->irgen->block_hash, op->module->module_hash);
            std::swap(eval->irgen->module, op->module);
            for (size_t i = 0; i < op->subtypes.size(); i++)
                eval->irgen->module
                    ->aliases[eval->irgen->block_hash + fn_name + "::"]
                    .emplace(target_ovl->statement->clause.types[i],
                             op->subtypes[i]);
            size_t og_size = eval->irgen->block_hash.size();
            eval->irgen->block_hash += fn_name + "::";
            auto sig = target_ovl->statement->signature;
            eval->irgen->saturateSignature(sig, op->module);
            eval->irgen->block_hash.erase(og_size, fn_name.size() + 2);

            FunctionDeclaration decl{"",
                                     {},
                                     StatementTreeCloner::copy_stat(
                                         target_ovl->statement->body, nullptr)};
    ...
    if (result.should_sret()) {
        eval->returned_alloc_addr = eval->irgen->builder->write_alloca(
            NativeType::get_size(eval->irgen->toNativeType(result)));
        eval->irgen->builder->write_1b_inst(OpCode::Dup);
    }
    if (!rhs_e.empty()) {
        eval->irgen->builder->write_2b_inst(OpCode::RevStackAddr,
                                            1 + rhs_e.size());
        eval->irgen->builder->write_2b_inst(OpCode::RevStackAddr, 1);
    }

    eval->irgen->builder->write_fn_addr(fn);
    eval->irgen->builder->write_2b_inst(OpCode::Call, 2 + result.should_sret());
    if (result.should_sret()) eval->irgen->builder->write_1b_inst(OpCode::Pop);
    if (result.is_non_owning(eval->irgen)) {
    ...
    if ((p == EQ || p == NE) && left_type_og.is_equal(right_type_og)) {
        if (left_type_og.get_decl_if_enum()) {
            ...
            if (p == EQ)
                ...
                irgen->builder->write_2b_inst(OpCode::CmpNe, 32);
            return {};
        }
    }
    auto& left_type = lhs->evaluated_type;
    auto lhs_e = std::visit(*this, lhs->toVariant());
    implicitConvert(lhs, left_type, target_ovl->left, false, true);

    auto& right_type = rhs->evaluated_type;
...
std::vector<Type> YVMExpressionEvaluator::doSingleStringLiteral(
    const std::string& text, StructNativeTy* str_type) {
    irgen->builder->write_alloca(NativeType::get_size(str_type));
    irgen->builder->write_const<uint64_t>(
        text.size());  // capacity to store in the string
    ...
    irgen->builder->write_1b_inst(OpCode::Switch);
    irgen->builder->write_const_string(irgen->builder->create_const_string(
        text, &reinterpret_cast<YVMEngine*>(irgen->module->engine)->vm));
    irgen->builder->write_2b_inst(OpCode::RevStackAddr, 2);
    irgen->builder->write_1b_inst(OpCode::MemCpy);
    // bring the obj and write the values into it, its at addr 3 because we
    // have the pointer and 2 numbers on stack
...
std::vector<Type> YVMExpressionEvaluator::doUnionVar(CallOperation* op,
                                                     Type& t) {
    ...
    auto arg = std::visit(YVMExpressionEvaluator{irgen},
                          op->arguments[0]->toVariant());
    auto& arg_ty = op->arguments[0]->evaluated_type;
    irgen->builder->write_2b_inst(OpCode::RevStackAddr, 1);
    irgen->builder->write_ptr_off(NativeType::getElementOffset(as_native, 0));
    implicitConvert(op->arguments[0].get(), arg_ty, decl->fields.at(variant),
                    true, false);
    irgen->builder->write_1b_inst(OpCode::Dup);
    irgen->builder->write_ptr_off(NativeType::getElementOffset(as_native, 1));
    irgen->builder->write_const<uint32_t>(
...
void YVMExpressionEvaluator::to_reference(const Type& type) {
    ...
    irgen->builder->write_alloca(
        NativeType::get_size(irgen->toNativeType(type)));
    irgen->builder->write_1b_inst(OpCode::Switch);
    irgen->builder->write_2b_inst(OpCode::RevStackAddr, 1);
    irgen->builder->write_2b_inst(OpCode::Store, irgen->toTypeEnum(type));
}

std::vector<Type> YVMExpressionEvaluator::LValueEvaluator::operator()(
    NameExpression* nm) {
    ...
    for (size_t i = irgen->variables.size(); i > 0; --i) {
        ...
        for (auto& var : irgen->variables[idx]) {
            ...
            if (var.second.idx.type ==
                YVMIRGenerator::VariableIndex::Checkpoint)
                irgen->builder->write_2b_inst(OpCode::StackCheckpoint,
                                              var.second.idx.index);
            else
                irgen->builder->write_2b_inst(OpCode::StackAddr,
                                              var.second.idx.index);
            return {};
        }
    }
...
std::vector<Type> YVMExpressionEvaluator::LValueEvaluator::operator()(
    ...
    if (op->op.type == TokenType::Star) {
        return std::visit(YVMExpressionEvaluator{irgen},
...
std::vector<Type> YVMExpressionEvaluator::operator()(IntegerLiteral* lit) {
    ...
    auto width = *lit->evaluated_type.integer_width();
    switch (width) {
    case 8:
        irgen->builder->write_const(static_cast<uint8_t>(ul));
        break;
    case 16:
        irgen->builder->write_const(static_cast<uint16_t>(ul));
        break;
    case 32:
        irgen->builder->write_const(static_cast<uint32_t>(ul));
        break;
    case 64:
        irgen->builder->write_const(static_cast<uint64_t>(ul));
        break;
    }
    return {};
}
std::vector<Type> YVMExpressionEvaluator::operator()(BooleanLiteral* lit) {
    if (lit->token.text == "true")
        irgen->builder->write_const(uint8_t{1});
    else
        irgen->builder->write_const(uint8_t{0});
    return {};
}
std::vector<Type> YVMExpressionEvaluator::operator()(TupleLiteral* tup) {
    auto native_ty = reinterpret_cast<StructNativeTy*>(
        irgen->toNativeType(tup->evaluated_type));
    irgen->builder->write_alloca(NativeType::get_size(native_ty));
    size_t idx = 0;
    for (auto& expr : tup->elements) {
        irgen->builder->write_1b_inst(OpCode::Dup);
        irgen->builder->write_ptr_off(
            NativeType::getElementOffset(native_ty, idx));
        std::visit(*this, expr->toVariant());
        irgen->builder->write_1b_inst(OpCode::Switch);
        implicitConvert(expr.get(), expr->evaluated_type,
                        tup->evaluated_type.subtypes[idx], true, false);
        irgen->builder->write_1b_inst(OpCode::Pop);
        idx++;
    }
    return {};
...
std::vector<Type> YVMExpressionEvaluator::operator()(ArrayLiteral* lit) {
    ...
    const auto& type = lit->evaluated_type;
    auto as_native =
        reinterpret_cast<ArrayNativeTy*>(irgen->toNativeType(type));
    irgen->builder->write_alloca(NativeType::get_size(as_native));
    size_t ret_addr = irgen->builder->last_alloc_addr();
    if (type.is_static_array()) {
        if (auto rpn_ptr = std::get_if<repeat_notation>(&lit->elements)) {
            for (size_t i = 0; i < type.static_array_size(); i++) {
                irgen->builder->write_1b_inst(OpCode::Dup);
                irgen->builder->write_2b_inst(OpCode::RevStackAddr, 2);
                irgen->builder->write_ptr_off(
                    NativeType::getElementOffset(as_native, i));
                clone(rpn_ptr->first.get(), type.subtypes[0], true, false);
                irgen->builder->write_1b_inst(OpCode::Pop);
            }
        ...
        for (size_t i = 0; i < elements.size(); ++i) {
            ...
            irgen->builder->write_ptr_off(
                NativeType::getElementOffset(as_native, i));
            implicitConvert(elements[i].get(), elements[i]->evaluated_type,
                            type.subtypes[0], true, false);
            irgen->builder->write_1b_inst(OpCode::Pop);
        }
    }
    returned_alloc_addr = ret_addr;
...
std::vector<Type> YVMExpressionEvaluator::operator()(RealLiteral* lit) {
    if (*lit->evaluated_type.float_width() == 32) {
        irgen->builder->write_const(
            std::strtof(lit->token.text.data(), nullptr));
    } else {
        irgen->builder->write_const(
            std::strtod(lit->token.text.data(), nullptr));
    }
    return {};
}
// str is of type {ptr, int64, int64} for buffer, size, capacity
std::vector<Type> YVMExpressionEvaluator::operator()(StringLiteral* lit) {
    auto str_type = reinterpret_cast<StructNativeTy*>(
        irgen->toNativeType(lit->evaluated_type));
    // creating a string is way too many instructions for a fairly common
    // task so we have this special optimized path for the common case of a
    // string without interpolation
    if (lit->literal.size() == 1 &&
        std::holds_alternative<std::string>(lit->literal[0]))
        return doSingleStringLiteral(std::get<std::string>(lit->literal[0]),
                                     str_type);
    ...
    // we can get the final allocation size by adding the values at the
    // stack top we can get the offset by adding the previous offset to the
    // previous size
    irgen->builder->write_const(uint64_t{0});
    irgen->builder->write_1b_inst(OpCode::Dup);
    std::vector<bool> should_free_vec;
    for (auto& substr : lit->literal) {
        if (auto string = std::get_if<std::string>(&substr)) {
            auto str = irgen->builder->create_const_string(
                *string,
                &reinterpret_cast<YVMEngine*>(irgen->module->engine)->vm);
            ...
            auto& expr_tp = expr->evaluated_type;
            if (expr_tp.is_integral() || expr_tp.is_floating_point()) {
                std::visit(*this, expr->toVariant());
                uint8_t intrinsic_idx = 0;
                // we can probably do something fancy like
                // checking the first character for i u or f and then using
                // adding log2 of integer width - 3, we could even use
                // countl_zero over log
                if (expr_tp.name == "i8")
                    intrinsic_idx = 1;
                else if (expr_tp.name == "i16")
                    intrinsic_idx = 2;
                else if (expr_tp.name == "i32")
                    intrinsic_idx = 3;
                else if (expr_tp.name == "i64")
                    intrinsic_idx = 4;
                else if (expr_tp.name == "u8")
                    intrinsic_idx = 5;
                else if (expr_tp.name == "u16")
                    intrinsic_idx = 6;
                else if (expr_tp.name == "u32")
                    intrinsic_idx = 7;
                else if (expr_tp.name == "u64")
                    intrinsic_idx = 8;
                else if (expr_tp.name == "f32")
                    intrinsic_idx = 9;
                else if (expr_tp.name == "f64")
                    intrinsic_idx = 10;
                irgen->builder->write_2b_inst(OpCode::ExternalIntrinsic,
                                              intrinsic_idx);
                irgen->builder->write_2b_inst(OpCode::RevStackAddr, 2);
                irgen->builder->write_2b_inst(OpCode::RevStackAddr, 4);
                irgen->builder->write_1b_inst(OpCode::Add64);
    ...
    irgen->builder->write_2b_inst(OpCode::RevStackAddr,
                                  0);  // offset of last string
    irgen->builder->write_2b_inst(OpCode::RevStackAddr,
                                  2);  // size of last string
    irgen->builder->write_1b_inst(OpCode::Add64);
    ...
    for (auto should_free : should_free_vec | std::views::reverse) {
        ...
        irgen->builder->write_2b_inst(OpCode::RevStackAddr,
                                      2);  // destinations
        irgen->builder->write_2b_inst(OpCode::RevStackAddr,
                                      4);  // the offset of the current string
        ...
        irgen->builder->write_1b_inst(
            OpCode::Switch);  // bring the pointer forward
        if (should_free)
...
std::vector<Type> YVMExpressionEvaluator::operator()(NameExpression* nm) {
    ...
    for (size_t i = irgen->variables.size(); i > 0; --i) {
        ...
        for (auto& var : irgen->variables[idx]) {
            ...
            if (!var.second.type.should_sret()) {
                irgen->builder->write_2b_inst(
                    OpCode::Load, irgen->toTypeEnum(var.second.type));
            }
            if (!type.is_lvalue && !type.is_trivially_destructible(irgen)) {
    ...
    if (type.is_integral() || type.is_char() || type.is_boolean() ||
...
std::vector<Type> YVMExpressionEvaluator::operator()(
...
std::vector<Type> YVMExpressionEvaluator::operator()(PrefixOperation* op) {
    ...
    case TokenType::Ampersand: {
        ...
        if (!target.subtypes[0].should_sret()) {
            irgen->builder->write_alloca(
                NativeType::get_size(irgen->toNativeType(target.subtypes[0])));
            irgen->builder->write_1b_inst(OpCode::Switch);
            irgen->builder->write_2b_inst(OpCode::RevStackAddr, 1);
            irgen->builder->write_2b_inst(
                OpCode::Store, irgen->toTypeEnum(target.subtypes[0]));
            return {};
        }
        // taking the address of the rvalue causes the lifetime to be
        // extended till the ref dies
        if (!target.deref().is_trivially_destructible(irgen) &&
            !target.deref().is_lvalue) {
            // we use a dup because lifetime extensions expect on the stack
            // top and one object below that for each element in the
            // extension vector
            irgen->builder->write_1b_inst(OpCode::Dup);
            return {target.deref()};
        }
...
std::vector<Type> YVMExpressionEvaluator::operator()(BinaryOperation* op) {
    ...
    auto lhs = op->lhs.get();
    auto rhs = op->rhs.get();
    // an overload is selected by the type checker for everything except dot
    // and equal
    if (op->selected) {
        // comparison has special needs
        switch (op->op.type) {
...
std::vector<Type> YVMExpressionEvaluator::operator()(PostfixOperation*) {
    return {};
}
std::vector<Type> YVMExpressionEvaluator::fillArgs(
    ...
    if (first_expr) {
        auto& tp = first_expr->evaluated_type;
    ...
    for (size_t i = 0; i < exprs.size(); i++) {
        ...
        auto this_uses_elf =
            !extended_lifetimes
                 .emplace_back(std::visit(*this, exprs[i]->toVariant()))
                 .empty();
        using_elf = using_elf ? true : this_uses_elf;

        implicitConvert(exprs[i].get(), tp, sig[i + is_bound], false, true);
    }
    if (!using_elf) {
        return {};
    }
    // at least one function argument is performing temporary lifetime
    // extension so there's gaps in the stack and we need to bring all args
    // together we also need to return all the extended lifetimes in order
    ...
    for (size_t i = extended_lifetimes.size(); i > 0; i--) {
        ...
        // we add extended_lifetimes.size() - i for the amount of new values
        // we added via this process i.e the rev stack addr
        auto s_addr = idx + extended_lifetimes.size() - i;
        for (const auto& elf :
             std::ranges::subrange(rev_view.begin(), rev_view.begin() + idx)) {
            s_addr += elf.size();
        }
...
std::vector<Type> YVMExpressionEvaluator::doInvoke(CallOperation* op,
                                                   const Type& left_t) {
    return {};
}
...
std::vector<Type> YVMExpressionEvaluator::operator()(CallOperation* op) {
    ...
    auto& callee_t = op->callee->evaluated_type;
    ...
    if (op->callee->evaluated_type.name == "__bound_fn") {
        ...
        if (left_t.deref().is_view()) {
            ...
            if (auto* name_expr =
                ...
                //     irgen->builder->CreateStructGEP(as_llvm_type, value,
                //     0));
                // auto callee = irgen->builder->CreateLoad(pointer_type,
                //     irgen->builder->CreateStructGEP(as_llvm_type, value,
                //     1 + std::distance(interface->methods.begin(), it)));

                // llvm::Value* return_value = fillArgs(uses_sret, t->sig,
                // args, param, op->arguments);
        ...
        auto& right_t = expr->rhs->evaluated_type;
        ...
        // if(is_lambda) args.push_back(std::visit(*this,
        // expr->rhs->toVariant()));
        irgen->builder->write_2b_inst(OpCode::Call,
    ...
    bool uses_sret = return_t.should_sret();
    auto extensions = fillArgs(uses_sret, callee_t.subtypes, {}, op->arguments);
...
std::vector<Type> YVMExpressionEvaluator::operator()(SubscriptOperation* op) {
    ...
    auto& idx_ty = op->index->evaluated_type;

    //========array indices are handled specially============
    // it can be a part of the type system but would cause many generic
    // instantiations
    if (expr_ty.deref().is_array() && idx_ty.is_integral() &&
        !idx_ty.is_signed_integral()) {
        // we could have evaluated this after the index, but the evaluation
        // order matters (used for the move semantics)
        std::visit(*this, op->object->toVariant());
        // integer literals
        std::visit(*this, op->index->toVariant());
        // integer literals don't need the implicit conversion
        if (idx_ty.is_unsigned_integral())
            implicitConvert(op->index.get(), idx_ty, Type{.name = "u64"}, false,
                            true);
        ...
        if (expr_ty.deref().is_static_array()) {
            auto elem_size = NativeType::getElementOffset(
                reinterpret_cast<ArrayNativeTy*>(as_native), 1);
    ...
    if (expr_ty.is_mutable || expr_ty.is_mutable_reference()) {
        ...
        std::tie(block, ovl) =
            resolveIdxMut(expr_ty.mutable_reference_to(), idx_ty, irgen);
    }
    std::ignore = tok;
    // if there is no mutable overload still default to the non mutable
    // overload
    if (!ovl) {
        std::tie(block, ovl) =
            resolveIdx(expr_ty.reference_to(), idx_ty, irgen);
        tok = TokenType::SquarePair;
    }
    // TODO
    return {};  // doBasicBinaryOp(this, ovl, tok, op->object.get(),
                // op->index.get(), expr_ty, idx_ty);
}
std::vector<Type> YVMExpressionEvaluator::operator()(GCNewExpression* expr) {
    auto& gc_tp = expr->evaluated_type;

    auto gc_native = reinterpret_cast<StructNativeTy*>(
        reinterpret_cast<YVMEngine*>(irgen->module->engine)
            ->struct_manager.get_struct_type({
                {irgen->toNativeType(gc_tp.deref()), NativeType::getI64()}
    }));
    irgen->builder->write_const(NativeType::get_size(gc_native));
    irgen->builder->write_2b_inst(OpCode::ExternalIntrinsic,
                                  0);  // gcmalloc instrinsic
    std::visit(*this, expr->target_expression->toVariant());
    irgen->builder->write_2b_inst(OpCode::RevStackAddr, 1);
    irgen->builder->write_ptr_off(NativeType::getElementOffset(gc_native, 0));
    auto& internal_ty = expr->target_expression->evaluated_type;
    implicitConvert(expr->target_expression.get(), internal_ty,
                    gc_tp.subtypes[0], true,
                    false);  // write the object into the gc memory
    irgen->builder->write_1b_inst(OpCode::Pop);
    irgen->builder->write_const<uint64_t>(0);
    irgen->builder->write_2b_inst(OpCode::RevStackAddr, 1);
...
std::vector<Type> YVMExpressionEvaluator::operator()(SpawnExpression* expr) {
    ...
    params_ty = reinterpret_cast<StructNativeTy*>(
        reinterpret_cast<YVMEngine*>(irgen->module->engine)
            ->struct_manager.get_struct_type(types));
    NativeTy* ret_ty = irgen->toNativeType(fn.sig.returnType);
    bool uses_sret = fn.sig.returnType.should_sret();

    auto fiber_ty =
        reinterpret_cast<StructNativeTy*>(irgen->toNativeType(*return_t));
    ...
    if (fn.is_bound) {
        ...
        if (auto rhs = dynamic_cast<NameExpression*>(bexpr->rhs.get())) {
            ...
            if (block == this_block && fn->sig.parameters[0].name == "this") {
                ...
                irgen->builder->write_ptr_off(NativeType::getElementOffset(
                    fiber_ty, 0));  // get the parameters pointer
                ...
                std::visit(
                    *this,
                    bexpr->lhs->toVariant());  // no need to remove lifetime
                                               // extensions because borrow
                                               // is not allowed
                irgen->builder->write_1b_inst(OpCode::Switch);
                implicitConvert(bexpr->lhs.get(), *left_t,
                                fn->sig.parameters[0].type, true, false);
                irgen->builder->write_1b_inst(OpCode::Pop);

                for (auto i : std::views::iota(1u, fn->sig.parameters.size())) {
                    if (i != fn->sig.parameters.size() - 1)
                        irgen->builder->write_1b_inst(OpCode::Dup);
                    ...
                    implicitConvert(
                        ...
                        *std::visit(type_checker,
                                    call_op->arguments[i - 1]->toVariant()),
...
std::vector<Type> YVMExpressionEvaluator::operator()(BlockExpression* expr) {
    irgen->pushScope();
    irgen->used_types.emplace_back();

    for (auto& sub_stat : expr->statements) {
        irgen->current_Statement = &sub_stat;

        std::visit(*irgen, sub_stat->toVariant());

        if (dynamic_cast<ReturnStatement*>(sub_stat.get())) {
            irgen->used_types.pop_back();
            irgen->popScope();
            return {};
        }
    }
    auto ret = expr->expr ? std::visit(*this, expr->expr->toVariant())
                          : std::vector<Type>{};
    irgen->used_types.pop_back();
    irgen->popScope();
    return ret;
...
std::vector<Type> YVMExpressionEvaluator::operator()(IfExpression* expr) {
    ...
    if (!std::visit(*this, expr->then_expr->toVariant()).empty()) {
        irgen->error(
            Error(expr->then_expr.get(),
                  "Lifetime extensions are not allowed in if expressions"));
    };
    if (expr->else_expr) {
...
std::vector<Type> YVMExpressionEvaluator::operator()(
    ConditionalExtraction* expr) {
    std::visit(*this, expr->condition->toVariant());
    auto& cond_tp = expr->condition->evaluated_type;
    auto cond_native =
        reinterpret_cast<StructNativeTy*>(irgen->toNativeType(cond_tp));
    // all types that do conditional extraction are of the form {data,
    // is_valid}
    irgen->builder->write_1b_inst(OpCode::Dup);
    irgen->builder->write_ptr_off(NativeType::getElementOffset(cond_native, 1));
    irgen->builder->write_2b_inst(OpCode::Load,
                                  Yvm::Type::u8);  // load the is_valid bool
    ...
    if (!cond_tp.is_value_conversion_result() &&
        !(expr->then_capture_tp != ConditionalExtraction::Own)) {
        if (cond_tp.is_ref_conversion_result())
            irgen->builder->write_2b_inst(OpCode::Load, Yvm::Type::ptr);
        if (!cond_tp.subtypes[0].should_sret())
            irgen->builder->write_2b_inst(
                OpCode::Load, irgen->toTypeEnum(cond_tp.subtypes[0]));
        clone(expr->condition.get(), cond_tp.subtypes[0], false, false);
    }
    Type variable_type =
        expr->then_capture_tp != ConditionalExtraction::Own
            ? Type{expr->then_capture_tp == ConditionalExtraction::RefMut
                       ? "__ref_mut"
                       : "__ref",
                   {cond_tp.subtypes[0]}}
            : cond_tp.subtypes[0];
    variable_type.saturate(irgen->module, irgen);
    irgen->variables.back().emplace_back(
        expr->captured_name, YVMIRGenerator::VariableEntry{
                                 YVMIRGenerator::VariableIndex{
                                                               irgen->builder->checkpoint(),
                                                               YVMIRGenerator::VariableIndex::Checkpoint},
...
std::vector<Type> YVMExpressionEvaluator::operator()(LambdaExpression* expr) {
    // for lambda expressions, every lambda has a different type this
    // generates the type name
    std::string lambda_ty =
        "__lambda" + irgen->block_hash +
        std::to_string(reinterpret_cast<std::uintptr_t>(expr));
    ...
    if (!expr->captures.empty()) {
        ...
        for (auto& capture : expr->captures) {
            ...
            // references cannot be captured as non owning because `&&` does
            // not exist(not yet?)
            if (type->is_reference() && capture.cp_type != Ownership::Owning) {
                irgen->error(
                    Error(expr, "Cannot capture reference as non owning"));
            }
            context_types.push_back(capture.cp_type == Ownership::Owning
                                        ? irgen->toNativeType(*type)
                                        : NativeType::getPtrTy());
        }
    }
...
std::vector<Type> YVMExpressionEvaluator::operator()(ScopeOperation* op) {
    ...
    if (ty.is_function()) {
        ...
        // for functions the block hash contains the function full path and
        // "::"
    ...
    //     llvm::ConstantInt::get(as_llvm, val); else return
    //     static_cast<llvm::ConstantInt*>(nullptr);
    // }, cnst.internal_repr);
    // else return
    // reinterpret_cast<llvm::Constant*>(std::get<void*>(cnst.internal_repr));
}

std::vector<Type> YVMExpressionEvaluator::operator()(ObjectLiteral* lit) {
    auto& type = lit->evaluated_type;
    auto as_native_type =
        reinterpret_cast<StructNativeTy*>(irgen->toNativeType(type));
    auto decl = type.get_decl_if_class(irgen);
    ...
    for (size_t i = 0; i < decl->vars.size(); i++) {
        ...
        std::visit(YVMExpressionEvaluator{irgen},
                   lit->values[var.name]->toVariant());
        irgen->builder->write_2b_inst(OpCode::RevStackAddr, 1);
        irgen->builder->write_ptr_off(
            NativeType::getElementOffset(as_native_type, i));
        implicitConvert(lit->values[var.name].get(),
                        lit->values[var.name]->evaluated_type, var.type, true,
                        false);
        irgen->builder->write_1b_inst(OpCode::Pop);
    }
...
std::vector<Type> YVMExpressionEvaluator::operator()(AsExpression* expr) {
    auto& ty = expr->expr->evaluated_type;
    auto& final_ty = expr->evaluated_type;
    auto final_as_native =
        reinterpret_cast<StructNativeTy*>(irgen->toNativeType(final_ty));
    ...
    irgen->builder->CreateStructGEP(internal_as_llvm, internal, 0);
        ...
        auto is_valid_ptr = irgen->builder->CreateStructGEP(as_llvm, val,
    1); auto idx_ptr = irgen->builder->CreateStructGEP(internal_as_llvm,
    internal, 1);

        auto i32_ty = llvm::Type::getInt32Ty(irgen->context);
        auto this_idx = llvm::ConstantInt::get(i32_ty,
    std::distance(subtypes.begin(), subtypes.find(expr->dest))); auto
    correct_idx = irgen->builder->CreateLoad(i32_ty, idx_ptr);
    ...
    if (final_ty->is_view())
    {
        ...
        auto [name, intf] = viewed.module->findInterface(viewed.block_hash,
    viewed.name); std::string full_name = name + intf->name; if (auto cls =
    ty->deref().get_decl_if_class(irgen))
        {
            auto details =
    ty->deref().module->findClass(ty->deref().block_hash,
    ty->deref().name).second; auto result = irgen->Alloca("interface_cast",
    as_llvm); auto this_ptr = irgen->builder->CreateStructGEP(as_llvm,
    result, 0); irgen->builder->CreateStore(internal, this_ptr); size_t idx
    = 0; for (auto& method : intf->methods)
            {
                idx++;
                auto fn_ptr = irgen->builder->CreateStructGEP(as_llvm,
    result, idx); std::string method_name = std::get<0>(*details) +
    "__interface" + full_name + "__%" + method->name; auto fn =
    irgen->code->getFunction(method_name); irgen->builder->CreateStore(fn,
    fn_ptr);
            }
            return result;
        }
    ...
    if (auto decl = ty.get_decl_if_union()) {
        ...
        irgen->builder->write_alloca(
            NativeType::get_size(final_as_native));     // stack: union, final
        irgen->builder->write_1b_inst(OpCode::Switch);  // stack: final, union
        irgen->builder->write_1b_inst(
            OpCode::Dup);  // stack: final, union, union
        irgen->builder->write_ptr_off(NativeType::getElementOffset(
            as_native, 1));  // stack: final, union, union_idx_ptr
        irgen->builder->write_2b_inst(
            OpCode::Load,
            Yvm::Type::u32);  // stack: final, union, union_idx
        irgen->builder->write_const(
            idx);  // stack: final, union, union_idx, target_idx
        irgen->builder->write_2b_inst(OpCode::CmpEq,
                                      32);  // stack: final, union, is_idx_valid
        irgen->builder->write_2b_inst(
            OpCode::RevStackAddr,
            2);  // stack: final, union, is_idx_valid, final
        irgen->builder->write_ptr_off(NativeType::getElementOffset(
            final_as_native,
            1));  // stack: final, union, is_valid_idx, final_idx
        irgen->builder->write_2b_inst(OpCode::Store,
                                      Yvm::Type::u32);  // stack: final, union
        irgen->builder->write_ptr_off(NativeType::getElementOffset(
            as_native, 0));  // stack: final, union_data_ptr
        irgen->builder->write_2b_inst(
            OpCode::RevStackAddr,
            1);  // stack: final, union_data_ptr, final
        irgen->builder->write_2b_inst(OpCode::Store, Yvm::Type::ptr);
