...
void YVMAppModule::addFunction(FunctionSignature sig, void* func,
                               std::string name) {
    ...
    if (sig.returnType.should_sret()) {
        ...
        ret_ty = toNativeType(sig.returnType, module_hash, nullptr, {});

    for (auto i : std::views::iota(size_t{0}, sig.parameters.size()))
        args[i] = sig.parameters[i].type.should_sret()
                      ? NativeType::getPtrTy()
                      : toNativeType(sig.parameters[i].type, module_hash,
                                     nullptr, {});
    ...
    em.write_const(func);
    em.write_2b_inst(Yvm::OpCode::NativeCall,
                     sig.parameters.size() + sig.returnType.should_sret());
