...
namespace Yoyo {
extern "C" {
YOYO_API char* YOYO_to_string_signed_integer_dont_use_name(
    int64_t value, size_t* buffer_length);
YOYO_API char* YOYO_to_string_unsigned_integer_dont_use_name(
    uint64_t value, size_t* buffer_length);
YOYO_API char* YOYO_to_string_float32_dont_use_name(float value,
                                                    size_t* buffer_length);
YOYO_API char* YOYO_to_string_float64_dont_use_name(double value,
                                                    size_t* buffer_length);
YOYO_API char* YOYO_to_string_enum_dont_use_name(int32_t value,
                                                 EnumDeclaration* decl,
                                                 size_t* buffer_length);
char* YOYO_to_string_signed_integer_dont_use_name(int64_t value,
                                                  size_t* buffer_length) {
...
char* YOYO_to_string_unsigned_integer_dont_use_name(uint64_t value,
                                                    size_t* buffer_length) {
    *buffer_length = std::formatted_size("{}", value);
    auto buffer = static_cast<char*>(malloc(*buffer_length));
    std::format_to(buffer, "{}", value);
...
char* YOYO_to_string_float64_dont_use_name(double value,
                                           size_t* buffer_length) {
    *buffer_length = std::formatted_size("{}", value);
    auto buffer = static_cast<char*>(malloc(*buffer_length));
    std::format_to(buffer, "{}", value);
...
char* YOYO_to_string_enum_dont_use_name(int32_t value, EnumDeclaration* decl,
                                        size_t* buffer_length) {
    auto& size = *buffer_length;
    std::string str = "<unknown>";
    auto it = std::ranges::find_if(
        decl->values, [value](std::pair<const std::string, int32_t>& val) {
...
std::pair<llvm::Value*, llvm::Value*> sint_to_str(llvm::Value* integer,
                                                  IRGenerator* irgen) {
    auto fn =
        irgen->code->getFunction("YOYO_to_string_signed_integer_dont_use_name");
    ...
    if (!fn) {
        ...
        fn = llvm::Function::Create(
            ...
            "YOYO_to_string_signed_integer_dont_use_name", irgen->code);
    }
    auto mem = irgen->builder->CreateCall(fn, {integer, size});
    return {mem, irgen->builder->CreateLoad(int64_ty, size)};
}
std::pair<llvm::Value*, llvm::Value*> uint_to_str(llvm::Value* integer,
                                                  IRGenerator* irgen) {
    auto fn = irgen->code->getFunction(
        "YOYO_to_string_unsigned_integer_dont_use_name");
    auto int64_ty = llvm::Type::getInt64Ty(irgen->context);
    auto ptr_ty = llvm::PointerType::get(irgen->context, 0);
    auto size = irgen->Alloca("int_as_str_size", int64_ty);
    if (!fn) {
        ...
        fn = llvm::Function::Create(
            type, llvm::GlobalValue::ExternalLinkage,
            "YOYO_to_string_unsigned_integer_dont_use_name", irgen->code);
    }
    auto mem = irgen->builder->CreateCall(fn, {integer, size});
    return {mem, irgen->builder->CreateLoad(int64_ty, size)};
}
std::pair<llvm::Value*, llvm::Value*> f32_to_str(llvm::Value* value,
                                                 IRGenerator* irgen) {
    ...
    if (!fn) {
        ...
        fn = llvm::Function::Create(type, llvm::GlobalValue::ExternalLinkage,
                                    "YOYO_to_string_float32_dont_use_name",
                                    irgen->code);
    }
    auto mem = irgen->builder->CreateCall(fn, {value, size});
    return {mem, irgen->builder->CreateLoad(int64_ty, size)};
}
std::pair<llvm::Value*, llvm::Value*> f64_to_str(llvm::Value* value,
                                                 IRGenerator* irgen) {
    ...
    if (!fn) {
        ...
        fn = llvm::Function::Create(type, llvm::GlobalValue::ExternalLinkage,
                                    "YOYO_to_string_float64_dont_use_name",
                                    irgen->code);
    }
    auto mem = irgen->builder->CreateCall(fn, {value, size});
    return {mem, irgen->builder->CreateLoad(int64_ty, size)};
...
llvm::Function* enum_to_string_fn(IRGenerator* irgen) {
    ...
    if (!fn) {
        ...
        auto ptr_ty = llvm::PointerType::get(irgen->context, 0);
        auto type =
            llvm::FunctionType::get(ptr_ty, {int32_ty, ptr_ty, ptr_ty}, false);
        fn = llvm::Function::Create(type, llvm::GlobalValue::ExternalLinkage,
                                    "YOYO_to_string_enum_dont_use_name",
                                    irgen->code);
    }
    return fn;
}
std::pair<llvm::Value*, llvm::Value*> ExpressionEvaluator::doToStr(
    llvm::Value* val, const Type& tp) {
    if (tp.is_str()) {
        ...
        return {irgen->builder->CreateLoad(
                    llvm::PointerType::get(irgen->context, 0), ptr),
                irgen->builder->CreateLoad(
                    llvm::Type::getInt64Ty(irgen->context), size)};
    }
    if (tp.is_signed_integral()) {
        auto as64 = irgen->builder->CreateSExt(
            val, llvm::Type::getInt64Ty(irgen->context));
        return sint_to_str(as64, irgen);
    }
    if (tp.is_unsigned_integral()) {
        auto as64 = irgen->builder->CreateZExt(
            val, llvm::Type::getInt64Ty(irgen->context));
        return uint_to_str(as64, irgen);
    }
    if (tp.name == "f32") return f32_to_str(val, irgen);
    if (tp.name == "f64") return f64_to_str(val, irgen);
    if (tp.is_char()) {
        ...
        for (unsigned i = 0; i < 4; ++i) {
            ...
            auto byte = irgen->builder->CreateLoad(
                i8, irgen->builder->CreateGEP(i8, copy, {idx_const}));
            size = irgen->builder->CreateAdd(
                size, irgen->builder->CreateZExt(
                          irgen->builder->CreateICmpNE(
                              byte, llvm::ConstantInt::get(i8, 0)),
                          i64));
        }
        auto addr = irgen->Malloc("char_tmp", size);
        irgen->builder->CreateMemCpy(addr, std::nullopt, copy, std::nullopt,
                                     size);
        return {addr, size};
    }
    if (auto decl = tp.get_decl_if_enum()) {
        auto size = irgen->Alloca("enum_to_str_size",
                                  llvm::Type::getInt64Ty(irgen->context));
        auto fn = enum_to_string_fn(irgen);
        auto llvm_decl = irgen->builder->CreateIntToPtr(
            llvm::ConstantInt::get(llvm::Type::getInt64Ty(irgen->context),
        ...
        auto sz = irgen->builder->CreateLoad(
            llvm::Type::getInt64Ty(irgen->context), size);
        return {addr, sz};
    }
    if (tp.is_optional()) {
        ...
        auto as_llvm = irgen->ToLLVMType(tp, false);
        auto if_null = llvm::BasicBlock::Create(irgen->context, "opt_is_null",
                                                fn, irgen->returnBlock);
        auto if_valid = llvm::BasicBlock::Create(irgen->context, "opt_is_valid",
                                                 fn, irgen->returnBlock);
        auto cont = llvm::BasicBlock::Create(irgen->context, "opt_str_cont", fn,
                                             irgen->returnBlock);

        auto is_valid = irgen->builder->CreateLoad(
        ...
        if (!tp.subtypes[0].should_sret()) {
            auto subtype =
                llvm::dyn_cast<llvm::StructType>(as_llvm)->getElementType(0);
        ...
        auto size =
            llvm::ConstantInt::get(llvm::Type::getInt64Ty(irgen->context), 4);
        auto ptr = irgen->Malloc("null_str", size);
        irgen->builder->CreateMemCpy(ptr, std::nullopt, null_str, std::nullopt,
                                     size);
        irgen->builder->CreateBr(cont);

        irgen->builder->SetInsertPoint(cont);
        auto ptr_phi = irgen->builder->CreatePHI(
            llvm::PointerType::get(irgen->context, 0), 2);
        auto value_phi = irgen->builder->CreatePHI(
            llvm::Type::getInt64Ty(irgen->context), 2);
    ...
    if (tp.is_tuple()) {
        ...
        for (auto& sub : tp.subtypes) {
            ...
            if (!sub.should_sret())
                sub_val = irgen->builder->CreateLoad(
                    irgen->ToLLVMType(sub, false), sub_val);
            args.push_back(doToStr(sub_val, sub));
            idx++;
        }
        // target output is (val, val2, val3...),
        // hence the total size is the size of inputs plus 2(, ) for every
        // input other than the first plus 2 for the open and close bracket
        auto total_size = irgen->builder->CreateAdd(
            args[0].second,
            llvm::ConstantInt::get(llvm::Type::getInt64Ty(irgen->context), 2));
        for (const auto& arg :
             std::ranges::subrange(args.begin() + 1, args.end())) {
            total_size = irgen->builder->CreateAdd(
                total_size, irgen->builder->CreateAdd(
        ...
        irgen->builder->CreateStore(
            llvm::ConstantInt::get(llvm::Type::getInt8Ty(irgen->context), '('),
            memory);
        llvm::Value* offset =
        ...
        irgen->builder->CreateMemCpy(current_ptr, std::nullopt, args[0].first,
                                     std::nullopt, args[0].second);
        ...
        for (auto& arg : std::ranges::subrange(args.begin() + 1, args.end())) {
            ...
            irgen->builder->CreateStore(llvm::ConstantInt::get(i8_ty, ','),
                                        current_ptr);
            ...
            offset = irgen->builder->CreateAdd(offset, const_one);
            current_ptr = irgen->builder->CreateGEP(i8_ty, memory, offset);
            irgen->builder->CreateMemCpy(current_ptr, std::nullopt, arg.first,
                                         std::nullopt, arg.second);
    ...
    if (tp.is_boolean()) {
        ...
        auto is_true = llvm::BasicBlock::Create(
            irgen->context, "bool_to_str_true", fn, irgen->returnBlock);
        auto cont = llvm::BasicBlock::Create(irgen->context, "bool_to_str_cont",
                                             fn, irgen->returnBlock);
        ...
        irgen->builder->SetInsertPoint(cont);
        auto i64 = llvm::Type::getInt64Ty(irgen->context);
        auto constant = irgen->builder->CreatePHI(
            llvm::PointerType::get(irgen->context, 0), 2);
        ...
        irgen->builder->CreateMemCpy(memory, std::nullopt, constant,
                                     std::nullopt, size);
        return {memory, size};
    }
    debugbreak();
