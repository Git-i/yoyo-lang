...
#include "engine.h"
namespace Yoyo {
std::pair<std::string, ModuleBase::FunctionDetails*> ModuleBase::findFunction(
...
std::pair<std::string,
    ...
    ModuleBase::findConst(const std::string& block, const std::string& name) {
...
std::pair<std::string, GenericFunctionDeclaration*> ModuleBase::findGenericFn(
    const std::string& block, const std::string& name) {
...
Type* ModuleBase::findAlias(const std::string& block, const std::string& name) {
    // Aliases require special behaviour because they can shadow each other
    // (only "This" is capable of this)
    std::vector<std::pair<std::string, Type*>> found_aliases;
    for (auto& [hash, details_list] : aliases) {
        if (!block.starts_with(hash)) continue;
        if (details_list.contains(name))
            found_aliases.emplace_back(hash, &details_list.at(name));
    }
    if (found_aliases.empty()) return nullptr;
    // we pick the one with the longest name (i.e the deepest block)
    return std::ranges::max(found_aliases, {},
                            [](auto& elem) { return elem.first.length(); })
        .second;
}
std::pair<std::string, InterfaceDeclaration*> ModuleBase::findInterface(
    const std::string& block, const std::string& name) {
...
std::pair<std::string, EnumDeclaration*> ModuleBase::findEnum(
    const std::string& block, const std::string& name) {
...
std::pair<std::string, UnionDeclaration*> ModuleBase::findUnion(
    const std::string& block, const std::string& name) {
...
MacroDeclaration* ModuleBase::findMacro(const std::string& block,
...
std::pair<std::string, GenericInterfaceDeclaration*>
    ModuleBase::findGenericInterface(const std::string& block,
                                     const std::string& name) {
...
std::pair<std::string, GenericAliasDeclaration*> ModuleBase::findGenericAlias(
    const std::string& block, const std::string& name) {
    for (auto& [hash, details_list] : generic_aliases) {
        if (!block.starts_with(hash)) continue;
        auto it = std::ranges::find_if(details_list, [&name](auto& details) {
...
std::pair<std::string,
    ...
    ModuleBase::findClass(const std::string& block, const std::string& name) {
...
std::pair<std::string, GenericClassDeclaration*> ModuleBase::findGenericClass(
    const std::string& block, const std::string& name) {
...
std::optional<std::string> ModuleBase::hashOf(const std::string& block,
                                              const std::string& name) {
    for (auto& [hash, details_list] : classes) {
        if (!block.starts_with(hash)) continue;
    ...
    if (auto [hash, intf] = findGenericInterface(block, name); intf)
        return hash;
