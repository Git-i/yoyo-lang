...
std::optional<Token> Scanner::NextToken() {
    while (!IsEof()) {
        ...
        case '+':
            return NextIs('=') ? Token{TokenType::PlusEqual, loc, {source.data() + position - 2, 2} } : 
        ...
        case '-': {
            if (NextIs('='))
                return Token{
                    ...
                    loc,
                    {source.data() + position - 2, 2}
                };
            if (NextIs('>'))
                return Token{
                    TokenType::Arrow, loc, {source.data() + position - 2, 2}
                };
            return Token{
                TokenType::Minus, loc, {source.data() + position - 1, 1}
            };
        }
        case '*':
            return NextIs('=') ? Token{TokenType::StarEqual, loc, {source.data() + position - 2, 2} } :
        ...
        case '%':
            return NextIs('=') ? Token{TokenType::PercentEqual, loc, {source.data() + position - 2, 2} } :
                Token{TokenType::Percent, loc, {source.data() + position - 1, 1} };
        case '!':
            return NextIs('=') ? Token{TokenType::BangEqual, loc, {source.data() + position - 2, 2} } : 
        ...
        case '&': {
            ...
            return Token{
                TokenType::Ampersand, loc, {source.data() + position - 1, 1}
            };
        }
        case '|': {
            if (NextIs('='))
            ...
            return Token{
                TokenType::Pipe, loc, {source.data() + position - 1, 1}
            };
        }
        case '>': {
            if (NextIs('='))
            ...
            return Token{
                TokenType::Greater, loc, {source.data() + position - 1, 1}
            };
        }
        case '<': {
            if (NextIs('=')) {
            ...
            return Token{
                TokenType::Less, loc, {source.data() + position - 1, 1}
            };
        }
        case '(':
            return Token{
                TokenType::LParen, loc, {source.data() + position - 1, 1}
            };
        case ')':
            return Token{
                TokenType::RParen, loc, {source.data() + position - 1, 1}
            };
        case '{':
            return Token{
                TokenType::LCurly, loc, {source.data() + position - 1, 1}
            };
        case '}':
            return Token{
                TokenType::RCurly, loc, {source.data() + position - 1, 1}
            };
        case '[':
            return Token{
                TokenType::LSquare, loc, {source.data() + position - 1, 1}
            };
        case ']':
            return Token{
                TokenType::RSquare, loc, {source.data() + position - 1, 1}
            };
        case '.': {
            if (NextIs('.')) {
                if (NextIs('='))
                    return Token{
                        TokenType::DoubleDotEqual,
                        loc,
                        {source.data() + position - 3, 3}
                    };
                return Token{
            ...
            return Token{
                TokenType::Dot, loc, {source.data() + position - 1, 1}
            };
        }
        case ',':
            return Token{
                TokenType::Comma, loc, {source.data() + position - 1, 1}
            };
        case ':': {
            if (NextIs(':')) {
                if (NextIs('<'))
            ...
            return Token{
                TokenType::Colon, loc, {source.data() + position - 1, 1}
            };
        }
        case '^':
            return Token{
                TokenType::Caret, loc, {source.data() + position - 1, 1}
            };
        case '#': {
            if (NextIs('('))
                return Token{
        ...
        case '?': {
            return Token{
                TokenType::Question, loc, {source.data() + position - 1, 1}
            };
        }
        case '\t':
            [[fallthrough]];
        ...
        case '=': {
            ...
            return Token{
                TokenType::Equal, loc, {source.data() + position - 1, 1}
            };
        }
        case '"':
        ...
        case '\\': {
            if (NextIs('}'))
                return Token{
                    TokenType::SlashBrace,
                    loc,
                    {source.data() + position - 2, 2}
                };
            return Token{
                TokenType::BackSlash, loc, {source.data() + position - 1, 1}
            };
        }
        default: {
...
Token Scanner::ScanIdentifier() {
    ...
    while (!IsEof() && (std::isalnum(Peek()) || Peek() == '_'))
        std::ignore = Get();
    auto view = std::string_view(source.begin() + iden_begin,
    ...
    if (!IsEof() && Peek() == '!') {
        std::ignore = Get();
        return {TokenType::Directive, loc, view};
    };
...
Token Scanner::ScanNumber() {
    ...
    if (IsEof() || Peek() != '.' || position + 1 >= source.size() ||
        !std::isdigit(PeekNext()))
        return {TokenType::IntegerLiteral, loc,
                std::string_view(source.begin() + number_begin,
                                 source.begin() + position)};
...
std::optional<Token> Scanner::ScanCharLiteral() {
    ...
    view = std::string_view{source.begin() + begin,
                            source.begin() + begin + num_bytes};
    if (Peek() == '\'') {
        std::ignore = Get();
...
SourceLocation Scanner::GetSourceLocation() const {
...
char Scanner::Peek() const { return source[position]; }
char Scanner::PeekNext() const { return source[position + 1]; }
