...
namespace Yoyo {
std::unique_ptr<Expression> BinaryOperationParselet::parse(
    Parser& parser, std::unique_ptr<Expression> left, Token tk) {
    ...
    if (tk.type == TokenType::Dot && parser.discard(TokenType::Try)) {
        ...
        return Expression::attachSLAndParent(
            std::make_unique<TryExpression>(std::move(left)), bg,
            parser.discardLocation, parser.parent);
    }
    auto fn = [&parser](std::unique_ptr<BinaryOperation>&& e) {
        e->lhs->parent = e.get();
        e->rhs->parent = e.get();
        e->parent = parser.parent;
        e->beg = e->lhs->beg;
        e->end = e->rhs->end;
        return e;
    };
    return fn(std::make_unique<BinaryOperation>(
        tk, std::move(left), parser.parseExpression(prec - prec_diff)));
}
std::unique_ptr<Expression> LogicalOperationParselet::parse(
    Parser& parser, std::unique_ptr<Expression> left, Token tk) {
    const auto prec_diff = is_right ? 1 : 0;
    ...
    return fn(std::make_unique<LogicalOperation>(
        tk, std::move(left), parser.parseExpression(prec - prec_diff)));
}
std::unique_ptr<Expression> PostfixOperationParselet::parse(
    Parser& parser, std::unique_ptr<Expression> left, Token tk) {
    auto fn = [&parser](std::unique_ptr<PostfixOperation>&& e) {
        e->operand->parent = e.get();
        e->parent = parser.parent;
        e->beg = e->operand->beg;
        e->end = SourceLocation{e->beg.line, e->beg.column + e->op.text.size()};
        return e;
    };
    return fn(std::make_unique<PostfixOperation>(tk, std::move(left)));
}
std::unique_ptr<Expression> CallOperationParselet::parse(
    Parser& parser, std::unique_ptr<Expression> left, Token tk) {
    auto fn = [&parser](std::unique_ptr<CallOperation>&& e) {
        e->callee->parent = e.get();
        e->parent = parser.parent;
        for (auto& arg : e->arguments) arg->parent = e.get();
        e->beg = e->callee->beg;
        e->end = parser.discardLocation;
        return e;
    };
    std::vector<std::unique_ptr<Expression>> arguments;
    if (parser.discard(TokenType::RParen))
        return fn(std::make_unique<CallOperation>(std::move(left),
                                                  std::move(arguments)));
    ...
    return fn(
        std::make_unique<CallOperation>(std::move(left), std::move(arguments)));
}

std::unique_ptr<Expression> ScopeOperationParselet::parse(
    Parser& parser, std::unique_ptr<Expression> left, Token tk) {
    auto as_name = dynamic_cast<NameExpression*>(left.get());
    if (!as_name) {
        parser.error("Expression to the left of '::' must be a name", tk);
        return nullptr;
    }
    ...
    if (curly && curly->type == TokenType::LCurly) {
        ...
        return Expression::attachSLAndParent(
            std::make_unique<ObjectLiteral>(std::move(original_type),
                                            std::move(body)),
            tk.loc, parser.discardLocation, parser.parent);
    }
    return Expression::attachSLAndParent(
        std::make_unique<ScopeOperation>(std::move(original_type)), left->beg,
        curly->loc, parser.parent);
}
std::unique_ptr<Expression> AsExpressionParselet::parse(
    Parser& parser, std::unique_ptr<Expression> left, Token tk) {
    auto bg = left->beg;
    return Expression::attachSLAndParent(
        std::make_unique<AsExpression>(std::move(left),
                                       parser.parseType(0).value_or(Type{})),
        bg, parser.Peek()->loc, parser.parent);
}

std::unique_ptr<Expression> SubscriptParselet::parse(
    Parser& parser, std::unique_ptr<Expression> left, Token tk) {
    auto expr = parser.parseExpression(0);
    if (!parser.discard(TokenType::RSquare))
        parser.error("Expected ']'", parser.Peek());
    return Expression::attachSLAndParent(
        std::make_unique<SubscriptOperation>(std::move(left), std::move(expr)),
