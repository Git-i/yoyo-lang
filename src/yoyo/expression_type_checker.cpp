...
ExpressionTypeChecker::Result peerResolve(std::ranges::forward_range auto types,
    ...
    for (const auto& type :
        ...
        if (!result.is_assignable_from(tp, irgen)) {
            if (tp.is_assignable_from(result, irgen))
                result = tp;
            else
                return {Error(type.second, "")};
        }
    }
    return {result};
}
// defined in type.cpp
extern std::vector<std::string_view> split(std::string_view str,
...
std::optional<Type> canBinOpLiteral(const Type& a, const Type& b) {
    ...
    if (b.name == "ilit" || b.name == "flit") return canBinOpLiteral(b, a);
    return std::nullopt;
}
static std::optional<Type> checkAddition(const Type& a, const Type& b,
    ...
    if (ty) return ty->result;
    return std::nullopt;
}
static std::optional<Type> checkRange(const Type& a, const Type& b,
    ...
    if (a.is_integral() && b.is_integral()) {
        ...
        if (a.name == "ilit" && b.name == "ilit")
            type_name = "range_i32";
        else if (a.is_assignable_from(b, irgen))
            type_name = "range_" + a.name;
        else if (b.is_assignable_from(a, irgen))
            type_name = "range_" + b.name;
        ;
...
static std::optional<Type> checkAssign(const Type& a, const Type& b,
    ...
    return Type{.name = "void",
                .module = a.module->engine->modules.at("core").get()};
}
...
static std::optional<Type> checkShr(const Type& a, const Type& b,
                                    IRGenerator* irgen) {
    if (a.name == "ilit" && b.name == "ilit") return a;
...
static std::optional<Type> checkShl(const Type& a, const Type& b,
                                    IRGenerator* irgen) {
    if (a.name == "ilit" && b.name == "ilit") return a;
...
static std::optional<Type> checkCmp(const Type& a, const Type& b,
                                    IRGenerator* irgen, TokenType tk) {
    auto bool_ty =
        Type{.name = "bool",
             .module = irgen->module->engine->modules.at("core").get()};
    if (a.name == "ilit" && b.name == "ilit") return bool_ty;
    if (a.is_equal(b) && tk == TokenType::DoubleEqual ||
        tk == TokenType::BangEqual) {
        if (a.get_decl_if_enum()) return bool_ty;
    }
    auto ty = resolveCmp(a, b, irgen).second;
    if (!ty) return std::nullopt;
    if (tk == TokenType::DoubleEqual || tk == TokenType::BangEqual)
        return bool_ty;  // all cmps support equality checks
    if (tk == TokenType::Spaceship) return ty->result;
    if (ty->result.name == "CmpOrd" || ty->result.name == "CmpPartOrd")
        return bool_ty;
...
static std::optional<Type> unaryNegateResult(ModuleBase* mod, const Type& typ) {
    ...
    if (typ.is_signed_integral() || typ.is_floating_point())
        return typ.strip_lvalue();

    return std::nullopt;
}
static std::optional<Type> unaryNotResult(ModuleBase* module,
                                          const Type& type) {
    if (type.is_boolean()) return type;
    return std::nullopt;
}
ExpressionTypeChecker::Result ExpressionTypeChecker::operator()(
    ArrayLiteral* lit) {
    ...
    if (std::holds_alternative<repeat_notation>(lit->elements)) {
        ...
        auto type =
            std::visit(ExpressionTypeChecker{irgen}, expr.second->toVariant());
        if (!type) return type;
        if (!type->is_integral())
            return {Error(lit, "Size must be an integral constant")};
        auto size =
            std::visit(ConstantEvaluator{irgen}, expr.second->toVariant());
        ...
        if (std::holds_alternative<int64_t>(size.internal_repr)) {
            ...
            err.markers.emplace_back(
                SourceSpan{expr.second->beg, expr.second->end},
                "This expression evaluated to " + std::to_string(val));
            return {std::move(err)};
            sz = val;
        } else
            sz = std::get<uint64_t>(size.internal_repr);
        if (target && target->is_array()) {
            auto type =
                std::visit(ExpressionTypeChecker{irgen, target->subtypes[0]},
                           expr.first->toVariant());
            if (!type) return type;
            return {
                Type{.name = "__arr_s" + std::to_string(sz),
                     .subtypes = {type.value()},
    ...
    auto subtype = peerResolve(
        elements |
            std::views::transform([this](std::unique_ptr<Expression>& elem) {
                bool swapped = false;
                if (target && target->is_array()) {
                    swapped = true;
                    std::swap(*target, target->subtypes[0]);
                }
                return std::make_pair(std::visit(*this, elem->toVariant()),
                                      elem.get());
    ...
    if (!subtype) {
        ...
        err.markers.emplace_back(
            subtype.error().span,
            "This element is not convertible to the others encountered");
        return {err};
    }
    return {
        Type{.name = "__arr_s" + std::to_string(elements.size()),
             .subtypes = {*subtype},
             .module = subtype->module->engine->modules.at("core").get()}
    };
}

std::optional<FunctionType> checkDot(BinaryOperation* expr, const Type& lhs,
                                     IRGenerator* irgen) {
    ...
    if (lhs.deref().is_tuple()) {
        if (auto idx = dynamic_cast<IntegerLiteral*>(expr->rhs.get())) {
            return lhs.deref()
                .subtypes[std::stol(std::string{idx->text})]
                .take_mutability_characteristics(lhs.deref());
        }
        return std::nullopt;
    }
    ...
    if (name_expr) {
        ...
        if (auto [block, fn] =
                lhs.deref().module->findFunction(this_block, name_expr->text);
            fn)
            if (block == this_block && !fn->sig.parameters.empty())
                if (fn->sig.parameters[0].name == "this") {
                    if (lhs.deref().module != irgen->module &&
                        fn->is_private()) {
                        return std::nullopt;
                    }
    ...
    if (rhs->is_interface_function() && cls) {
        ...
        std::string name = rhs->name.substr("__interface_fn"sv.size(),
                                            pos - "__interface_fn"sv.size());
        std::string fn_name = rhs->name.substr(pos + 1);
        auto it = std::ranges::find_if(cls->impls, [&name](auto& tp) {
            return (tp.impl_for.block_hash + tp.impl_for.name +
                    IRGenerator::mangleGenericArgs(tp.impl_for.subtypes)) ==
                   name;
        });
        if (it == cls->impls.end()) return std::nullopt;
        auto method = std::ranges::find_if(
            it->methods, [&fn_name](auto& tp) { return tp->name == fn_name; });
...
ExpressionTypeChecker::Result checkUnionVar(CallOperation* op, Type callee_ty,
                                            IRGenerator* irg) {
    using namespace std::string_view_literals;
    size_t dollar_off = callee_ty.name.find_first_of('$');
    std::string variant = std::string(callee_ty.name.begin() + dollar_off + 1,
                                      callee_ty.name.end());
    std::string type_name =
    ...
    if (op->arguments.size() != 1)
        return {Error(op, "Union initializer must have one argument")};
    auto arg_ty =
        std::visit(ExpressionTypeChecker{irg, decl->fields.at(variant)},
                   op->arguments[0]->toVariant())
            .value_or_error();
    if (!decl->fields.at(variant).is_assignable_from(arg_ty, irg)) {
        return {
            Error(op, "Union field cannot be initialized with provided type")};
    }
...
ExpressionTypeChecker::Result ExpressionTypeChecker::operator()(
    ...
    if (expr->op.type != TokenType::Dot)
        rhs = std::visit(*this, expr->rhs->toVariant()).value_or_error();
    ...
    case Plus:
        result = checkAddition(lhs, rhs, irgen);
        break;
    case Star:
        result = checkStar(lhs, rhs, irgen);
        break;
    case Minus:
        result = checkMinus(lhs, rhs, irgen);
        break;
    case Slash:
        result = checkDivide(lhs, rhs, irgen);
        break;
    case Percent:
        result = checkPercent(lhs, rhs, irgen);
        break;
    case DoubleGreater:
        result = checkShr(lhs, rhs, irgen);
        break;
    case DoubleLess:
        result = checkShl(lhs, rhs, irgen);
        break;
    case DoubleEqual:
        [[fallthrough]];
    case GreaterEqual:
    ...
    case Spaceship:
        [[fallthrough]];
    case BangEqual:
        result = checkCmp(lhs, rhs, irgen, expr->op.type);
        break;
    case Pipe:
        result = checkBitOr(lhs, rhs);
        break;
    case Caret:
        result = checkBitXor(lhs, rhs);
        break;
    case Ampersand:
        result = checkBitAnd(lhs, rhs);
        break;
    case Dot:
        result = checkDot(expr, lhs, irgen);
        break;
    case Equal:
        result = checkAssign(lhs, rhs, irgen);
        break;
    case DoubleDot:
        [[fallthrough]];
    case DoubleDotEqual:
        result = checkRange(lhs, rhs, expr->lhs.get(), expr->rhs.get(), irgen);
        break;
    default:;  // TODO
    }
    if (result) return {std::move(result).value()};
    if (!expr->op.is_assignment()) {
        Error err(expr, "No operator '" + std::string{expr->op.text} +
                            "' exists for the given operands");
        err.markers.emplace_back(SourceSpan{expr->lhs->beg, expr->lhs->end},
                                 "Expression is of type: '" +
                                     lhs.pretty_name(irgen->block_hash) + "'");
        err.markers.emplace_back(SourceSpan{expr->rhs->beg, expr->rhs->end},
                                 "Expression is of type: '" +
                                     rhs.pretty_name(irgen->block_hash) + "'");
        return {err};
    } else {
        if (!lhs.is_mutable) {
        ...
        err.markers.emplace_back(
            SourceSpan{expr->lhs->beg, expr->lhs->end},
            "Expression is of type " + lhs.pretty_name(irgen->block_hash));
        err.markers.emplace_back(
            SourceSpan{expr->rhs->beg, expr->rhs->end},
            "Expression is of type " + rhs.pretty_name(irgen->block_hash));
        return {err};
    }
}

ExpressionTypeChecker::Result ExpressionTypeChecker::operator()(
    LogicalOperation* expr) {
    auto lhs = std::visit(*this, expr->lhs->toVariant()).value_or_error();
    ...
    Error err(
        expr,
        "Logical operators '&&' and '||' are only available with booleans");
    err.markers.emplace_back(SourceSpan{expr->lhs->beg, expr->lhs->end},
                             "Expression is of type: " + lhs.full_name());
    err.markers.emplace_back(SourceSpan{expr->rhs->beg, expr->rhs->end},
                             "Expression is of type: " + rhs.full_name());
    return {std::move(err)};
}
...
void generic_replace(Type& type, const std::string& generic,
                     const Type& other) {
    if (type.name == generic) type = other;
    for (auto& sub : type.subtypes) generic_replace(sub, generic, other);
}
ExpressionTypeChecker::Result ExpressionTypeChecker::operator()(
    GenericNameExpression* expr) {
    // todo: inline generic functions (probably disallow those)
    if (auto [hash, decl] =
            irgen->module->findGenericFn(irgen->block_hash, expr->text);
        decl) {
        if (expr->arguments.size() != decl->clause.types.size()) {
            return {Error(
                expr, "Generic argument count mismatch",
                std::format("'{}' was declared with {} arguments here: "
                            "@yspn[{}:{},{}:{}]",
                            expr->text, decl->clause.types.size(),
                            decl->beg.line, decl->beg.column,
                            decl->body->beg.line, decl->body->beg.column))};
        }

        irgen->generateGenericFunction(irgen->module, hash, decl,
                                       expr->arguments);
        if (auto [_, fn_decl] = irgen->module->findFunction(
                hash,
                expr->text + IRGenerator::mangleGenericArgs(expr->arguments));
            fn_decl) {
...
ExpressionTypeChecker::Result ExpressionTypeChecker::operator()(
    ...
    if (tp.is_array() && idx.is_integral() && !idx.is_signed_integral())
        return {tp.is_mutable ? tp.subtypes[0].mutable_reference_to()
                              : tp.subtypes[0].reference_to()};
    if (tp.deref().is_array() && idx.is_integral() && !idx.is_signed_integral())
        return {tp.is_mutable_reference()
                    ? tp.deref().subtypes[0].mutable_reference_to()
                    : tp.deref().subtypes[0].reference_to()};
    if (tp.is_mut_slice()) return {tp.subtypes[0].mutable_reference_to()};
    if (tp.is_slice()) return {tp.subtypes[0].reference_to()};
    if (tp.is_error_ty()) return {tp};
    ...
    if (expr_ty.is_mutable || expr_ty.is_mutable_reference()) {
        ovl =
            resolveIdxMut(expr_ty.mutable_reference_to(), idx_ty, irgen).second;
    }
    ...
    auto err = Error(op, "Operator [] does not exist between types " +
                             obj_name + " and " + idx_name);
    err.markers.emplace_back(SourceSpan{op->object->beg, op->object->end},
                             "Expression is of type " + obj_name);
    err.markers.emplace_back(SourceSpan{op->index->beg, op->index->end},
                             "Expression is of type " + idx_name);
    return {err};
}
ExpressionTypeChecker::Result ExpressionTypeChecker::operator()(
...
// If its the first time the hash doesn't have to match and we can check the
// engine for modules but on subsequent attemps we can't check the engine
// and the hashes have to be exact
bool advanceScope(Type& type, ModuleBase*& md, std::string& hash,
    ...
    if (auto dets = md->findClass(hash, type.name); dets.second) {
        if (!first && std::get<0>(*dets.second) != hash + type.name + "::")
            return false;
    ...
    if (auto [this_hash, fn] = md->findGenericFn(hash, type.name); fn) {
        ...
        for (auto& sub : type.subtypes) sub.saturate(md, irgen);
        auto mangled_name =
            fn->name + IRGenerator::mangleGenericArgs(type.subtypes);
        if (auto [_, exists] = md->findFunction(this_hash, mangled_name);
            !exists)
...
ExpressionTypeChecker::Result ExpressionTypeChecker::operator()(
    ...
    while (!iterator.is_end()) {
        ...
        if (!advanceScope(type, md, hash, irgen, false)) {
            return {Error(scp, "The name '" + type.name +
    ...
    if (hash.ends_with("%%interface")) {
        auto pos =
            hash.find_last_of('%', hash.size() - "%%interface"sv.size() - 1);
        auto name = hash.substr(pos + 1,
                                hash.size() - "%%interface"sv.size() - pos - 1);
        ...
        if (it == interface->methods.end())
            return {Error(scp, "No method name '" + last.name +
                                   "' in the specified interface")};
        return {Type{"__interface_fn" + actual_hash + interface->name + "$" +
                     last.name}};
    }
    // is_enum
    if (auto [actual_hash, enm] = md->findEnum(hash, second_to_last); enm) {
        if (md != irgen->module && enm->is_private())
            return {Error(scp, "The enum type " + actual_hash +
                                   enm->identifier + " is private")};
        if (!last.subtypes.empty())
            return {Error(scp, "Enum child cannot have subtypes")};
        if (enm->values.contains(last.name))
            return {
                Type{.name = second_to_last,
                     .module = md,
    ...
    if (auto [actual_hash, unn] = md->findUnion(hash, second_to_last); unn) {
        if (md != irgen->module && unn->is_private())
            return {Error(scp, "The union type " + actual_hash + unn->name +
                                   " is private")};
        if (!last.subtypes.empty())
            return {Error(scp, "Union child cannot have subtypes")};
    ...
    if (auto [name, fn] = md->findFunction(hash, last.name); fn) {
        if (md != irgen->module && fn->is_private())
            return {Error(
                scp, "The function type " + name + fn->name + " is private")};
    ...
    return {Error(scp, "The name '" + last.name + "' does not exist in \"" +
                           hash + "\"")};
}
ExpressionTypeChecker::Result ExpressionTypeChecker::operator()(
    ObjectLiteral* obj) {
    ...
    if (!decl)
        return {
            Error(obj, "The type tp does not exist or is not a class/struct")};
    if (obj->t.module != irgen->module && decl->is_private())
        return {Error(obj, "The type " + obj->t.full_name() + " is private")};
    ...
    for (auto& var : decl->vars) {
        if (!obj->values.contains(var.name))
            return {
                Error(obj, "Type tp has no member named '" + var.name + "'")};
        auto& expr = obj->values.at(var.name);
        auto expr_t = std::visit(ExpressionTypeChecker{irgen, var.type},
                                 expr->toVariant())
                          .value_or_error();
        auto as_mut = var.type;
        as_mut.is_mutable = true;
        as_mut.is_lvalue = true;
...
ExpressionTypeChecker::Result ExpressionTypeChecker::operator()(
    ...
    if (auto decl = from.get_decl_if_union()) {
        if (!expr->dest.subtypes.empty())
            return {Error(expr,
                          "'as' expressions for unions must use only the "
                          "variant name")};
        if (!expr->dest.block_hash.empty())
            return {Error(expr,
                          "'as' expressions for unions must use only the "
                          "variant name")};
        if (!decl->fields.contains(expr->dest.name))
            return {Error(expr, "Specified field does not exist in the union")};
        return {
            Type{"__conv_result_ref",
                 {decl->fields.at(expr->dest.name)},
                 nullptr, irgen->module->engine->modules.at("core").get(),
                 from.is_mutable,
                 from.is_lvalue}
        };
    }
    ...
    if (expr->dest.is_view()) {
        ...
        if (!from.is_reference())
            return {
                Error(expr, "Interface cast must be from a reference type")};
        if (auto cls = from.deref().get_decl_if_class(irgen)) {
            ...
            if (it != cls->impls.end()) {
                if (expr->dest.is_mut_view())
                    if (from.is_mutable_reference() || from.is_gc_reference())
                        return {expr->dest};
                if (expr->dest.is_gc_view())
                    if (from.is_gc_reference()) return {expr->dest};
                if (expr->dest.is_view()) return {expr->dest};
                return {Error(expr, "Reference characteristic mismatch")};
            }
            return {Error(expr,
                          "Cannot perform interface cast as class does not "
                          "implement interface")};
        }
    }
...
ExpressionTypeChecker::Result ExpressionTypeChecker::operator()(
    ...
    bool consider_target = target && target->is_tuple() &&
                           target->subtypes.size() == tup->elements.size();
    Type tp{.name = "__tup",
            .module = irgen->module->engine->modules.at("core").get()};
    for (size_t i = 0; i < tup->elements.size(); ++i) {
        auto type_i = std::visit(ExpressionTypeChecker{irgen},
                                 tup->elements[i]->toVariant())
                          .value_or_error();
        // if we can implicit convert to the target type we use that
        if (consider_target &&
            target->subtypes[i].is_assignable_from(type_i, irgen)) {
            tp.subtypes.push_back(target->subtypes[i]);
            continue;
        }
        if (type_i.name == "ilit" || type_i.name == "flit") {
            auto val = std::visit(ConstantEvaluator{irgen},
                                  tup->elements[i]->toVariant());
...
bool ExpressionTypeChecker::hasToStr(const Type& t) {
    ...
    if (t.is_tuple()) {
        for (const auto& t : t.subtypes)
            if (!hasToStr(t)) return false;
        return true;
    }
...
ExpressionTypeChecker::Result ExpressionTypeChecker::operator()(
    ...
    case Ampersand: {
        ...
        return {
            Type{.name = "__ref",
                 .subtypes = {std::move(op_type)},
                 .module = irgen->module->engine->modules.at("core").get()}
        };
    }
    case RefMut: {
        // if(op_type.is_reference()) return std::nullopt;
        if (!op_type.is_mutable)
            return {
                Error(op, "Cannot mutably reference an immutable expression")};
        return {
            Type{.name = "__ref_mut",
                 .subtypes = {std::move(op_type)},
                 .module = irgen->module->engine->modules.at("core").get()}
...
// takes two types that are (supposed to be) equal but one is generic and
// returns a list of substitutions
std::optional<std::unordered_map<std::string, Type>> genericMatch(
    const Type& generic, const Type& resolved, std::span<std::string> generics,
    IRGenerator* irgen) {
    if (auto it = std::ranges::find_if(
            generics,
            [&generic](const std::string& val) { return val == generic.name; });
        it != generics.end()) {
        return {{{*it, resolved}}};
    }
    ...
    if (!it.is_end()) {
        auto first =
            std::string(generic.name.begin(),
                        generic.name.begin() + generic.name.find_first_of(':'));
        ...
        while (!it.is_end()) {
            ...
            Type rtp = resolved_it.is_end() ? resolved_it.last(true)
                                            : resolved_it.next();
            ...
            for (size_t i = 0; i < gtp.subtypes.size(); i++) {
                ...
                auto match = genericMatch(gtp.subtypes[i], rtp.subtypes[i],
                                          generics, irgen);
    ...
    if (it.is_end()) {
        ...
        for (size_t i = 0; i < tp.subtypes.size(); i++) {
            auto match = genericMatch(tp.subtypes[i], resolved.subtypes[i],
                                      generics, irgen);
            if (!match) return std::nullopt;
            for (auto& [name, tp] : *match) {
                if (!results.contains(name)) {
...
struct ConstraintInferenceChecker {
    ...
    bool operator()(ImplConstraint& impl_con) {
        // If the type hasn't been resolved yet we skip it, this does mean
        // that we may require more passes
        ...
        for (auto& intf : cls->impls) {
            auto match =
                genericMatch(impl_con.other, intf.impl_for, generics, irgen);
            if (match) {
                for (auto& [name, tp] : *match) {
                    if (!results.contains(name)) {
                        results[name] = tp;
...
std::optional<std::vector<Type>> deduceTypeArgs(const FunctionType& generic_fn,
                                                std::span<Type> input,
                                                IRGenerator* irgen) {
    if (generic_fn.sig.parameters.size() == 0) return std::nullopt;
    constexpr std::string_view gfn = "__generic_fn";
    std::string name(generic_fn.name.begin() + gfn.size(),
                     generic_fn.name.end());
    auto decl =
        generic_fn.module->findGenericFn(generic_fn.block_hash, name).second;
    std::unordered_map<std::string, Type> results;
    // we try to infer substitutions from parameter types first
    for (size_t i = 0; i < input.size(); i++) {
        auto match = genericMatch(generic_fn.sig.parameters[i].type, input[i],
                                  decl->clause.types, irgen);
        if (!match) return std::nullopt;
        for (auto& [name, tp] : *match) {
            if (!results.contains(name)) {
                results[name] = tp;
    ...
    // after the above not all types may have substitutions, so we try to
    // infer other substitution from constrinats we return if all types have
    ...
    for (auto [type, constraints] : decl->clause.constraints) {
        for (auto& con : constraints) {
            auto success = std::visit(
                ConstraintInferenceChecker{results, type, decl->clause.types,
                                           irgen},
                con);
...
ExpressionTypeChecker::Result ExpressionTypeChecker::operator()(
    ...
    auto callee_ty =
        std::visit(*this, op->callee->toVariant()).value_or_error();
    if (callee_ty.is_error_ty()) return {std::move(callee_ty)};

    if (callee_ty.name.starts_with("__union_var"))
        return checkUnionVar(op, std::move(callee_ty), irgen);

    if (!callee_ty.is_function() && !callee_ty.is_lambda() &&
        !callee_ty.name.starts_with("__generic_fn"))
        return {
            Error(op->callee.get(), "Attempt to call non-function expression")};
    auto& as_fn = reinterpret_cast<FunctionType&>(callee_ty);
    // If the function is bound (something.function()) we skip checking the
    // first args type
    if (op->arguments.size() + callee_ty.is_bound !=
        as_fn.sig.parameters.size()) {
    ...
    if (callee_ty.name.starts_with("__generic_fn")) {
        ...
        for (auto& arg : op->arguments)
            inputs.emplace_back(
                std::visit(*this, arg->toVariant()).value_or_error());
        auto args = deduceTypeArgs(callee_ty, inputs, irgen);
        if (!args) {
            op->callee = nullptr;  // we null the calle as a marker that
                                   // we've done this before
            return {Error(op, "Could not deduce generic args")};
        }
        // modify the node if possible to prevent doing this gymnastics over
        // and over
        if (auto as_bin = dynamic_cast<BinaryOperation*>(op->callee.get())) {
            if (auto as_name =
                    dynamic_cast<NameExpression*>(as_bin->rhs.get())) {
                auto beg = as_name->beg, end = as_name->end;
                as_bin->rhs = std::make_unique<GenericNameExpression>(
                    as_name->text, *args);
                as_bin->rhs->beg = beg;
                as_bin->rhs->end = end;
            }
        } else if (auto as_name =
                       dynamic_cast<NameExpression*>(op->callee.get())) {
            auto beg = as_name->beg, end = as_name->end;
            op->callee =
                std::make_unique<GenericNameExpression>(as_name->text, *args);
            op->callee->beg = beg;
            op->callee->end = end;
        }
        return (*this)(op);
    ...
    for (size_t i = 0; i < op->arguments.size(); ++i) {
        auto tp =
            std::visit(*this, op->arguments[i]->toVariant()).value_or_error();
        if (!as_fn.sig.parameters[i + callee_ty.is_bound]
                 .type.can_accept_as_arg(tp, irgen)) {
            auto& type = as_fn.sig.parameters[i + callee_ty.is_bound].type;
            Error err(op->arguments[i].get(),
                      "Cannot convert argument to expected parameter type");
            err.markers.emplace_back(
                SourceSpan{op->callee->beg, op->callee->end},
                "Expected argument of type '" +
                    type.pretty_name(irgen->block_hash) +
                    "' becuase function is of type '" +
                    as_fn.sig.pretty_name(irgen->block_hash) + "'");
            err.markers.emplace_back(
                SourceSpan{op->arguments[i]->beg, op->arguments[i]->end},
                "Expression of type '" + tp.pretty_name(irgen->block_hash) +
                    "'");
            return {err};
        }
    }
...
ExpressionTypeChecker::Result ExpressionTypeChecker::operator()(
    ...
    for (auto& val : lit->literal) {
        if (std::holds_alternative<std::unique_ptr<Expression>>(val)) {
            ...
            if (!hasToStr(ty))
                return {Error(expr.get(),
                              "Expression cannot be converted to string")};
        }
    }
    return {
        Type{.name = "str",
...
ExpressionTypeChecker::Result ExpressionTypeChecker::operator()(
    ...
    auto internal_ty = std::visit(*this, expr->target_expression->toVariant())
                           .value_or_error();
    if (internal_ty.is_error_ty()) return {internal_ty};
    if (internal_ty.is_non_owning(irgen)) {
        Error err(expr,
                  "Cannot store non-owning type with the garbage collector");
        err.markers.emplace_back(
            SourceSpan{expr->target_expression->beg,
                       expr->target_expression->end},
            "Expression is of type '" +
                internal_ty.pretty_name(irgen->block_hash) +
                "' which is non owning");
        return {err};
    }
...
ExpressionTypeChecker::Result ExpressionTypeChecker::operator()(
    SpawnExpression* sxr) {
    auto inner_type =
        std::visit(*this, sxr->call_expr->toVariant()).value_or_error();
    return {
        Type{
             .name = "Fiber",
             .subtypes = {std::move(inner_type)},
             .module = irgen->module->engine->modules.at("core").get(),
             .block_hash = "core::",
             }
    };
