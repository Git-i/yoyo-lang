#include "ir_gen.h"
namespace Yoyo {
bool TopLevelVisitor::operator()(
    std::unique_ptr<FunctionDeclaration> decl) const {
...
bool TopLevelVisitor::operator()(std::unique_ptr<ConstantDeclaration> decl) {
    ...
    if (!decl->type.is_integral() && !decl->type.is_boolean() &&
        !decl->type.is_floating_point() && !decl->type.is_char()) {
        irgen->error(
            Error(decl.get(),
                  "Constants must be integers, booleans, chars or floats"));
        return false;
    }
    auto tp = irgen->ToLLVMType(decl->type, false);
    auto val = std::visit(ConstantEvaluator{irgen}, decl->expr->toVariant());

    if (decl->type.is_integral() && !val->getType()->isIntegerTy() ||
        val->getType() == llvm::Type::getInt1Ty(irgen->context)) {
        irgen->error(
            Error(decl.get(), "Expression does not evaluate to integer type"));
        return false;
    }
    if (decl->type.is_integral() || decl->type.is_floating_point()) {
        ...
        if (const_int) {
            if (const_int->isNegative()) {
                if (decl->type.is_unsigned_integral()) {
                    irgen->error(
                        Error(decl.get(), "Resultant value is negative"));
                    return false;
                }
                int64_t val_int = const_int->getSExtValue();
                auto max = getIntMinOf(decl->type);
                if (val_int < max) {
                    irgen->error(Error(decl.get(),
                                       "Resultant value too small for type"));
                    return false;
                }
            } else {
                ...
                if (val_int > max) {
                    irgen->error(Error(decl.get(),
                                       "Resultant value too large for type"));
                    return false;
                }
            }
        ...
        if (fp) {
            val = llvm::dyn_cast<llvm::Constant>(
                irgen->builder->CreateFPTrunc(val, tp));
        }
    }

    irgen->module->constants[irgen->block_hash].emplace_back(
        std::move(decl->type), std::move(decl->name), val);
    return true;
}
bool TopLevelVisitor::operator()(std::unique_ptr<ClassDeclaration> decl) const {
    ...
    if (!decl->methods.empty())
        irgen->module->functions.at(std::get<0>(*ty)).clear();
    ...
    for (auto& fn : decl_ptr->methods) {
        auto fn_decl =
            reinterpret_cast<FunctionDeclaration*>(fn.function_decl.get());
    ...
    for (auto& impl : decl_ptr->impls) {
        ...
        for (auto& mth : impl.methods) {
            ...
            if (it == interface->methods.end())
                ...
                irgen->saturateSignature((*it)->signature,
                                         impl.impl_for.module);
                irgen->in_class = true;
                if (!implementsInterfaceMethod(mth->signature,
                                               (*it)->signature))
                    irgen->error(Error(mth.get(),
...
bool handleBinaryOverload(OperatorOverload* decl, IRGenerator* irgen) {
    if (decl->signature.parameters.size() != 2)
        irgen->error(
            Error(decl, "More or less than 2 operands for binary operator"));
    ...
    if (decl->tok == TokenType::Spaceship) {
        std::string err =
            "Comparison operator overload must return either core::CmpEq, "
            "core::CmpOrd or core::CmpPartOrd";
    ...
    FunctionDeclaration fn_decl(name, std::move(decl->signature),
                                std::move(decl->body));
    (*irgen)(&fn_decl);
...
bool TopLevelVisitor::operator()(std::unique_ptr<OperatorOverload> decl) {
    ...
    if (!std::ranges::any_of(
            decl->signature.parameters, [this](FunctionParameter& param) {
                return param.type.deref().module == irgen->module;
            })) {
        irgen->error(Error(
            decl.get(), "This module does not own any type being overloaded"));
        return false;
    }
    ...
    if (decl->tok == TokenType::Minus) {
        if (decl->signature.parameters.size() == 2)
            return handleBinaryOverload(decl.get(), irgen);
        if (decl->signature.parameters.size() != 1) {
            irgen->error(Error(decl.get(),
                               "Unary '-' with too many / too few operands"));
            return false;
        }
    }
    debugbreak();
