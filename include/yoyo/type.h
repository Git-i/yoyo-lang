...
struct YOYO_API Type {
    ...
    // shared ptr because I want the object to be copyable, its mostly
    // nullptr anyway this is to store the signature for callable and
    // storable functions
    ...
    [[nodiscard]] bool is_assignable_from(const Type& other,
                                          IRGenerator* irgen) const;
    ...
    /// returns a dereferenced version of a reference type or itself if not
    /// a reference
    ...
    [[nodiscard]] bool is_mutable_reference() const {
        return name == "__ref_mut";
    }
    ...
    /// Merges the types into one variant type or returns one of them if
    /// they're equal if @c a is already a variant it adds @c b to the
    /// list(if @c b is not already there) if @c b is a variant is does the
    /// same if not it makes @c a a variant with subtypes @c a and @c b
...
struct UnsaturatedTypeIterator {
    ...
    [[nodiscard]] Type next();
    [[nodiscard]] Type last(bool parse = false);
    [[nodiscard]] uint32_t num_iters();
    size_t pos = 0;
