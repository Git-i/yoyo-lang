...
public:
    ...
    std::vector<std::unordered_map<
        std::string, std::tuple<llvm::Value*, Type, llvm::Value*>>>
        variables;
    ...
    llvm::StructType* hanldeClassDeclaration(
        std::span<const ClassVariable> vars, Ownership own,
...
public:
    ...
    explicit LLVMExpressionEvaluator(LLVMIRGenerator* gen,
                                     std::optional<Type> target = std::nullopt)
        : irgen(gen), target(std::move(target)) {}
    llvm::Value* implicitConvert(Expression* xp, llvm::Value*, const Type&,
                                 const Type&, llvm::Value* = nullptr) const;
    llvm::Value* clone(Expression* xp, llvm::Value* value,
                       const Type& left_type,
                       llvm::Value* into = nullptr) const;
    ...
    llvm::Value* doRange(Expression*, Expression*, const Type&, const Type&,
                         const Type&);
    llvm::Value* doCmp(ComparisonPredicate p, Expression*, Expression*,
                       const Type& left_type, const Type& right_type,
                       const Type&);
    llvm::Value* fillArgs(bool, const FunctionSignature&,
                          std::vector<llvm::Value*>&, llvm::Value*,
