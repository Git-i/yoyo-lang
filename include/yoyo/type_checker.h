...
struct BinaryOperableConstraint {
    ...
    Type result;
    TokenType op;
    // we keep a constraint local cache of all substitutions we've made as
    // they may create new type variables and we want to avoid making too
    // many unnecesary type variables
    std::unordered_map<OperatorOverload*, std::unordered_map<std::string, Type>>
        substitution_cache;
    ASTNode* expr;
};
...
// This is different from `ConvertibleToConstraint` because of union variant
// handling
struct AsConstraint {
    Type input_type;
    Type dest;
...
using TypeCheckerConstraintUnderlyingType = std::variant<
    IsIntegerConstraint, CanStoreIntegerConstraint, IsFloatConstraint,
    CanStoreRealConstraint, ToStringConstraint, HasUnaryMinusConstraint,
    HasUnaryNotConstraint, IsReferenceToConstraint, IsNotReferenceConstraint,
    BinaryOperableConstraint, ComparableConstraint, EqualConstraint,
    OwningConstraint, IsInvocableConstraint, ValidAsFunctionArgConstraint,
    IsReturnOfConstraint, ImplInterfaceConstraint, ExtractsToConstraint,
    RefExtractsToConstraint, NonOwningConstraint, AsConstraint,
    ConvertibleToConstraint, BinaryDotCompatibleConstraint,
    ElseExtractsToConstraint, ElseRefExtractsToConstraint,
    IfStatementConstraint, EqualOrIsVoidConstraint, IndexOperableConstraint,
    HasFieldConstraint, AllFieldsConstraint, BorrowResultConstraint,
    BorrowResultMutConstraint, IfEqualThenConstrain>;
struct TypeCheckerConstraint : public TypeCheckerConstraintUnderlyingType {
    template <typename T>
    TypeCheckerConstraint(T&& t) noexcept
        : TypeCheckerConstraintUnderlyingType(t) {}
};
...
struct UnificationTable {
    ...
    uint32_t type_to_id(const Type& tp) {
        ...
        uint32_t res;
        auto [ptr, ec] = std::from_chars(tp.name.c_str() + 1,
                                         tp.name.c_str() + tp.name.size(), res);
        if (ec != std::errc{}) debugbreak();
        return res;
    }
    Type id_to_type(uint32_t id) { return Type{"?" + std::to_string(id)}; }
    ...
    uint32_t unite(uint32_t id, uint32_t id2, IRGenerator* irgen,
                   TypeCheckerState*);
    Domain* domain_of(uint32_t id) { return &domains[find(id)]; }
};
...
struct TypeCheckerState {
    ...
    // (should not be called by the constraint checker, it has a special
    // method for that)
    ...
    // A variation of Type::is_non_owning that is guaranteed to not
...
struct TypeChecker {
    ...
    TypeChecker new_target(std::optional<Type>,
                           bool in_mutable_ctx = false) const;
