...
public:
    ...
    // using Type::<?1>::{ Type2::<?2>::Type3::<?3> as Alias1::<?1, ?2, ?3>,
    // Type2Alt::<?2> as AliasAlt::<?1, ?2> } so the number trackes what
    // generic we're on
    ...
    void synchronizeTo(
        std::span<const TokenType>
            t);  ///< skip all tokens till the next token of type @c t
...
private:
    ...
    std::string& source;
    Scanner scn;
    std::unordered_map<TokenType, std::shared_ptr<PrefixParselet>>
        prefixParselets;
    std::unordered_map<TokenType, std::shared_ptr<InfixParselet>>
        infixParselets;
