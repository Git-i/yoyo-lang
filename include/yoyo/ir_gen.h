...
public:
    ...
    std::unique_ptr<Statement>*
        current_Statement;  // we keep the current the statement in the case
                            // we want to steal it
    ...
    static std::string mangleGenericArgs(const T& list)
        requires std::same_as<std::remove_cvref_t<decltype(*list.begin())>,
                              Type>
    {
        if (std::ranges::empty(list)) return "";
        std::string final = "::<" + (*list.begin()).full_name();
        for (const auto& tp :
             std::ranges::subrange(list.begin() + 1, list.end()))
            final += "," + tp.full_name();
        final += ">";
        return final;
    }
    ...
    virtual std::optional<Type> getVariableType(const std::string& name,
                                                Expression*) = 0;
...
public:
    std::optional<FunctionType> checkNameWithinClassOrModule(
        ModuleBase*, ClassDeclaration* type, std::string_view name);
    std::optional<FunctionType> checkNameWithinEnum(EnumDeclaration* type,
                                                    std::string_view name);
    explicit ExpressionTypeChecker(IRGenerator* gen,
                                   std::optional<Type> target = std::nullopt)
        : irgen(gen), target(std::move(target)) {}
    bool hasToStr(const Type& t);
    /// We return FunctionType because it's a subclass of Type and some
    /// expressions ( @c NameExpression and @c BinaryExpression ) can be a
    /// function
    struct Result : std::variant<FunctionType, Error> {
        operator bool() const {
            return std::holds_alternative<FunctionType>(*this);
        }
        FunctionType* operator->() { return &std::get<0>(*this); }
        FunctionType& value() & {
            if (!*this) debugbreak();
        ...
        const FunctionType& value() const& {
            if (!*this) debugbreak();
            return std::get<0>(*this);
        }
...
struct MacroEvaluator {
    ...
    using VarintTy = std::variant<
        std::monostate, std::string, std::unique_ptr<ASTNode>,
        std::unique_ptr<ASTNode>*, bool, double, int64_t, OwnedToken, TokSeq,
        TokSeq*, std::vector<ObjectTy>, std::vector<ObjectTy>*,
