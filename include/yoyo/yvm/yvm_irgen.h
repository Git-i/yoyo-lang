...
public:
    ...
    std::vector<  //< the list of variables by each scope
                  ...
                  /// the std::pair<> is the variable name and the stack_addr
    ...
    virtual void doFunction(FunctionDeclaration* decl) override {
        (*this)(decl);
    };
    ...
    bool GenerateIR(
        ...
        std::vector<std::unique_ptr<Statement>> statements, YVMModule* md,
        Engine* eng,
...
public:
    ...
    enum ComparisonPredicate { EQ, GT, LT, EQ_GT, EQ_LT, NE, SPACE };
    explicit YVMExpressionEvaluator(YVMIRGenerator* gen)
        : irgen(gen), returned_alloc_addr(size_t(-1)) {}
    /// on_stack means to write it to a pointer on the stack top other wise
    /// put it on new `alloca` if `on_stack` is on, we except the value to
    /// be below the stack top if `on_stack` is off, we expect the value to
    /// be at the stack top either ways the value is the resulting value is
    /// pushed on the stack top
    ///
    /// perform_load controls if we should output a value for non-sret type
    /// (ints) for non-sret types perform_load used with on_stack
    /// effectively duplicates the value perform_load set and on_stack not
    /// set does not allocate perform_load not set and on_stack set does not
    /// allocate perform_load not set and on_stack not set will allocate
    void implicitConvert(Expression* xp, const Type&, const Type&,
                         bool on_stack, bool perform_load);
    void clone(Expression* xp, const Type& left_type, bool on_stack,
               bool perform_load);

    void destroy(const Type& type) const;
    std::vector<Type> doDot(Expression* lhs, Expression* rhs,
                            const Type& left_type, bool load_prim = true);
    std::vector<Type> doAddition(Expression*, Expression*, const Type&,
    ...
    std::vector<Type> doShr(Expression*, Expression*, const Type&, const Type&,
                            OverloadDetailsBinary*);
    std::vector<Type> doMinus(Expression*, Expression*, const Type&,
                              const Type&, OverloadDetailsBinary*);
