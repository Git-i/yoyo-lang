...
public:
    ...
    Result addFunction(std::string sig_str, R (*func)(Args...),
                       std::string name) {
        ...
        size_t param_size =
            sig.returnType.should_sret() + sig.parameters.size();
        size_t provided_size = sizeof...(Args);
        if (param_size != provided_size) return Result::ParameterCountMismatch;
        if (sig.returnType.should_sret()) {
            if (!std::is_same_v<R, void>) return Result::ReturnTypeMismatch;
        }
        addFunction(std::move(sig), reinterpret_cast<void*>(func),
                    std::move(name));
