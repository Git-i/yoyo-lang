...
struct ForwardDeclaratorPass1 {
    ...
    bool operator()(FunctionDeclaration* decl) const {
        ...
        std::visit(ForwardDeclaratorPass1{md, new_blk},
                   decl->body->toVariant());
        return true;
    }
    ...
    bool operator()(ClassDeclaration* decl) const {
        ...
        for (auto& stt : decl->stats) {
            std::visit(ForwardDeclaratorPass1{md, mangled_name_prefix},
                       stt->toVariant());
        }
        md->aliases[mangled_name_prefix]["This"] = Type{.name = decl->name};
        md->classes[block].emplace_back(
            mangled_name_prefix, std::unique_ptr<ClassDeclaration>{decl});
    ...
    bool operator()(ModuleImport* imp) {
        reinterpret_cast<YVMEngine*>(md->engine)
    ...
    bool operator()(OperatorOverload* op) {
        if (op->signature.parameters.size() == 2)
            md->overloads.add_binary_detail_for(
                op->tok,
                OverloadDetailsBinary{op->signature.parameters[0].type,
                                      op->signature.parameters[1].type,
                                      op->signature.returnType, op},
                block);
        else if (op->signature.parameters.size() == 1)
            md->overloads.add_unary_detail_for(
                op->tok,
                OverloadDetailsUnary{op->signature.parameters[0].type,
                                     op->signature.returnType, op},
                block);
        return true;
    }
    ...
    bool operator()(InterfaceDeclaration* decl) {
        ...
        md->aliases[mangled_name_prefix]["This"] =
            Type{.name = "impl", .subtypes = {Type{.name = decl->name}}};
        auto old_hash = std::move(md->module_hash);
        md->module_hash = block;
        md->aliases[mangled_name_prefix]["This"].saturate(md, nullptr);
        md->module_hash = std::move(old_hash);
    ...
    bool operator()(BlockExpression* stat) {
        ...
        if (stat->expr)
            std::visit(ForwardDeclaratorPass1{md, block},
                       stat->expr->toVariant());
        return false;
    }
