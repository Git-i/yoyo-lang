...
using ExpressionVariant = std::variant<
    IntegerLiteral*, BooleanLiteral*, TupleLiteral*, ArrayLiteral*,
    RealLiteral*, StringLiteral*, NameExpression*, PrefixOperation*,
    BinaryOperation*, GroupingExpression*, LogicalOperation*, PostfixOperation*,
    CallOperation*, SubscriptOperation*, LambdaExpression*, ScopeOperation*,
    ObjectLiteral*, NullLiteral*, CharLiteral*, GenericNameExpression*,
    GCNewExpression*, AsExpression*, MacroInvocation*, SpawnExpression*,
    TryExpression*, BlockExpression*, IfExpression*, ConditionalExtraction*>;
...
public:
    ...
    Type evaluated_type;
    static auto attachSLAndParent(std::unique_ptr<Expression> self,
...
public:
    ...
    std::unique_ptr<Expression> lhs;
    std::unique_ptr<Expression> rhs;
    LogicalOperation(const Token& op, std::unique_ptr<Expression> left,
                     std::unique_ptr<Expression> right)
        : op(op), lhs(std::move(left)), rhs(std::move(right)) {}
    ExpressionVariant toVariant() override;
};
...
public:
    ...
    std::string hash;
    LambdaExpression(std::vector<Capture> captures, FunctionSignature sig,
                     std::unique_ptr<Statement> body);
    ExpressionVariant toVariant() override;
};
...
public:
    Type type;
    explicit ScopeOperation(Type tp) : type(std::move(tp)) {}
    ExpressionVariant toVariant() override;
};
...
public:
    ...
    AsExpression(std::unique_ptr<Expression> expr, Type dest)
        : expr(std::move(expr)), dest(std::move(dest)) {}
    ExpressionVariant toVariant() override;
};
...
public:
    ...
    // because control goes elsewhere, we set it to true becaue unreachable
    // nodes are not visited by the CFG generator
...
public:
    ...
    bool then_transfers_control = true;
    bool else_transfers_control = true;
    ConditionalExtraction(std::string name, CaptureType cap,
                          std::unique_ptr<Expression> cond,
                          std::unique_ptr<Expression> body,
                          std::unique_ptr<Expression> else_,
                          std::string else_name = "",
                          CaptureType else_cap = Own)
        : captured_name(std::move(name)),
          then_capture_tp(cap),
          else_capture_tp(else_cap),
          condition(std::move(cond)),
          body(std::move(body)),
          else_capture(std::move(else_name)),
          else_body(std::move(else_)) {}
    ExpressionVariant toVariant() override;
};
}  // namespace Yoyo
