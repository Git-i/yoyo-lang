...
struct YOYO_API Error {
    ...
    Error(SourceSpan spn, std::string sum, std::string desc = "")
        : span{spn}, summary(std::move(sum)), description(std::move(desc)) {}
    Error(SourceLocation beg, SourceLocation end, std::string sum,
          std::string desc = "")
        : span{beg, end},
          summary(std::move(sum)),
          description(std::move(desc)) {}
    Error(ASTNode* node, std::string sum, std::string desc = "")
        : span(node == nullptr ? SourceSpan{}
                               : SourceSpan{node->beg, node->end}),
          summary(std::move(sum)),
          description(std::move(desc)) {}
