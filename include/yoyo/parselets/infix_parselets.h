...
public:
    ...
    virtual std::unique_ptr<Expression> parse(Parser& parser,
...
public:
    std::unique_ptr<Expression> parse(Parser& parser,
                                      std::unique_ptr<Expression> left,
                                      Token tk) override;
    uint32_t precedence() override { return prec; }
    explicit BinaryOperationParselet(uint32_t precedence, bool is_r = false)
        : prec(precedence), is_right(is_r) {}
};
...
public:
    std::unique_ptr<Expression> parse(Parser& parser,
                                      ...
                                      Token tk) override;
    uint32_t precedence() override { return prec; }
    explicit LogicalOperationParselet(uint32_t precedence, bool is_r = false)
        : prec(precedence), is_right(is_r) {}
};
